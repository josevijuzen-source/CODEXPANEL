...
def getLogsFromFile(request):
    try:
        ...
        status = {"status": 0, "logstatus":0,"error":"Could not fetch data from log file, please see CodexCP main log file through command line."}
        logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[getLogsFromFile]")
        final_json = json.dumps(status)
        return HttpResponse(final_json)
        ...
        try:
            if request.method == 'POST':
                ...
                allowedLogFiles = {
                    ...
                    "/var/log/messages",
                    "/var/log/syslog",
                    "/home/codexpanel/error-logs.txt",
                }

                if fileName not in allowedLogFiles:
                    return ACLManager.loadErrorJson('cleanStatus', 0)

                execPath = "/usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/serverLogs.py"
