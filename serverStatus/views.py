...
def litespeedStatus(request):
    try:
        ...
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[litespeedStatus]")
            proc = httpProc(request, 'serverStatus/litespeedStatus.html',
                            {"processList": processList,
                             "liteSpeedVersionStatus": "For some reaons not able to load version details, see CodexCP main log file.",
    ...
    try:
        ...
        if reboot == 1:
            if installUtilities.reStartLiteSpeedSocket() == 1:
                ...
                status = {"reboot": 0, "shutdown": 0, "error_message": "Please see CodexCP main log file."}
        else:
        ...
        logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[stopOrRestartLitespeed]")
        return HttpResponse("Not Logged in as admin")


def CodexCPMainLogFile(request):
    proc = httpProc(request, 'serverStatus/CodexCPmainlogfile.html', None, 'admin')
    return proc.render()
...
def getFurtherDataFromLogFile(request):
    try:
        ...
        fewLinesOfLogFile = logging.CodexCPLogFileWriter.readLastNFiles(50, logging.CodexCPLogFileWriter.fileName)
        ...
        status = {"logstatus": 0,
                  "error": "Could not fetch data from log file, please see CodexCP main log file through command line."}
        logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[getFurtherDataFromLogFile]")
        return HttpResponse("Not Logged in as admin")
...
def switchTOLSWS(request):
    try:
        ...
        execPath = "sudo /usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/serverStatus/serverStatusUtil.py"
...
def changeLicense(request):
    try:
        ...
        try:
            ...
            command = 'sudo chown -R codexpanel:codexpanel /usr/local/lsws/conf'
            ProcessUtilities.executioner(command)
...
def topProcessesStatus(request):
    try:
        ...
        with open("/home/codexpanel/top", "w") as outfile:
            subprocess.call("top -n1 -b", shell=True, stdout=outfile)

        data = open('/home/codexpanel/top', 'r').readlines()
        ...
        data['CodexPanelVersion'] = 'v%s.%s' % (VERSION, str(BUILD))
    ...
    try:
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
            ...
            if type == 'codexpanel':
                ...
                command = 'cat /usr/local/CodexCP/AllCPUbuntu.json'
            ...
            if type == 'installed':
                ...
                for items in packages:
                    if items == '':
                        ...
                        startForUpdate = startForUpdate + 1
            elif type == 'codexpanel':
                command = 'cat /usr/local/CodexCP/CPCent7repo.json'
                packages = json.loads(ProcessUtilities.outputExecutioner(command))

        ## make list of packages that need update

        # if os.path.exists(ProcessUtilities.debugPath):
        #    logging.CodexCPLogFileWriter.writeToFile('All packages: %s' % (str(packages)))
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.CodexCPLogFileWriter.writeToFile('Final packages: %s' % (str(finalPackages)))
        ...
        for items in finalPackages:
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile('[ERROR] %s. [fetchPackages:773]' % (str(msg)))
            elif ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                try:
                    ...
                    print(str(msg))
                    logging.CodexCPLogFileWriter.writeToFile('[ERROR] %s. [fetchPackages:839]' % (str(msg)))
...
def lockStatus(request):
    try:
        ...
        if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
            ...
            yumConfTmp = '/home/codexpanel/yumTemp'
...
def codexpanelPort(request):
    port = ProcessUtilities.fetchCurrentPort()
    proc = httpProc(request, "serverStatus/changecodexpanelPort.html", {'port': port}, 'admin')
    return proc.render()
