#!/bin/bash
#codexpanel utility script

export LC_CTYPE=en_US.UTF-8
SUDO_TEST=$(set)
BRANCH_NAME="stable"
GIT_URL="github.com/josevijuzen-source/CODEXPANEL"
GIT_CONTENT_URL="raw.githubusercontent.com/josevijuzen-source/CODEXPANEL"
...
check_OS() {
	...
	if grep -q -E "CentOS Linux 7|CentOS Linux 8|CentOS Stream" /etc/os-release ; then
	  ...
	  echo -e "\ncodexpanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, CentOS 7, CentOS 8, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03...\n"
  	  exit
	fi
...
if [[ $TMP_YN == "1" ]] ; then
	if [[ -f /etc/codexpanel/watchdog.sh ]] ; then
		bash /etc/codexpanel/watchdog.sh kill
	fi
		rm -f /etc/codexpanel/watchdog.sh
		rm -f /usr/local/bin/watchdog
		wget -O /etc/codexpanel/watchdog.sh https://$GIT_CONTENT_URL/$BRANCH_NAME/CPScripts/watchdog.sh
...
self_check() {
echo -e "\nChecking CodexPanel Utility update..."
SUM=$(md5sum /usr/bin/codexpanel_utility)
SUM1=${SUM:0:32}
#get md5sum of local file

rm -f /usr/local/codexpanel/codexpanel_utility.sh
wget -q -O /usr/local/codexpanel/codexpanel_utility.sh https://codexpanel.sh/misc/codexpanel_utility.sh
chmod 600 /usr/local/codexpanel/codexpanel_utility.sh


SUM=$(md5sum /usr/local/codexpanel/codexpanel_utility.sh)
SUM2=${SUM:0:32}
#get md5sum of remote file.

if [[ $SUM1 == $SUM2 ]] ; then
	echo -e "\ncodexpanel Utility Script is up to date...\n"
else
	local_string=$(head -2 /usr/bin/codexpanel_utility)
	remote_string=$(head -2 /usr/local/codexpanel/codexpanel_utility.sh)
	#check file content before replacing itself in case failed to download the file.
	if [[ $local_string == $remote_string ]] ; then
	echo -e "\nUpdating CodexPanel Utility Script..."
	rm -f /usr/bin/codexpanel_utility
	mv /usr/local/codexpanel/codexpanel_utility.sh /usr/bin/codexpanel_utility
	chmod 700 /usr/bin/codexpanel_utility
	echo -e "\ncodexpanel Utility update compelted..."
...
if [[ $SERVER_COUNTRY == "CN" ]] ; then
	GIT_URL="gitee.com/qtwrk/codexpanel"
	GIT_CONTENT_URL="gitee.com/qtwrk/codexpanel/raw"
fi

#echo -e "codexpanel Upgrade will start in 10 seconds"
#echo -e "If you want to cancel, please press CTRL + C to cancel it"
#sleep 10
echo -e "codexpanel upgrading..."
rm -f /usr/local/codexpanel_upgrade.sh
wget -O /usr/local/codexpanel_upgrade.sh -q https://$GIT_CONTENT_URL/${BRANCH_NAME}/codexpanel_upgrade.sh
...
curl --silent https://codexpanel.sh/misc/faq.sh | sudo -u nobody bash | less -r
exit
}
	...
	if [[ $TMP_YN == "1" ]] ; then
		...
		if [[ -f /usr/local/lsmcd/bin/lsmcd ]] ; then
			...
			if [[ $SERVER_OS == "CentOS" ]] || [[ $SERVER_OS == "openEuler" ]] ; then
				...
				wget https://cdn.codexpanel.sh/litespeed/lsmcd.tar.gz
...
main_page() {
echo -e "		codexpanel Utility Tools \e[31m(beta)\e[39m

  1. Upgrade codexpanel.
...
panel_check(){
if [[ ! -f /etc/codexpanel/machineIP ]] ; then
	echo -e "\nCan not detect codexpanel..."
	echo -e "\nExit..."
	exit
fi
}
...
sudo_check() {
	...
	if [[ $(id -u) != 0 ]]  > /dev/null; then
		echo -e "\nYou must use root user to use CodexPanel Utility..."
		exit
	else
