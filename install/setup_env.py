#!/usr/bin/env python3
"""
Environment setup script for codexpanel
...
def setup_env_file(codexpanel_root="/usr/local/CodexCP"):
    """Set up the .env file with auto-detected IP addresses"""
    env_template_path = os.path.join(codexpanel_root, ".env.template")
    env_path = os.path.join(codexpanel_root, ".env")
...
if __name__ == "__main__":
    ...
    codexpanel_root = sys.argv[1] if len(sys.argv) > 1 else "/usr/local/CodexCP"

    success = setup_env_file(codexpanel_root)
