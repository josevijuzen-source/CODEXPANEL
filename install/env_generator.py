#!/usr/bin/env python3
"""
codexpanel Environment Configuration Generator
        ...
        try:
            with urllib.request.urlopen(url, timeout=10) as response:
                ...
                if re.match(r'^(\d{1,3}\.){3}\d{1,3}$', ip):
                    print(f"âœ“ Detected public IP: {ip}")
                    return ip
        except Exception as e:
            print(f"Failed to get IP from {url}: {e}")
            continue

    print("âš ï¸  Could not detect public IP address")
    return None
...
def create_env_file(codexpanel_path, mysql_root_password=None, codexpanel_db_password=None):
    """
    Create .env file with generated secure credentials
    
    Args:
        codexpanel_path: Path to CodexPanel installation directory
        mysql_root_password: Optional MySQL root password (will generate if None)
        codexpanel_db_password: Optional CodexPanel DB password (will generate if None)
    """
    ...
    if not codexpanel_db_password:
        codexpanel_db_password = generate_secure_password(24)
    
    secret_key = generate_secret_key(64)
    
    # Auto-detect IP addresses for ALLOWED_HOSTS
    print("ðŸ” Auto-detecting server IP addresses...")
    ...
    # This ensures CodexPanel works regardless of how the server is accessed
    allowed_hosts.append('*')

    allowed_hosts_str = ','.join(allowed_hosts)
    print(f"âœ“ ALLOWED_HOSTS configured: {allowed_hosts_str}")

    # Create .env content
    env_content = f"""# CodexPanel Environment Configuration
...
# Database Configuration - CodexPanel Database
DB_NAME=codexpanel
DB_USER=codexpanel
DB_PASSWORD={codexpanel_db_password}
DB_HOST=localhost
DB_PORT=3306
...
"""
    ...
    # Write .env file
    env_file_path = os.path.join(codexpanel_path, '.env')
    ...
    print(f"âœ“ Generated secure .env file at: {env_file_path}")
    print(f"âœ“ MySQL Root Password: {mysql_root_password}")
    print(f"âœ“ CodexPanel DB Password: {codexpanel_db_password}")
    print(f"âœ“ Django Secret Key: {secret_key[:20]}...")
    
    return {
        'mysql_root_password': mysql_root_password,
        'codexpanel_db_password': codexpanel_db_password,
        'secret_key': secret_key
    }

def create_env_backup(codexpanel_path, credentials):
    """
    ...
    """
    backup_content = f"""# CodexPanel Credentials Backup
...
codexpanel Database Password: {credentials['codexpanel_db_password']}
Django Secret Key: {credentials['secret_key']}

# To restore these credentials, copy them to your .env file
"""
    
    backup_file_path = os.path.join(codexpanel_path, '.env.backup')
    ...
    print(f"âœ“ Created credentials backup at: {backup_file_path}")
    print("âš ï¸  IMPORTANT: Record these credentials and delete the backup file for security")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python env_generator.py <codexpanel_path> [mysql_root_password] [codexpanel_db_password]")
        sys.exit(1)
    
    codexpanel_path = sys.argv[1]
    mysql_root_password = sys.argv[2] if len(sys.argv) > 2 else None
    codexpanel_db_password = sys.argv[3] if len(sys.argv) > 3 else None
    
    if not os.path.exists(codexpanel_path):
        print(f"Error: CodexPanel path does not exist: {codexpanel_path}")
        sys.exit(1)
    
    try:
        credentials = create_env_file(codexpanel_path, mysql_root_password, codexpanel_db_password)
        create_env_backup(codexpanel_path, credentials)
        print("\nâœ“ Environment configuration generated successfully!")
        print("âœ“ Remember to delete .env.backup after recording credentials")
    except Exception as e:
