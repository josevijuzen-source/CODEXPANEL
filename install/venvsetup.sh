...
#CodexPanel installer script for Ubuntu 18.04 and CentOS 7.X
DEV="OFF"
BRANCH="stable"
POSTFIX_VARIABLE="ON"
...
SERIAL_NO=""
DIR=$(pwd)
TEMP=$(curl --silent https://codexpanel.net/version.txt)
CP_VER1=${TEMP:12:3}
CP_VER2=${TEMP:25:1}
SERVER_OS="CentOS"
VERSION="OLS"
...
cd /root/codexpanel-tmp/lsws-$LSWS_STABLE_VER/bin
...
special_change(){
sed -i 's|codexpanel.sh|'$DOWNLOAD_SERVER'|g' install.py
sed -i 's|mirror.codexpanel.net|'$DOWNLOAD_SERVER'|g' install.py
...
#LATEST_URL="https://codexpanel.sh/latest.php"
...
#line1="$(grep -n "github.com/usmannasir/CodexPanel" install.py | head -n 1 | cut -d: -f1)"
#line2=$((line1 - 1))
#sed -i "${line2}i\ \ \ \ \ \ \ \ subprocess.call(command, shell=True)" install.py
#sed -i "${line2}i\ \ \ \ \ \ \ \ command = 'tar xzvf codexpanel-git.tar.gz'" install.py
#sed -i "${line2}i\ \ \ \ \ \ \ \ subprocess.call(command, shell=True)" install.py
...
sed -i 's|yum -y install https://codexpanel.sh/gf-release-latest.gf.el7.noarch.rpm|wget -O /etc/yum.repos.d/gf.repo https://'$DOWNLOAD_SERVER'/gf-plus/gf.repo|g' install.py
...
if  echo $OUTPUT | grep -q "CentOS Linux 7" ; then
	...
	echo -e "\ncodexpanel is supported on Ubuntu 18.04, CentOS 7.x and CloudLinux 7.x...\n"
	exit 1
fi
}
...
echo -e "\nUsage: wget https://codexpanel.sh/codexpanel.sh"
echo -e "\nchmod +x codexpanel.sh"
echo -e "\n./codexpanel.sh -v ols/SERIAL_NUMBER -c 1 -a 1"
echo -e "\n -v or --version: choose to install CodexPanel OpenLiteSpeed or CodexPanel Enterprise, available options are \e[31mols\e[39m and \e[31mSERIAL_NUMBER\e[39m, default ols"
...
echo -e "\n Example:"
echo -e "\n ./codexpanel.sh -v ols -p r or ./codexpanel.sh --version ols --password random"
echo -e "\n This will install CodexPanel OpenLiteSpeed and randomly generate the password."
echo -e "\n ./codexpanel.sh default"
...
interactive_mode() {
echo -e "		codexpanel Installer v$CP_VER1$CP_VER2
...
interactive_others() {
if [ ! -e "/etc/codexpanel/machineIP" ]; then
echo -e "\nYou don't have CodexPanel installed...\n"
exit
fi

echo -e "		codexpanel Addons v$CP_VER1$CP_VER2
...
<<COMMENT
echo -e "\nInstall minimal service for codexpanel? This will skip PowerDNS, Postfix and Pure-FTPd."
...
debug="1"
if [[ $debug == "0" ]] ; then
	echo "/usr/local/codexpanel/bin/python2 install.py $SERVER_IP $SERIAL_NO $LICENSE_KEY"
	exit
fi

if [[ $debug == "1" ]] ; then
	if [[ $DEV == "ON" ]] ; then
	/usr/local/codexpanel/bin/python install.py $SERVER_IP $SERIAL_NO $LICENSE_KEY
	else
	/usr/local/codexpanel/bin/python2 install.py $SERVER_IP $SERIAL_NO $LICENSE_KEY
	fi
...
echo "Creating CodexPanel virtual environment..."
if python3 -m venv --system-site-packages /usr/local/codexpanel 2>&1 | grep -q "unrecognized option"; then
    # Fallback to virtualenv if python3 -m venv doesn't support --system-site-packages
    virtualenv --system-site-packages /usr/local/codexpanel
elif python3 -m venv --system-site-packages /usr/local/codexpanel 2>/dev/null; then
...
wget -O requirements.txt https://raw.githubusercontent.com/usmannasir/CodexPanel/1.8.0/requirments.txt
...
if [[ $DEV == "ON" ]] ; then
	#install dev branch 
	#wget https://raw.githubusercontent.com/usmannasir/CodexPanel/$BRANCH_NAME/requirments.txt
	cd /usr/local/
	python3.6 -m venv codexpanel
	source /usr/local/codexpanel/bin/activate
	wget -O requirements.txt https://raw.githubusercontent.com/usmannasir/CodexPanel/$BRANCH_NAME/requirments.txt
	pip3.6 install --ignore-installed -r requirements.txt
	# Install python-dotenv for loading .env file (critical for AlmaLinux 8)
	pip3.6 install python-dotenv
fi

if [ -f requirements.txt ] && [ -d CodexPanel ] ; then
	rm -rf codexpanel
	rm -f requirements.txt
fi

if [[ $SERVER_COUNTRY == "CN" ]] ; then
	wget https://codexpanel.sh/codexpanel-git.tar.gz
	tar xzvf codexpanel-git.tar.gz > /dev/null
	cp -r CodexPanel /usr/local/codexpanel
	cd codexpanel/install
else
	if [[ $DEV == "ON" ]] ; then
	git clone https://github.com/usmannasir/CodexPanel
	cd codexpanel
...
curl https://codexpanel.sh/?version
}
...
if grep "\[ERROR\] We are not able to run ./install.sh return code: 1.  Fatal error, see /var/log/installLogs.txt for full details" /var/log/installLogs.txt > /dev/null; then 
	cd ${DIR}/codexpanel/install/lsws-*
	./install.sh
...
if [[ $DEV == "ON" ]] ; then
python3.6 -m venv /usr/local/CodexCP
...
for version in $(ls /usr/local/lsws | grep lsphp); 
	do
		...
		if [[ $SERVER_OS == "Ubuntu" ]] ; then
			if [[ ! -d /usr/local/lsws/codexpanel-tmp ]] ; then
				...
				cd /usr/local/lsws/codexpanel-tmp
...
chmod +x /etc/profile.d/codexpanel.sh
RAM2=$(free -m | awk 'NR==2{printf "%s/%sMB (%.2f%%)\n", $3,$2,$3*100/$2 }')
DISK2=$(df -h | awk '$NF=="/"{printf "%d/%dGB (%s)\n", $3,$2,$5}')
ELAPSED="$(($SECONDS / 3600)) hrs $((($SECONDS / 60) % 60)) min $(($SECONDS % 60)) sec"
MYSQLPASSWD=$(cat /etc/codexpanel/mysqlPassword)
echo "$ADMIN_PASS" > /etc/codexpanel/adminPass
/usr/local/codexpanel/bin/python2 /usr/local/CodexCP/plogical/adminPass.py --password $ADMIN_PASS
systemctl restart lscpd
systemctl restart lsws
echo "/usr/local/codexpanel/bin/python2 /usr/local/CodexCP/plogical/adminPass.py --password \"\$@\"" > /usr/bin/adminPass
...
echo -e "         \e[31m/etc/codexpanel/mysqlPassword\e[39m with new password as well   "
echo "                                                                   "
...
sed -i 's|lsws-5.3.8|lsws-'$LSWS_STABLE_VER'|g' /usr/local/CodexCP/serverStatus/serverStatusUtil.py
sed -i 's|lsws-5.4.2|lsws-'$LSWS_STABLE_VER'|g' /usr/local/CodexCP/serverStatus/serverStatusUtil.py
sed -i 's|lsws-5.3.5|lsws-'$LSWS_STABLE_VER'|g' /usr/local/CodexCP/serverStatus/serverStatusUtil.py
...
SERVER_IP=$(curl --silent --max-time 10 -4 https://codexpanel.sh/?ip)
...
SERVER_COUNTRY=$(curl --silent --max-time 5 https://codexpanel.sh/?country)
...
#test string
if [[ $SERVER_COUNTRY == "CN" ]] ; then
DOWNLOAD_SERVER="codexpanel.sh"
else
DOWNLOAD_SERVER="cdn.codexpanel.sh"
fi
