...
class InstallCodexPanel:
    ...
    def install_package(self, package_name, options=""):
        ...
        # InstallCodexPanel always uses verbose mode (no silent option)
    ...
    def OSFlags():
        if os.path.exists("/etc/redhat-release"):
            ...
            if data.find('CloudLinux 8') > -1 or data.find('cloudlinux 8') > -1:
                InstallCodexPanel.CloudLinux8 = 1
    ...
    def getLatestLSWSVersion():
        ...
        try:
            ...
            if versions:
                ...
                InstallCodexPanel.stdOut(f"Found latest LSWS Enterprise version: {latest_version}", 1)
                return latest_version
            else:
                InstallCodexPanel.stdOut("Could not find version pattern in HTML, using fallback", 1)

        except Exception as e:
            InstallCodexPanel.stdOut(f"Failed to fetch latest LSWS version: {str(e)}, using fallback", 1)
    ...
    def detectPlatform(self):
        ...
        try:
                ...
                with open('/etc/lsb-release', 'r') as f:
                    ...
                    if 'Ubuntu' in content or 'ubuntu' in content:
                        ...
                        if 'DISTRIB_RELEASE=20.04' in content:
                            InstallCodexPanel.stdOut("Ubuntu 20.04 detected: custom OLS binary requires GLIBC_2.34 (22.04+); keeping stock OLS", 1)
                            return 'skip'
                        return 'ubuntu'
            ...
            InstallCodexPanel.stdOut("WARNING: Could not detect platform, defaulting to rhel9", 1)
            return 'rhel9'

        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [detectPlatform]")
            InstallCodexPanel.stdOut(f"ERROR detecting platform: {msg}, defaulting to rhel9", 1)
            return 'rhel9'

    def downloadCustomBinary(self, url, destination):
        """Download custom binary file"""
        try:
            InstallCodexPanel.stdOut(f"Downloading {os.path.basename(destination)}...", 1)
            ...
            if os.path.exists(destination):
                ...
                if file_size > 10240:  # 10KB
                    if file_size > 1048576:  # 1MB
                        InstallCodexPanel.stdOut(f"Downloaded successfully ({file_size / (1024*1024):.2f} MB)", 1)
                    else:
                        InstallCodexPanel.stdOut(f"Downloaded successfully ({file_size / 1024:.2f} KB)", 1)

                    return True
                else:
                    InstallCodexPanel.stdOut(f"ERROR: Downloaded file too small ({file_size} bytes)", 1)
                    return False
            else:
                InstallCodexPanel.stdOut("ERROR: Download failed - file not found", 1)
                return False

        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [downloadCustomBinary]")
            InstallCodexPanel.stdOut(f"ERROR: {msg}", 1)
            return False
    ...
    def verifyChecksum(self, file_path, expected_sha256):
        ...
        try:
            ...
            if actual.lower() == expected_sha256.lower():
                InstallCodexPanel.stdOut(f"SHA256 verified: {os.path.basename(file_path)}", 1)
                return True
            InstallCodexPanel.stdOut(f"ERROR: SHA256 mismatch for {os.path.basename(file_path)}", 1)
            InstallCodexPanel.stdOut(f"  expected: {expected_sha256}", 1)
            InstallCodexPanel.stdOut(f"  actual:   {actual}", 1)
            return False
        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [verifyChecksum]")
            InstallCodexPanel.stdOut(f"ERROR: {msg} [verifyChecksum]", 1)
            return False
    ...
    def checkGlibcCompat(self, binary_path):
        ...
        try:
            ...
            if 'not found' in output:
                InstallCodexPanel.stdOut("ERROR: Downloaded binary has unresolved libraries (incompatible with this OS):", 1)
                for line in output.splitlines():
                    if 'not found' in line:
                        InstallCodexPanel.stdOut(f"  {line.strip()}", 1)
                return False
            return True
        except Exception as msg:
            InstallCodexPanel.stdOut(f"WARNING: Could not run ldd pre-check ({msg}); continuing", 1)
            return True

    def installCustomOLSBinaries(self):
        """Install custom OpenLiteSpeed binaries with PHP config support"""
        try:
            InstallCodexPanel.stdOut("Installing Custom OpenLiteSpeed Binaries", 1)
            InstallCodexPanel.stdOut("=" * 50, 1)

            # Check architecture
            if not self.detectArchitecture():
                InstallCodexPanel.stdOut("WARNING: Custom binaries only available for x86_64", 1)
                InstallCodexPanel.stdOut("Skipping custom binary installation", 1)
                InstallCodexPanel.stdOut("Standard OLS will be used", 1)
                return True  # Not a failure, just skip

            # Detect platform
            platform = self.detectPlatform()
            InstallCodexPanel.stdOut(f"Detected platform: {platform}", 1)
            ...
            if platform == 'skip':
                InstallCodexPanel.stdOut("Custom binary installation skipped for this platform; using standard OLS", 1)
                return True  # Not a failure, just skip

            # Platform-specific URLs and checksums (OpenLiteSpeed v2.5.0 â€” all features config-driven, static linking)
            ...
            # rhel9 artifact covers EL9 + EL10 (AlmaLinux 10); ubuntu artifact covers 22.04/24.04 (not 20.04 â€” see detectPlatform)
            BINARY_CONFIGS = {
                'rhel8': {
                    'url': 'https://codexpanel.net/openlitespeed-2.5.0-x86_64-rhel8',
                    'module_url': 'https://codexpanel.net/codexpanel_ols-2.7.3-x86_64-rhel8.so',
                    'modsec_url': 'https://codexpanel.net/mod_security-2.5.0-x86_64-rhel8.so',
                    'sha256': {
                ...
                'rhel9': {
                    'url': 'https://codexpanel.net/openlitespeed-2.5.0-x86_64-rhel9',
                    'module_url': 'https://codexpanel.net/codexpanel_ols-2.7.3-x86_64-rhel9.so',
                    'modsec_url': 'https://codexpanel.net/mod_security-2.5.0-x86_64-rhel9.so',
                    'sha256': {
                ...
                'ubuntu': {
                    'url': 'https://codexpanel.net/openlitespeed-2.5.0-x86_64-ubuntu',
                    'module_url': 'https://codexpanel.net/codexpanel_ols-2.7.3-x86_64-ubuntu.so',
                    'modsec_url': 'https://codexpanel.net/mod_security-2.5.0-x86_64-ubuntu.so',
                    'sha256': {
            ...
            if not config:
                InstallCodexPanel.stdOut(f"ERROR: No binaries available for platform {platform}", 1)
                InstallCodexPanel.stdOut("Skipping custom binary installation", 1)
            ...
            try:
                ...
                if os.path.exists(OLS_BINARY_PATH):
                    ...
                    InstallCodexPanel.stdOut(f"Backup created at: {backup_dir}", 1)
                ...
                InstallCodexPanel.stdOut(f"WARNING: Could not create backup: {e}", 1)
            ...
            InstallCodexPanel.stdOut("Downloading custom binaries...", 1)

            # Download OpenLiteSpeed binary
            if not self.downloadCustomBinary(OLS_BINARY_URL, tmp_binary):
                InstallCodexPanel.stdOut("ERROR: Failed to download or verify OLS binary", 1)
                InstallCodexPanel.stdOut("Continuing with standard OLS", 1)
            ...
            if not self.verifyChecksum(tmp_binary, SHA256.get('binary')):
                InstallCodexPanel.stdOut("ERROR: OLS binary failed checksum verification; keeping stock OLS", 1)
                return True  # Not fatal, continue with standard OLS
            if not self.checkGlibcCompat(tmp_binary):
                InstallCodexPanel.stdOut("ERROR: OLS binary is not ABI-compatible with this OS; keeping stock OLS", 1)
                return True  # Not fatal, continue with standard OLS
            ...
            if MODULE_URL:
                if not self.downloadCustomBinary(MODULE_URL, tmp_module):
                    InstallCodexPanel.stdOut("ERROR: Failed to download or verify module", 1)
                    InstallCodexPanel.stdOut("Continuing with standard OLS", 1)
                    return True  # Not fatal, continue with standard OLS
                if not self.verifyChecksum(tmp_module, SHA256.get('module')):
                    InstallCodexPanel.stdOut("ERROR: Module failed checksum verification; keeping stock OLS", 1)
                    return True  # Not fatal, continue with standard OLS
                module_downloaded = True
            else:
                InstallCodexPanel.stdOut("Note: No CodexPanel module for this platform", 1)
            ...
            if MODSEC_URL:
                InstallCodexPanel.stdOut("Downloading ModSecurity WAF module...", 1)
                if self.downloadCustomBinary(MODSEC_URL, tmp_modsec):
                    if self.verifyChecksum(tmp_modsec, SHA256.get('modsec')):
                        ...
                        InstallCodexPanel.stdOut("WARNING: ModSecurity failed checksum verification; continuing without it", 1)
                else:
                    InstallCodexPanel.stdOut("WARNING: Failed to download ModSecurity module; continuing without it", 1)

            # Install OpenLiteSpeed binary
            InstallCodexPanel.stdOut("Installing custom binaries...", 1)

            try:
                shutil.move(tmp_binary, OLS_BINARY_PATH)
                os.chmod(OLS_BINARY_PATH, 0o755)
                InstallCodexPanel.stdOut("Installed OpenLiteSpeed binary", 1)
            except Exception as e:
                InstallCodexPanel.stdOut(f"ERROR: Failed to install binary: {e}", 1)
            ...
            if module_downloaded:
                try:
                    ...
                    InstallCodexPanel.stdOut("Installed CodexPanel module", 1)
                except Exception as e:
                    InstallCodexPanel.stdOut(f"ERROR: Failed to install module: {e}", 1)
            ...
            if modsec_downloaded:
                try:
                    ...
                    InstallCodexPanel.stdOut("Installed ModSecurity WAF module", 1)
                except Exception as e:
                    InstallCodexPanel.stdOut(f"WARNING: Failed to install ModSecurity: {e}", 1)
            ...
            if os.path.exists(OLS_BINARY_PATH):
                if not module_downloaded or os.path.exists(MODULE_PATH):
                    InstallCodexPanel.stdOut("Verifying new binary...", 1)
                    try:
                        ...
                        if 'LiteSpeed' in version_output or 'OpenLiteSpeed' in version_output:
                            InstallCodexPanel.stdOut("Binary version check passed", 1)
                        else:
                            InstallCodexPanel.stdOut("WARNING: Could not verify binary version", 1)
                    except Exception as e:
                        ...
                        InstallCodexPanel.stdOut(f"ERROR: Binary verification failed: {e}", 1)
                        ...
                        if os.path.exists(backup_binary):
                            InstallCodexPanel.stdOut("Rolling back to stock OpenLiteSpeed binary...", 1)
                            try:
                                ...
                                InstallCodexPanel.stdOut("Rollback completed; using stock OLS", 1)
                            except Exception as rollback_err:
                                InstallCodexPanel.stdOut(f"WARNING: Rollback may have failed: {rollback_err}", 1)
                    ...
                    InstallCodexPanel.stdOut("=" * 50, 1)
                    InstallCodexPanel.stdOut("Custom Binaries Installed Successfully", 1)
                    InstallCodexPanel.stdOut("Features enabled:", 1)
                    InstallCodexPanel.stdOut("  - Static-linked cross-platform binary", 1)
                    if module_downloaded:
                        InstallCodexPanel.stdOut("  - Apache-style .htaccess support", 1)
                        InstallCodexPanel.stdOut("  - php_value/php_flag directives", 1)
                        InstallCodexPanel.stdOut("  - Enhanced header control", 1)
                    if modsec_downloaded:
                        InstallCodexPanel.stdOut("  - ModSecurity WAF module", 1)
                    InstallCodexPanel.stdOut(f"Backup: {backup_dir}", 1)
                    InstallCodexPanel.stdOut("=" * 50, 1)
                    return True

            InstallCodexPanel.stdOut("ERROR: Installation verification failed", 1)
            return False
    ...
    def configureCustomModule(self):
        ...
        try:
            InstallCodexPanel.stdOut("Configuring CodexPanel module...", 1)

            CONFIG_FILE = "/usr/local/lsws/conf/httpd_config.conf"

            if not os.path.exists(CONFIG_FILE):
                InstallCodexPanel.stdOut("WARNING: Config file not found", 1)
                InstallCodexPanel.stdOut("Module will be auto-loaded", 1)
                return True
            ...
            if 'codexpanel_ols' in content:
                ...
                if new_content != content:
                    ...
                    with open(CONFIG_FILE, 'w') as f:
                        f.write(new_content)
                    InstallCodexPanel.stdOut("Module was disabled (ls_enabled 0); re-enabled LSCache module", 1)
                else:
                    InstallCodexPanel.stdOut("Module already configured", 1)
                return True
...
"""
            ...
            InstallCodexPanel.stdOut("Module configured successfully", 1)
            return True

        except Exception as msg:
            logging.InstallLog.writeToFile(str(msg) + " [configureCustomModule]")
            InstallCodexPanel.stdOut(f"WARNING: Module configuration failed: {msg}", 1)
            InstallCodexPanel.stdOut("Module may still work via auto-load", 1)
    ...
    def installLiteSpeed(self):
        if self.ent == 0:
                ...
                if os.path.exists(conf_path):
                    ...
                    if 'autoSSL' not in content:
                        ...
                        with open(conf_path, 'w') as f:
                            f.write(content)
                        InstallCodexPanel.stdOut("Auto-SSL enabled in httpd_config.conf", 1)
            except Exception as e:
                InstallCodexPanel.stdOut(f"WARNING: Could not enable Auto-SSL: {e}", 1)
            ...
            try:
                ...
                lsws_version = InstallCodexPanel.getLatestLSWSVersion()

                if InstallCodexPanel.ISARM():
                ...
                if self.serial == '1111-2222-3333-4444':
                    command = f'wget -q --output-document=/root/codexpanel/install/lsws-{lsws_version}/trial.key http://license.litespeedtech.com/reseller/trial.key'
                    install_utils.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
                else:
    ...
    def fix_ols_configs(self):
        try:
            ...
            InstallCodexPanel.stdOut("Fixing OpenLiteSpeed configurations!", 1)
            ...
            InstallCodexPanel.stdOut("OpenLiteSpeed Configurations fixed!", 1)
    ...
    def changePortTo80(self):
        try:
            InstallCodexPanel.stdOut("Changing default port to 80..", 1)

            file_path = self.server_root_path + "conf/httpd_config.conf"
            if self.modify_file_content(file_path, {"*:8088": "*:80"}):
                InstallCodexPanel.stdOut("Default port is now 80 for OpenLiteSpeed!", 1)
            else:
                return 0
    ...
    def installAllPHPVersions(self):
        ...
        if self.distro == ubuntu:
            ...
            InstallCodexPanel.stdOut("LiteSpeed PHPs successfully installed!", 1)
    ...
    def installSieve(self):
        ...
        try:
            InstallCodexPanel.stdOut("Installing Sieve (Dovecot Sieve) for email filtering...", 1)
...
""")
            ...
            InstallCodexPanel.stdOut("Sieve successfully installed and configured!", 1)
            return 1
    ...
    def setupWebmail():
        ...
        try:
            ...
            if not os.path.exists('/etc/dovecot/dovecot.conf'):
                InstallCodexPanel.stdOut("Dovecot not installed, skipping webmail setup.", 1)
                return 1

            # Skip if already configured
            if os.path.exists('/etc/codexpanel/webmail.conf') and os.path.exists('/etc/dovecot/master-users'):
                InstallCodexPanel.stdOut("Webmail master user already configured.", 1)
                return 1

            InstallCodexPanel.stdOut("Setting up webmail master user for SSO...", 1)
            ...
            # Ensure /etc/codexpanel/ exists
            os.makedirs('/etc/codexpanel', exist_ok=True)
...
"""
            ...
            InstallCodexPanel.stdOut("Webmail master user setup complete!", 1)
            return 1
...
"""
    ...
    def fixMariaDB(self):
        self.stdOut("Setup MariaDB so it can support codexpanel's needs")
        ...
        self.stdOut("MariaDB is now setup so it can support codexpanel's needs")
    ...
    def installPureFTPD(self):
        if self.distro == ubuntu:
            ...
            if get_Ubuntu_release() == 18.10:
                ...
                packages = [
                    ('pure-ftpd-common_1.0.47-3_all.deb', 'wget https://rep.codexpanel.net/pure-ftpd-common_1.0.47-3_all.deb'),
                    ('pure-ftpd-mysql_1.0.47-3_amd64.deb', 'wget https://rep.codexpanel.net/pure-ftpd-mysql_1.0.47-3_amd64.deb')
                ]
    ...
    def startPureFTPD(self):
        ...
        InstallCodexPanel.stdOut("Pure-FTPd enabled for startup.", 1)
        InstallCodexPanel.stdOut("Note: Pure-FTPd will start after database setup is complete.", 1)
    ...
    def installPureFTPDConfigurations(self, mysql):
        try:
            ...
            InstallCodexPanel.stdOut("Configuring PureFTPD..", 1)
            ...
            dataWritten = "MYSQLPassword " + InstallCodexPanel.mysqlPassword + '\n'
            for items in data:
            ...
            InstallCodexPanel.stdOut("PureFTPD configured!", 1)
            ...
            if self.distro == ubuntu or self.distro == cent8 or self.distro == openeuler:
                ...
                try:
                    ...
                    if e.errno != errno.EEXIST and e.errno != errno.ENOENT:
                        InstallCodexPanel.stdOut("[ERROR] Unable to rename /etc/resolv.conf to install PowerDNS: " +
                ...
                try:
                    ...
                    InstallCodexPanel.stdOut("Created temporary /etc/resolv.conf with Google DNS", 1)
                except IOError as e:
                    InstallCodexPanel.stdOut("[ERROR] Unable to create /etc/resolv.conf: " + str(e), 1, 1, os.EX_OSERR)
    ...
    def installPowerDNSConfigurations(self, mysqlPassword, mysql):
        try:
            ...
            InstallCodexPanel.stdOut("Configuring PowerDNS..", 1)
            ...
            try:
                ...
                InstallCodexPanel.stdOut("[ERROR] Failed to copy PowerDNS config: " + str(e), 1)
            ...
            with open(dnsPath, "r") as f:
                ...
                if not content or "launch=gmysql" not in content:
                    InstallCodexPanel.stdOut("[WARNING] PowerDNS config appears empty or incomplete, attempting to fix...", 1)
                    ...
                    try:
                        ...
                        InstallCodexPanel.stdOut("[WARNING] Failed to re-copy config: " + str(copy_error), 1)
                        
                        # Fallback: directly write the essential MySQL configuration
                        InstallCodexPanel.stdOut("[INFO] Directly writing MySQL backend configuration...", 1)
                        try:
                            mysql_config = f"""# PowerDNS MySQL Backend Configuration
launch=gmysql
gmysql-host=localhost
gmysql-port=3306
gmysql-user=codexpanel
gmysql-password={mysqlPassword}
gmysql-dbname=codexpanel
...
"""
                            ...
                            InstallCodexPanel.stdOut("[SUCCESS] MySQL backend configuration written directly", 1)
                        except Exception as write_error:
                            InstallCodexPanel.stdOut("[ERROR] Failed to write MySQL config: " + str(write_error), 1)
                            raise
            
            InstallCodexPanel.stdOut("PowerDNS config file prepared at: " + dnsPath, 1)
            ...
            InstallCodexPanel.stdOut("PowerDNS configured!", 1)
    ...
    def startPowerDNS(self):
        ...
        # It will be started after Django migrations create the required tables
        InstallCodexPanel.stdOut("PowerDNS enabled for startup.", 1)
        InstallCodexPanel.stdOut("Note: PowerDNS will start after database setup is complete.", 1)
...
def Main(cwd, mysql, distro, ent, serial=None, port="8090", ftp=None, dns=None, publicip=None, remotemysql=None,
    ...
    InstallCodexPanel.mysqlPassword = install_utils.generate_pass()
    InstallCodexPanel.mysql_Root_password = install_utils.generate_pass()

    file_name = '/etc/codexpanel/mysqlPassword'

    if remotemysql == 'OFF':
        if os.access(file_name, os.F_OK):
            password = open(file_name, 'r')
            InstallCodexPanel.mysql_Root_password = password.readline()
            password.close()
        else:
            password = open(file_name, "w")
            password.writelines(InstallCodexPanel.mysql_Root_password)
            password.close()
    else:
    ...
    try:
        ...
        install_utils.call(command, distro, '[chmod]',
                                      '',
                                      1, 0, os.EX_OSERR)
        command = 'chown root:codexpanel %s' % (file_name)
        install_utils.call(command, distro, '[chmod]',
                                      '',
                                      1, 0, os.EX_OSERR)
    except:
    ...
    if distro in [centos, cent8, openeuler]:
        ...
        InstallCodexPanel.mysqlPassword = InstallCodexPanel.mysql_Root_password

    installer = InstallCodexPanel("/usr/local/lsws/", cwd, distro, ent, serial, port, ftp, dns, publicip, remotemysql,
    ...
    mysqlUtilities.createDatabase("CodexPanel", "CodexPanel", InstallCodexPanel.mysqlPassword, publicip)
    ...
    if dns is None:
        ...
        installer.installPowerDNSConfigurations(InstallCodexPanel.mysqlPassword, mysql)
