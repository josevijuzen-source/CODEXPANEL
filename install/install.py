...
class preFlightsChecks:
    debug = 1
    CodexPanelMirror = "mirror.codexpanel.net/pip"
    cdn = 'codexpanel.sh'
    ...
    def __init__(self, rootPath, ip, path, cwd, codexpanelPath, distro, remotemysql=None, mysqlhost=None, mysqldb=None,
                 mysqluser=None, mysqlpassword=None, mysqlport=None):
        self.ipAddr = ip
        self.path = path
        self.cwd = cwd
        self.server_root_path = rootPath
        self.codexpanelPath = codexpanelPath
        self.distro = distro
    ...
    def installQuota(self,):
        try:
            ...
            if self.distro == ubuntu:
                ...
                # Skip apt update as it was already done in codexpanel.sh
                self.install_package("quota", silent=True)
    ...
    def setup_account_codexpanel(self):
        try:
            ...
            logging.InstallLog.writeToFile("[ERROR] setup_account_codexpanel. " + str(msg))

    def installcodexpanelRepo(self):
        self.stdOut("Install CodexPanel repo")
        ...
        if self.distro == ubuntu:
            try:
                ...
                logging.InstallLog.writeToFile("[ERROR] Exception during CodexPanel install")
                preFlightsChecks.stdOut("[ERROR] Exception during CodexPanel install")
    ...
    def generate_secure_env_file(self, mysql_root_password, codexpanel_db_password):
        """
...
""" % password
        ...
        # Note: password = MySQL root password, mysqlPassword = CodexPanel DB password
        ...
        command = "/usr/local/codexpanel/bin/python manage.py migrate"
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
        ...
        command = 'mv static /usr/local/CodexCP/public/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
    ...
    def fixcodexpanelPermissions(self):
        ...
        fileM = ['/usr/local/lsws/FileManager/', '/usr/local/CodexCP/install/FileManager',
                 '/usr/local/CodexCP/serverStatus/litespeed/FileManager', '/usr/local/lsws/Example/html/FileManager']
        ...
        clScripts = ['/usr/local/CodexCP/CLScript/panel_info.py', '/usr/local/CodexCP/CLScript/CloudLinuxPackages.py',
                     '/usr/local/CodexCP/CLScript/CloudLinuxUsers.py',
                     '/usr/local/CodexCP/CLScript/CloudLinuxDomains.py',
                     '/usr/local/CodexCP/CLScript/CloudLinuxResellers.py', '/usr/local/CodexCP/CLScript/CloudLinuxAdmins.py',
        ...
        command = 'mkdir -p /usr/local/lscp/codexpanel/snappymail/data/_data_/_default_/configs/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'mkdir -p /usr/local/lscp/codexpanel/snappymail/data/_data_/_default_/domains/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'mkdir -p /usr/local/lscp/codexpanel/snappymail/data/_data_/_default_/storage/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'mkdir -p /usr/local/lscp/codexpanel/snappymail/data/_data_/_default_/temp/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        command = 'mkdir -p /usr/local/lscp/codexpanel/snappymail/data/_data_/_default_/cache/'
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        # Set proper ownership early
        command = "chown -R lscpd:lscpd /usr/local/lscp/codexpanel/snappymail/"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        # Set proper permissions - make all data directories group writable
        command = "chmod -R 775 /usr/local/lscp/codexpanel/snappymail/data/"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        command = "chown -R lscpd:lscpd /usr/local/CodexCP/public/snappymail/data 2>/dev/null || true"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

        snappymailinipath = '/usr/local/lscp/codexpanel/snappymail/data/_data_/_default_/configs/application.ini'
    ...
    def download_install_phpmyadmin(self):
        try:
            ...
            command = 'unzip /usr/local/CodexCP/public/phpmyadmin.zip -d /usr/local/CodexCP/public'
            preFlightsChecks.call(command, self.distro, '[download_install_phpmyadmin]',
                                  command, 1, 0, os.EX_OSERR)
...
"""
            ...
            command = 'chown -R lscpd:lscpd /usr/local/CodexCP/public/phpmyadmin'
            preFlightsChecks.call(command, self.distro, '[chown -R lscpd:lscpd /usr/local/CodexCP/public/phpmyadmin]',
                                  'chown -R lscpd:lscpd /usr/local/CodexCP/public/phpmyadmin', 1, 0, os.EX_OSERR)

            if self.remotemysql == 'ON':
                command = "sed -i 's|'localhost'|'%s'|g' %s" % (
                    self.mysqlhost, '/usr/local/CodexCP/public/phpmyadmin/config.inc.php')
                preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setup_email_Passwords(self, mysqlPassword, mysql):
        try:
            ...
            if mysql == 'Two':
                dataWritten = "connect = host=127.0.0.1 dbname=codexpanel user=codexpanel password=" + mysqlPassword + " port=3307\n"
            else:
                dataWritten = "connect = host=localhost dbname=codexpanel user=codexpanel password=" + mysqlPassword + " port=3306\n"
    ...
    def downoad_and_install_raindloop(self):
        try:
            ...
            command = 'unzip snappymail-%s.zip -d /usr/local/CodexCP/public/snappymail' % (preFlightsChecks.SnappyVersion)
            preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
            ...
            command = "mkdir -p /usr/local/lscp/codexpanel/rainloop/data"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
...
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            ...
            command = f'wget -O /usr/local/CodexCP/snappymail_codexpanel.php  https://raw.githubusercontent.com/the-djmaze/snappymail/master/integrations/codexpanel/install.php'
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def installLSCPD(self):
        try:
            ...
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            try:
                os.mkdir('/usr/local/lscp/codexpanel')
            except:
                pass
            try:
                os.mkdir('/usr/local/lscp/codexpanel/logs')
            except:
                pass
    ...
    def setupComodoRules(self):
        try:
            ...
            command = "wget https://codexpanel.net/modsec/comodo.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setupPythonWSGI(self):
        try:
            ...
            command = "/usr/local/codexpanel/bin/python ./configure.py"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
            ...
            if not os.path.exists('/usr/local/CodexCP/bin/'):
                os.mkdir('/usr/local/CodexCP/bin/')

            command = "cp lswsgi /usr/local/CodexCP/bin/"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def setupLSCPDDaemon(self):
        try:
            ...
            path = "/usr/local/CodexCP/conf/"
            command = "mkdir -p " + path
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            path = "/usr/local/CodexCP/conf/token_env"
            writeToFile = open(path, "w")
...
"""
    ...
    def install_default_keys(self):
        try:
            ...
            command = "ssh-keygen -f /root/.ssh/codexpanel -t rsa -N ''"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def test_Requests(self):
        try:
            import requests
            getVersion = requests.get('https://codexpanel.net/version.txt')
            ...
            command = f"pip install{pip_flags} http://mirror.codexpanel.net/urllib3-1.22.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            command = f"pip install{pip_flags} http://mirror.codexpanel.net/requests-2.18.4.tar.gz"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

    def installation_successfull(self):
        print("###################################################################")
        print("                CodexPanel Successfully Installed                  ")
        print("                                                                   ")
...
"""
    ...
    def setUpFirstAccount():
        try:
            command = 'python /usr/local/CodexCP/plogical/adminPass.py --password 1234567'
            subprocess.call(shlex.split(command))
        except:
            pass
    ...
    def installRestic(self):
        try:
            ...
            if os.path.exists(CentOSPath) or os.path.exists(openEulerPath):
                ...
                # Skip apt-get update as it was already done in codexpanel.sh
...
"""
    ...
    def installDNS_codexpanelACMEFile(self):

        os.chdir(self.cwd)

        filePath = '/root/.acme.sh/dns_codexpanel.sh'
        shutil.copy('dns_codexpanel.sh', filePath)
    ...
    def startDeferredServices(self):
        ...
        if os.path.exists('/home/codexpanel/powerdns'):
        ...
        if os.path.exists('/home/codexpanel/pureftpd'):
...
def configure_jwt_secret():
    try:
        ...
        fastapi_file = '/usr/local/CodexCP/fastapi_ssh_server.py'
...
def main():
    ...
    logging.InstallLog.writeToFile("Starting CodexPanel installation..,10")
    preFlightsChecks.stdOut("Starting CodexPanel installation..")
    ...
    try:
        os.mkdir("/etc/codexpanel")
    except:
        pass

    machineIP = open("/etc/codexpanel/machineIP", "w")
    ...
    distro = get_distro()
    checks = preFlightsChecks("/usr/local/lsws/", args.publicip, "/usr/local", cwd, "/usr/local/CodexCP", distro,
    ...
    checks.installDNS_codexpanelACMEFile()
    ...
    try:
        # command = 'mkdir -p /usr/local/lscp/codexpanel/snappymail/data/data/default/configs/'
        # subprocess.call(shlex.split(command))
        ...
        # so the password recovery in codexpanel.sh continues to work while no
        # install ever ships with predictable webmail admin credentials.
        snappymailAdminPassword = generate_pass()

        writeToFile = open('/usr/local/lscp/codexpanel/snappymail/data/_data_/_default_/configs/application.ini', 'a')
...
?>""" % (snappymailAdminPassword)
        ...
        command = '/usr/local/lsws/lsphp83/bin/php /usr/local/CodexCP/public/snappymail.php'
        subprocess.call(shlex.split(command))
    ...
    checks.fixcodexpanelPermissions()
    ...
    logging.InstallLog.writeToFile("codexpanel installation successfully completed!,80")
