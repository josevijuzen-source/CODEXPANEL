...
# Test Plugin Installation Script for codexpanel
...
PLUGIN_DIR="/home/codexpanel/plugins"
codexpanel_DIR="/usr/local/CodexCP"
GITHUB_REPO="https://github.com/codexpanel/testPlugin.git"
TEMP_DIR="/tmp/codexpanel_plugin_install"
...
# Function to check if CodexPanel is installed
check_codexpanel() {
    if [ ! -d "$codexpanel_DIR" ]; then
        print_error "codexpanel is not installed at $codexpanel_DIR"
        print_error "Please install CodexPanel first: https://codexpanel.net/docs/"
        exit 1
    fi
    
    # Check if CodexPanel is running
    if ! $SERVICE_CMD is-active --quiet lscpd; then
        print_warning "codexpanel service (lscpd) is not running. Starting it..."
        $SERVICE_CMD start lscpd
    fi
    
    print_success "codexpanel installation verified"
}
...
create_plugin_directory() {
    ...
    # Create CodexPanel plugin directory
    mkdir -p "$codexpanel_DIR/$PLUGIN_NAME"
    
    # Set proper permissions
    chown -R codexpanel:codexpanel "$PLUGIN_DIR" 2>/dev/null || chown -R root:root "$PLUGIN_DIR"
    chmod -R 755 "$PLUGIN_DIR"
    
    chown -R codexpanel:codexpanel "$codexpanel_DIR/$PLUGIN_NAME" 2>/dev/null || chown -R root:root "$codexpanel_DIR/$PLUGIN_NAME"
    chmod -R 755 "$codexpanel_DIR/$PLUGIN_NAME"
...
install_plugin_files() {
    ...
    # Copy plugin files
    cp -r "$TEMP_DIR"/* "$codexpanel_DIR/$PLUGIN_NAME/"
    
    # Create symlink
    ln -sf "$codexpanel_DIR/$PLUGIN_NAME" "$PLUGIN_DIR/$PLUGIN_NAME"
    ...
    chmod +x "$codexpanel_DIR/$PLUGIN_NAME/install.sh" 2>/dev/null || true
...
update_django_settings() {
    ...
    SETTINGS_FILE="$codexpanel_DIR/codexpanel/settings.py"
...
update_urls() {
    ...
    URLS_FILE="$codexpanel_DIR/codexpanel/urls.py"
...
restart_services() {
    print_status "Restarting CodexPanel services..."
    ...
    case "$OS_NAME" in
        "ubuntu"|"debian")
            if $SERVICE_CMD is-active --quiet codexpanel; then
                $SERVICE_CMD restart codexpanel
                print_success "codexpanel service restarted"
            fi
            ;;
        "almalinux"|"rocky"|"rhel"|"centos"|"cloudlinux")
            if $SERVICE_CMD is-active --quiet codexpanel; then
...
verify_installation() {
    ...
    if [ ! -d "$codexpanel_DIR/$PLUGIN_NAME" ]; then
    ...
    if [ ! -f "$codexpanel_DIR/$PLUGIN_NAME/meta.xml" ]; then
...
display_summary() {
    ...
    echo "Installation Directory: $codexpanel_DIR/$PLUGIN_NAME"
    ...
    echo "âœ“ Enable/Disable Toggle"
    echo "âœ“ Test Button with Popup Messages"
    echo "âœ“ Settings Page"
    echo "âœ“ Activity Logs"
    echo "âœ“ Inline Integration"
    echo "âœ“ Complete Documentation"
    echo "âœ“ Official CodexPanel Guide"
    echo "âœ“ Advanced Development Guide"
    ...
    echo "âœ“ Security Information Page"
    echo "âœ“ Multi-OS Compatibility"
    echo ""
    echo "Supported Operating Systems:"
    echo "âœ“ Ubuntu 22.04, 20.04"
    echo "âœ“ Debian (compatible)"
    echo "âœ“ AlmaLinux 8, 9, 10"
    echo "âœ“ RockyLinux 8, 9"
    echo "âœ“ RHEL 8, 9"
    echo "âœ“ CloudLinux 8"
    echo "âœ“ CentOS 9"
    echo ""
...
uninstall_plugin() {
    ...
    # Remove plugin files
    rm -rf "$codexpanel_DIR/$PLUGIN_NAME"
