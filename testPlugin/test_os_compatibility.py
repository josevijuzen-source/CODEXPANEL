...
class OSCompatibilityTester:
    ...
    def run_all_tests(self):
        ...
        print("ðŸ” Testing OS Compatibility for CodexPanel Test Plugin")
        print("=" * 60)
        ...
        # Test 8: CodexPanel Integration
        self.test_codexpanel_integration()
    ...
    def test_file_permissions(self):
        ...
        try:
            ...
            can_create_codexpanel_dir = True
            ...
            try:
                os.makedirs(f"{codexpanel_dir}/test", exist_ok=True)
                os.rmdir(f"{codexpanel_dir}/test")
            except PermissionError:
                can_create_codexpanel_dir = False
            
            self.test_results['file_permissions'] = {
                'status': 'PASS' if can_create_plugin_dir and can_create_codexpanel_dir else 'WARN',
                'can_create_plugin_dir': can_create_plugin_dir,
                'can_create_codexpanel_dir': can_create_codexpanel_dir,
                'plugin_dir': plugin_dir,
                'codexpanel_dir': codexpanel_dir
            }
            
            print(f"   {'âœ…' if can_create_plugin_dir else 'âš ï¸'} Plugin Directory: {plugin_dir}")
            print(f"   {'âœ…' if can_create_codexpanel_dir else 'âš ï¸'} CodexPanel Directory: {codexpanel_dir}")
    ...
    def test_codexpanel_integration(self):
        """Test CodexPanel integration"""
        ...
        try:
            ...
            codexpanel_installed = os.path.exists(codexpanel_dir)
            
            # Check if Django settings exist
            settings_file = f"{codexpanel_dir}/codexpanel/settings.py"
            settings_exist = os.path.exists(settings_file)
            
            # Check if URLs file exists
            urls_file = f"{codexpanel_dir}/codexpanel/urls.py"
            urls_exist = os.path.exists(urls_file)
            
            # Check if lscpd service exists
            lscpd_exists = os.path.exists("/usr/local/lscp/bin/lscpd")
            
            self.test_results['codexpanel_integration'] = {
                'status': 'PASS' if codexpanel_installed and settings_exist and urls_exist else 'WARN',
                'codexpanel_installed': codexpanel_installed,
                'settings_exist': settings_exist,
                'urls_exist': urls_exist,
                'lscpd_exists': lscpd_exists
            }
            
            print(f"   {'âœ…' if codexpanel_installed else 'âš ï¸'} CodexPanel Installed: {codexpanel_installed}")
    ...
    def display_results(self):
        ...
        if failed_tests == 0:
            ...
            print("\nâŒ Some tests failed. The plugin may not work properly on this OS.")
        
        # Show detailed results
        print("\nðŸ“‹ Detailed Results:")
        for test_name, result in self.test_results.items():
            status_icon = {'PASS': 'âœ…', 'WARN': 'âš ï¸', 'FAIL': 'âŒ'}[result['status']]
