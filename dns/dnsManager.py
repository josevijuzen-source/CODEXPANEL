...
class DNSManager:
    defaultNameServersPath = '/home/codexpanel/defaultNameservers'
    ...
    def addDeleteDNSRecords(self, request = None, userID = None):
        ...
        if not os.path.exists('/home/codexpanel/powerdns'):
            finalData = {"status": 0}
        else:
            finalData = {"status": 1}
    ...
    def installPowerDNS(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                try:
                    ...
                    if e.errno != errno.EEXIST and e.errno != errno.ENOENT:
                        logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], "[ERROR] Unable to rename /etc/resolv.conf to install PowerDNS: " +
                                                 str(e) + "[404]")
                        return 0
                    try:
            ...
            logging.CodexCPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPowerDNS]")
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
...
"""
            ...
            logging.CodexCPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPowerDNSConfigurations]")
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
    ...
    def startPowerDNS(self):
        ...
        if output.strip() != 'active':
            logging.CodexCPLogFileWriter.writeToFile('[ERROR] PowerDNS failed to start. Service status: ' + output)
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
    ...
    def ResetDNSConfigurations(self):
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing DNS..,5')
            ...
            if self.installPowerDNSConfigurations(settings.DATABASES['default']['PASSWORD']) == 0:
                logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'installPowerDNSConfigurations failed. [404].')
                return 0
