# Resource Limits Guide for CodexPanel

## Overview

CodexPanel now supports comprehensive resource limits for shared hosting environments using OpenLiteSpeed's native cgroups v2 integration. This feature allows you to enforce CPU, memory, I/O, inode, process, and connection limits on a per-package basis.
...
2. **CodexPanel Version**: v2.4.5 or later
...
1. **Navigate to Packages**
   - Go to `Packages â†’ Create Package`
...
CodexPanel automatically handles OpenLiteSpeed cgroups setup:
...
1. **Check if enforcement is enabled**:
   ```bash
   mysql -u root -e "SELECT packageName, enforceDiskLimits FROM CodexPanel.packages_package;"
   ```
...
4. **Check CodexPanel logs**:
   ```bash
   tail -100 /usr/local/CodexCP/logs/access.log
   ```
...
1. CodexPanel should auto-run lssetup
...
- GitHub: https://github.com/usmannasir/CodexPanel/issues
- Community Forum: https://community.codexpanel.net
- Documentation: https://docs.codexpanel.net
