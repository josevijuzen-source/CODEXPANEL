...
def add_destination(request):
    try:
        ...
        if data['type'].lower() == IncBackupProvider.SFTP.name.lower():
            ...
            python_path = Path('/usr/local/CodexCP/bin/python')
            backup_utils = Path(vhu.codexpanel) / "plogical/backupUtilities.py"
...
def submit_backup_creation(request):
    try:
        ...
        temp_path = Path("/home/codexpanel/") / str(randint(1000, 9999))
...
def get_backup_status(request):
    try:
        ...
        if os.path.exists(status):
            command = "cat " + status
            result = pu.outputExecutioner(command, 'codexpanel')
...
def selectwebsiteRetorev2(request):
    ...
    try:
        ...
        #/home/codexpanel.net/.config/rclone/rclone.conf
