...
All notable changes to CodexPanel are documented here. The canonical,
continuously updated changelog also lives at
https://codexpanel.net/KnowledgeBase/home/change-logs/

## Unreleased

### CodexPanel-OLS stack update (core 2.5.1 / module 2.7.5 / mod_security 2.5.1)
- Upgraded the custom OpenLiteSpeed stack shipped by `upgrade.py`:
  OpenLiteSpeed core **2.5.1**, `CodexPanel_ols.so` **2.7.5**,
  `mod_security.so` **2.5.1**.
- **Fixes the 4xx segfault / Cloudflare 520 storm**: module versions
  2.7.0â€“2.7.3 crashed the OLS worker on every 4xx generated for a request
  ...
  `CodexPanel_ols` repo (`BUGREPORT_CodexPanel_ols_4xx_segfault.md`,
...
- Ubuntu < 22.04 (e.g. 20.04, glibc 2.31) now skips the custom overlay
  entirely â€” the `ubuntu` artifact needs GLIBC â‰¥ 2.34 (ticket #OXHTOK7AH).
- AlmaLinux/Rocky/RHEL 10 now install the `rhel9` artifact (el9 binary covers
  el10).

**Support notes:**
- Servers where support removed the `module CodexPanel_ols { }` block from
  `httpd_config.conf` as the emergency 520 mitigation: **restore the block
  (`ls_enabled 1`) after this upgrade lands** â€” full procedure in
  `docs/FIELD_RECOVERY_AND_ROLLOUT.md` Â§4 (CodexPanel_ols repo).
- `ls_enabled 0` + a FULL restart is now a safe kill-switch, but **only with
  module â‰¥ 2.7.5**. On 2.7.0â€“2.7.3 it does not stop the crash â€” upgrade,
  don't toggle.
...
## v2.4.8 (build 8) â€” 2026-05-30

A panel-wide UI/UX overhaul focused on making CodexPanel easier to use, calmer
to look at, and faster â€” plus integrated development services. See
`docs/UI-Guide.md` for a full walkthrough.

### Navigation & information architecture
- Replaced the deep, nested sidebar with a short, **flat, object-based
  navigation** grouped into Hosting / Account / Administration / Help â€” no more
  accordions.
...
- Added a global **command palette** (`Ctrl/âŒ˜-K`) that searches every page and
...
- Added a single global **theme harmonizer** that re-skins every internal page
  (the many pages carrying their own embedded styles) to the design tokens â€”
...
- Integrated a prominent but tasteful **Build Services** area (sidebar entry,
  ...
  the development services on CodexPanel.net, with per-service deep links and
  UTM tracking.
...
- Completed the dark theme so cards, tables, modals, pagination and the
  activity board switch correctly â€” not just the shell.
