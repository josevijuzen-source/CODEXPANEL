    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'CodexPanelCleaner purgeLimitDomain ' + domainName + ' ' + str(operationVal)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'CodexPanelCleaner updateDomainLimit ' + domainName + ' ' + str(newLimit)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'CodexPanelCleaner purgeLimit ' + emailAddress + ' ' + str(operationVal)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'CodexPanelCleaner purgeLog ' + emailAddress + ' ' + str(operationVal)
                cacheClient.handleCachePurgeRequest(command)
    ...
    try:
        ...
        try:
            if request.method == 'POST':
                ...
                command = 'CodexPanelCleaner purgeLimitEmail ' + emailAddress + ' ' + str(monthlyLimit) + ' ' + str(
                    hourlyLimit)
                cacheClient.handleCachePurgeRequest(command)
        ...
        try:
            if request.method == 'POST':
                ...
                if installStatus.find("[200]") > -1:
                    ...
                    execPath = "export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/root/bin  && /usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/mailUtilities.py"
        ...
        try:
            if request.method == 'POST':
                ...
                if mailUtilities.checkIfSpamAssassinInstalled() == 1:
                    ...
                    # logging.CodexCPLogFileWriter.writeToFile(str(data))
                    ...
                    if mailUtilities.checkIfRspamdInstalled() == 1:
                        ...
                        for items in data:
                            ...
                            if items.find('action =') > -1:
                                tempData = items.split(' ')
                                # logging.CodexCPLogFileWriter.writeToFile(str(tempData) + "action")
                                try:
                                    if len(tempData) > 4:
                                        a = tempData[4]
                                    elif len(tempData) > 2:
                        ...
                        for i in postdata:
                            if (i.find('smtpd_milters=') > -1 or i.find('smtpd_milters =') > -1) and i.find('non_smtpd_milters') < 0:
                                ...
                                if len(split_data) > 1:
                                    ...
                                    if os.path.exists(ProcessUtilities.debugPath):
                                        logging.CodexCPLogFileWriter.writeToFile(f'smtpd_milters: {tempData}')
                            ...
                            if i.find('non_smtpd_milters=') > -1 or i.find('non_smtpd_milters =') > -1:
                                ...
                                if len(eq_split) > 1:
                                    ...
                                    if os.path.exists(ProcessUtilities.debugPath):
                                        logging.CodexCPLogFileWriter.writeToFile(f'non_smtpd_milters: {tempData}')
                        ...
                        for i in postdata:
                            if i.find('write_servers =') > -1:
                                tempData = i.split(' ')
                                # logging.CodexCPLogFileWriter.writeToFile(str(tempData) + "redis")
                                if len(tempData) > 2:
                                    write = tempData[2]
                                    i = write.rstrip(";")
                                    j = i.split('"')
                                    if len(j) > 1:
                                        write_servers = j[1]
                                    # logging.CodexCPLogFileWriter.writeToFile(str(write_servers) + "write_servers")

                            if i.find('read_servers =') > -1:
                                tempData = i.split(' ')
                                # logging.CodexCPLogFileWriter.writeToFile(str(tempData) + "redis2")
                                if len(tempData) > 2:
                                    read = tempData[2]
                                    i = read.rstrip(";")
                                    j = i.split('"')
                                    if len(j) > 1:
                                        read_servers = j[1]
                                    # logging.CodexCPLogFileWriter.writeToFile(str(read_servers) + "read_servers")
...
def unistallRspamd(request):
    try:
        logging.CodexCPLogFileWriter.writeToFile("unistallRspamd...1")
        userID = request.session['userID']
        currentACL = ACLManager.loadedACL(userID)
    ...
    try:
        ...
        if ACLManager.CheckForPremFeature('email-debugger'):
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile("Result....3:" + str(msg))
        else:
            dic = {'status': 0, 'error_message': 'Kindly purchase email debugger Add-on'}
            json_data = json.dumps(dic)
            return HttpResponse(json_data)
...
def fixMailSSL(request):
    try:
        ...
        if ACLManager.CheckForPremFeature('email-debugger'):
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.CodexCPLogFileWriter.writeToFile("Result....3:" + str(res.content))
            return res
        else:
            dic = {'status': 0, 'error_message': 'Kindly purchase email debugger Add-on'}
            json_data = json.dumps(dic)
