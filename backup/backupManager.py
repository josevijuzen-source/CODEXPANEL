...
class BackupManager:
    localBackupPath = '/home/codexpanel/localBackupPath'
    ...
    def changeFileRetention(self, request=None, userID=None, data=None):
        try:
            ...
            # logging.CodexCPLogFileWriter.writeToFile("...... FileRetentiontime...%s "%Retentiontime)

            gD = GDrive.objects.get(name=selectedAccount)
            # logging.CodexCPLogFileWriter.writeToFile("...... GDrive obj...%s " % GDrive)
            ...
            conf = gD.auth
            # logging.CodexCPLogFileWriter.writeToFile("...... conf...%s " % conf)
            config = json.loads(conf)
            # logging.CodexCPLogFileWriter.writeToFile("...... config...%s " % config)
    ...
    def backupStatus(self, userID=None, data=None):
        try:
            ...
            final_json = json.dumps(final_dic)
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [backupStatus]")
            return HttpResponse(final_json)
    ...
    def cancelBackupCreation(self, userID=None, data=None):
        try:
            ...
            execPath = "/usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/backupUtilities.py"
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [cancelBackupCreation]")
        ...
        try:
            ...
            if data['type'] == 'SFTP':
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile(execPath)
    ...
    def scheduleBackup(self, request, userID=None, data=None):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [scheduleBackup]')
    ...
    def scheduleDelete(self, userID=None, data=None):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(findTxt)
            logging.CodexCPLogFileWriter.writeToFile(backupFreq)

            path = "/etc/crontab"

            command = "cat " + path
            output = ProcessUtilities.outputExecutioner(command).split('\n')
            tempCronPath = "/home/codexpanel/" + str(randint(1000, 9999))
    ...
    def submitRemoteBackups(self, userID=None, data=None):
        try:
            ...
            ## Ask for Remote version of codexpanel

            try:
                finalData = json.dumps({'username': "admin", "password": password})

                url = "https://" + ipAddress + ":8090/api/CodexPanelVersion"
            ...
            pathToKey = "/home/codexpanel/" + str(randint(1000, 9999))
            ...
            execPath = "/usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/remoteTransferUtilities.py"
    ...
    def starRemoteTransfer(self, userID=None, data=None):
        try:
            ...
            try:
                ...
                ipFile = os.path.join("/etc", "CodexPanel", "machineIP")
                f = open(ipFile)
                ownIP = f.read()
                ...
                if os.path.exists('/usr/local/CodexCP/debug'):
    ...
    def CreateV2BackupStatus(self, userID=None, data=None):
        try:
            ...
            statusFile = f'/home/codexpanel/{domain}_rustic_backup_log'
            ...
            #currentStatus:"cat: /home/codexpanel/9219: No such file or directory"
        ...
        try:
            if request.GET.get('status', 'none') == 'success':
                ...
                try:
                    ...
                    payload = {
                        'sub': subscription,
                        'key': ProcessUtilities.outputExecutioner(f'cat /root/.ssh/codexpanel.pub'),  # Replace with the actual SSH public key
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if api_data.get('status') == 1:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile(f'Successfully fetched backup stats for {ocb.sftpUser} [ManageOCBackups]')
                else:
                    logging.CodexCPLogFileWriter.writeToFile(f'Platform API returned error: {api_data.get("error_message")} [ManageOCBackups]')
            else:
                logging.CodexCPLogFileWriter.writeToFile(f'Platform API returned HTTP {response.status_code} [ManageOCBackups]')
        except Exception as e:
            logging.CodexCPLogFileWriter.writeToFile(f'Failed to fetch backup stats: {str(e)} [ManageOCBackups]')
    ...
    def RestoreOCBackups(self, request=None, userID=None, data=None):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [RestoreOCBackups]')
            return HttpResponse("Error: " + str(msg))
        ...
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"Failed to get backup destination: {str(e)} [RestoreOCBackups]")
        ...
        try:
            ...
            if check_exists == "NOT_EXISTS":
                logging.CodexCPLogFileWriter.writeToFile(f"SSH key not found at {private_key_path} [RestoreOCBackups]")
            ...
            if not key_content or key_content.startswith('cat:'):
                logging.CodexCPLogFileWriter.writeToFile(f"Failed to read SSH key at {private_key_path} [RestoreOCBackups]")
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"SSH Authentication failed: {str(e)} [RestoreOCBackups]")
            return HttpResponse("Error: SSH Authentication failed. Please check your One-click Backup configuration.")
        except paramiko.SSHException as e:
            logging.CodexCPLogFileWriter.writeToFile(f"SSH Connection failed: {str(e)} [RestoreOCBackups]")
            return HttpResponse(f"Error: Failed to connect to backup server: {str(e)}")
        except Exception as e:
            logging.CodexCPLogFileWriter.writeToFile(f"Unexpected error during SSH operation: {str(e)} [RestoreOCBackups]")
    ...
    def fetchOCSites(self, request=None, userID=None, data=None):
        ...
        try:
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"OneClickBackup with id {id} not found for user {userID} [fetchOCSites]")
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Backup destination {ocb.sftpUser} not found [fetchOCSites]")
                data_ret = {'status': 0, 'error_message': 'Backup destination not configured. Please deploy your backup account first.'}
                return HttpResponse(json.dumps(data_ret))
            except (KeyError, json.JSONDecodeError) as e:
                logging.CodexCPLogFileWriter.writeToFile(f"Invalid backup destination config for {ocb.sftpUser}: {str(e)} [fetchOCSites]")
            ...
            if check_exists == "NOT_EXISTS":
                logging.CodexCPLogFileWriter.writeToFile(f"SSH key not found at {private_key_path} [fetchOCSites]")
            ...
            if not key_content or key_content.startswith('cat:'):
                logging.CodexCPLogFileWriter.writeToFile(f"Failed to read SSH key at {private_key_path} [fetchOCSites]")
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.CodexCPLogFileWriter.writeToFile(f"Failed to load SSH key: {str(e)} [fetchOCSites]")
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"SSH Authentication failed for {ocb.sftpUser}@{ip}: {str(e)} [fetchOCSites]")
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"SSH Connection failed to {ip}: {str(e)} [fetchOCSites]")
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Unexpected SSH error connecting to {ip}: {str(e)} [fetchOCSites]")
            ...
            try:
                ...
                for directory in directories:
                    if directory and '/' in directory:
                        try:
                            ...
                            logging.CodexCPLogFileWriter.writeToFile(f"Failed to parse directory path '{directory}': {str(e)} [fetchOCSites]")
                            continue
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Failed to execute command on remote server: {str(e)} [fetchOCSites]")
                data_ret = {'status': 0, 'error_message': f'Failed to list backups: {str(e)}'}
                return HttpResponse(json.dumps(data_ret))

        except json.JSONDecodeError as e:
            logging.CodexCPLogFileWriter.writeToFile(f"Invalid JSON in request: {str(e)} [fetchOCSites]")
            data_ret = {'status': 0, 'error_message': 'Invalid request format.'}
            return HttpResponse(json.dumps(data_ret))
        except KeyError as e:
            logging.CodexCPLogFileWriter.writeToFile(f"Missing required field in request: {str(e)} [fetchOCSites]")
            data_ret = {'status': 0, 'error_message': f'Missing required field: {str(e)}'}
            return HttpResponse(json.dumps(data_ret))
        except Exception as msg:
            logging.CodexCPLogFileWriter.writeToFile(f"Unexpected error in fetchOCSites: {str(msg)}")
    ...
    def StartOCRestore(self, request=None, userID=None, data=None):
        try:
            ...
            extraArgs['tempStatusPath'] = "/home/codexpanel/" + str(randint(1000, 9999))
    ...
    def DeployAccount(self, request=None, userID=None, data=None):
        ...
        try:
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Invalid request data in DeployAccount: {str(e)}")
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"OneClickBackup {backup_id} not found for user {userID} [DeployAccount]")
                return HttpResponse(json.dumps({
                    'status': 0,
            ...
            if ocb.state == 1:
                logging.CodexCPLogFileWriter.writeToFile(f"Backup plan {backup_id} already deployed [DeployAccount]")
                return HttpResponse(json.dumps({
                    'status': 1,
            ...
            # Read SSH public key
            try:
                ssh_pub_key = ProcessUtilities.outputExecutioner('cat /root/.ssh/codexpanel.pub').strip()
                if not ssh_pub_key or ssh_pub_key.startswith('cat:'):
                    raise Exception("Failed to read SSH public key")
            except Exception as e:
                logging.CodexCPLogFileWriter.writeToFile(f"Failed to read SSH public key: {str(e)} [DeployAccount]")
                return HttpResponse(json.dumps({
                    'status': 0,
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"API request failed: {str(e)} [DeployAccount]")
                return HttpResponse(json.dumps({
                    'status': 0,
            ...
            if response.status_code != 200:
                logging.CodexCPLogFileWriter.writeToFile(f"API returned status {response.status_code}: {response.text} [DeployAccount]")
                return HttpResponse(json.dumps({
                    'status': 0,
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Invalid JSON response from API: {response.text} [DeployAccount]")
                return HttpResponse(json.dumps({
                    'status': 0,
            ...
            if api_status == 1 or api_error == "Already deployed.":
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"SFTP account {deployment_status} for {ocb.sftpUser} [DeployAccount]")
                ...
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile(f"Failed to create destination: {str(e)} [DeployAccount]")
                    return HttpResponse(json.dumps({
                        'status': 0,
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"API returned error: {api_error} [DeployAccount]")
                return HttpResponse(json.dumps({
                    'status': 0,
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"Unexpected error in DeployAccount: {str(e)}")
            return HttpResponse(json.dumps({
                'status': 0,
    ...
    def ReconfigureSubscription(self, request=None, userID=None, data=None):
        try:
            ...
            payload = {
                ...
                'key': ProcessUtilities.outputExecutioner(f'cat /root/.ssh/codexpanel.pub'),
                'serverIP': ACLManager.fetchIP(),
                'email': data['email'],
                'code': data['code']
            }
            ...
            if response.status_code == 200:
                ...
                if response_data.get('status') == 1:
                    ...
                    # Create SFTP destination in codexpanel
                    finalDic = {
                        'IPAddress': response_data.get('ipAddress'),
                        'password': 'NOT-NEEDED',
                        'backupSSHPort': '22',
