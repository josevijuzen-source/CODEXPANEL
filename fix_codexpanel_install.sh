...
if [[ ! -f /usr/local/CodexCP/bin/activate ]]; then
    print_error "codexpanel virtual environment not found!"
    print_status "Creating virtual environment..."
    
    # Try python3 -m venv first
    if python3 -m venv --system-site-packages /usr/local/CodexCP 2>/dev/null; then
        print_status "Virtual environment created successfully with python3 -m venv"
    else
        # Fallback to virtualenv
        virtualenv -p /usr/bin/python3 --system-site-packages /usr/local/CodexCP
    fi
fi

# Activate virtual environment
print_status "Activating CodexPanel virtual environment..."
...
if python -c "import django" 2>/dev/null; then
    ...
    print_status "Installing CodexPanel requirements (this may take a few minutes)..."
...
if [[ -f /usr/local/lscp/conf/pythonenv.conf ]] && grep -q '^PYTHONHOME=/usr' /usr/local/lscp/conf/pythonenv.conf 2>/dev/null; then
    ...
    RUNTIME_REQ=""
    if [[ -f /etc/codexpanel/codexpanel-requirments-runtime.txt ]] && grep -q 'Django==' /etc/codexpanel/codexpanel-requirments-runtime.txt 2>/dev/null; then
...
if [[ ! -f /usr/local/CodexCP/bin/lswsgi ]]; then
    ...
    /usr/local/CodexCP/bin/python ./configure.py
    make
...
chown -R codexpanel:codexpanel /usr/local/CodexCP/lib 2>/dev/null || true
chown -R codexpanel:codexpanel /usr/local/CodexCP/lib64 2>/dev/null || true
...
echo ""
print_status "codexpanel fix completed!"
echo ""
echo "You can now access CodexPanel at: https://$(hostname -I | awk '{print $1}'):8090"
echo ""
