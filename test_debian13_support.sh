...
# Debian 13 Support Test Script for codexpanel
# This script tests the compatibility of CodexPanel with Debian 13
...
# Function to test CodexPanel OS detection logic
test_codexpanel_os_detection() {
    print_test_header "codexpanel OS Detection Logic Test"
    
    # Test the OS detection logic from codexpanel.sh
    if grep -q -E "Debian GNU/Linux 11|Debian GNU/Linux 12|Debian GNU/Linux 13" /etc/os-release; then
        print_success "codexpanel OS detection logic recognizes Debian 11/12/13"
    else
        print_error "codexpanel OS detection logic does not recognize current Debian version"
        return 1
    fi
}
...
test_web_server_compatibility() {
    ...
    if command -v apache2 >/dev/null 2>&1; then
        ...
        print_warning "Apache2 not found (will be installed by codexpanel)"
    fi
...
run_all_tests() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  CodexPanel Debian 13 Compatibility Test${NC}"
    echo -e "${BLUE}========================================${NC}\n"
    ...
    if [[ $TESTS_FAILED -eq 0 ]]; then
        echo -e "\n${GREEN}âœ… All tests passed! Debian 13 appears to be compatible with codexpanel.${NC}"
        return 0
    elif [[ $TESTS_FAILED -le 2 ]]; then
        echo -e "\n${YELLOW}âš ï¸  Most tests passed. Debian 13 should be compatible with minor issues.${NC}"
        return 0
    else
        echo -e "\n${RED}âŒ Multiple tests failed. Debian 13 may have compatibility issues.${NC}"
        return 1
    fi
}
...
show_usage() {
    ...
    echo ""
    echo "This script tests CodexPanel compatibility with Debian 13."
