...
def generateAccess(request):
    try:
        ...
        keySavePath = '/home/codexpanel/phpmyadmin_%s' % (admin.userName)
        try:
            GlobalUserDB.objects.get(username=admin.userName).delete()
        except:
            pass
...
def upgrademysqlnow(request):
    try:
        ...
        execPath = f"/usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/mysqlUtilities.py UpgradeMariaDB --version {version} --tempStatusPath {tempStatusPath}"
