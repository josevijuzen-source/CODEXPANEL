...
## Objective Fix permissions issues on codexpanel/cPanel/Plesk for a linux user or users
...
elif [ -f /usr/bin/codexpanel ]; then
    	# CodexPanel check /usr/bin/codexpanel
    	ControlPanel="codexpanel"
...
helptext () {
    ...
    echo "-all: run on all CodexPanel accounts"
    echo "--account or -a: specify a codexpanel/cPanel/Plesk account"
#   echo "--domain or -d: specify a CodexPanel domain"
    tput sgr0
    exit 0
}
...
fixperms () {
	...
	elif [ "${ControlPanel}" == "codexpanel" ] ; then

		fixperms_codexpanel "${Username}"
		fixmailperms_codexpanel
