...
## Objective Fix session issues on CodexPanel and standardized session paths.
# Fixes #430
# https://github.com/usmannasir/CodexPanel/issues/430
...
if [[ ! -e /usr/local/CodexCP/bin/cleansessions ]]; then
	touch /usr/local/CodexCP/bin/cleansessions
	chmod +x /usr/local/CodexCP/bin/cleansessions
	cat >> /usr/local/CodexCP/bin/cleansessions <<"EOL"
#!/bin/bash
...
command="/usr/local/CodexCP/bin/cleansessions >/dev/null 2>&1"
job="09,39 * * * * $command"
cat <(grep -i -v "$command" <(crontab -l)) <(echo "$job") | crontab -

echo "Checking cleansessions file"
cat /usr/local/CodexCP/bin/cleansessions
