...
## Objective Find A CodexPanel Users Domlogs Stats for last 5 days for all of their domains. v2
## https://gitlab.com/codexpaneltoolsnscripts/snapshotbycodexpaneluser
## How to use.
# ./codexpanelSnapshotBycodexpanelUser.sh username
#./codexpanelSnapshotcodexpanelUser.sh exampleuserbob
#
##bash <(curl -s https://gitlab.com/codexpaneltoolsnscripts/snapshotbycodexpaneluser/-/raw/master/codexpanelSnapshotBycodexpanelUser.sh || wget -qO - https://gitlab.com/codexpaneltoolsnscripts/snapshotbycodexpaneluser/-/raw/master/codexpanelSnapshotBycodexpanelUser.sh) exampleuserbob;
##
Username=$1
...
user_codexpanelSnapshot="${Username}-codexpanelSnapshot_${Now}.txt";

#create logfile in user's homedirectory.
#sudo touch "$user_codexpanelSnapshot"

#chown logfile to user
#sudo chown ${Username}:${Username} "$user_codexpanelSnapshot";
...
echo "Contents have been saved to ${user_codexpanelSnapshot}"
}

# log everything, but also output to stdout
main_function 2>&1 | tee -a "${user_codexpanelSnapshot}"