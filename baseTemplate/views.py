...
def versionManagement(request):
    ...
    u = "https://api.github.com/repos/josevijuzen-source/CODEXPANEL/commits?sha=v%s.%s" % (latestVersion, latestBuild)
    logging.writeToFile(u)
    r = requests.get(u)
    latestcomit = r.json()[0]['sha']

    command = "git -C /usr/local/CodexCP/ rev-parse HEAD"
...
@ensure_csrf_cookie
def upgrade_CodexPanel(request):
    if request.method == 'POST':
        try:
            upgrade_command = 'sh <(curl https://raw.githubusercontent.com/josevijuzen-source/CODEXPANEL/main/preUpgrade.sh || wget -O - https://raw.githubusercontent.com/josevijuzen-source/CODEXPANEL/main/preUpgrade.sh)'
            result = subprocess.run(upgrade_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                    universal_newlines=True)

            if result.returncode == 0:
                response_data = {'success': True, 'message': 'codexpanel upgrade completed successfully.'}
            else:
                response_data = {'success': False,
                                 'message': 'codexpanel upgrade failed. Error output: ' + result.stderr}
...
def getSystemStatus(request):
    try:
        ...
        if currentACL.get('admin', 0):
            ...
            # and was running on every dashboard poll â€” cache the result briefly.
...
def versionManagment(request):
    ...
    u = "https://api.github.com/repos/josevijuzen-source/CODEXPANEL/commits?sha=v%s.%s" % (latestVersion, latestBuild)
    logging.CodexCPLogFileWriter.writeToFile(u)
    r = requests.get(u)
    latestcomit = r.json()[0]['sha']
...
def design(request):
    ### Load Custom CSS
    try:
        from baseTemplate.models import CodexPanelCosmetic
    ...
    sha_url = "https://api.github.com/repos/josevijuzen-source/CODEXPANEL-Themes/commits"

    sha_res = requests.get(sha_url)

    sha = sha_res.json()[0]['sha']

    l = "https://api.github.com/repos/josevijuzen-source/CODEXPANEL-Themes/git/trees/%s" % sha
    fres = requests.get(l)
...
def getthemedata(request):
    try:
        ...
        # logging.CodexCPLogFileWriter.writeToFile(str(data) + "  [themedata]")

        url = "https://raw.githubusercontent.com/josevijuzen-source/CODEXPANEL-Themes/main/%s/design.css" % data['Themename']
...
def buildServices(request):
    # In-panel landing for CodexPanel development services (Android, iOS,
    # web and custom software). The full marketing page lives on
    # codexpanel.net; this page introduces the offering and deep-links out.
...
def runonboarding(request):
    try:
        ...
        command = f'/usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/virtualHostUtilities.py OnBoardingHostName --virtualHostName {hostname} --path {tempStatusPath} --rdns {rDNSCheck}'
...
def analyzeSSHSecurity(request):
    try:
        ...
        # Check if user has CodexPanel addons
        if not ACLManager.CheckForPremFeature('all'):
            return HttpResponse(json.dumps({
                ...
                'addon_url': 'https://codexpanel.net/codexpanel-addons'
        ...
        if not alerts:
            ...
            try:
                ...
                if 'PermitRootLogin yes' in sshd_config:
                    ssh_config_recommendations.append('â€¢ Disable root login: Set "PermitRootLogin no" in /etc/ssh/sshd_config')
                if 'Port 22' in sshd_config:
                    ssh_config_recommendations.append('â€¢ Change default SSH port from 22 to reduce automated attacks')
            except:
                pass
...
def blockIPAddress(request):
    ...
    try:
        ...
        if success:
            # Log the action
            import plogical.CodexCPLogFileWriter as logging
            logging.CodexCPLogFileWriter.writeToFile(f'IP address {ip_address} blocked via CodexPanel dashboard by user {user_id}')
