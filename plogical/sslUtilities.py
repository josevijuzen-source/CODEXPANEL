...
class sslUtilities:
        ...
        try:
            ...
            with open(cert_path, 'rb') as cert_file:
                ...
                if san_extension:
                    ...
                    try:
                        logging.CodexCPLogFileWriter.writeToFile(f'Covered domains: {str(san_domains)}')
                    except:
                        pass
    ...
    def CheckIfSSLNeedsToBeIssued(virtualHostName):
        ...
        if os.path.exists(filePath):
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.CodexCPLogFileWriter.writeToFile(f'SSL provider for {virtualHostName} is {SSLProvider}.')
    ...
    def checkIfSSLMap(virtualHostName):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [IO Error with main config file [checkIfSSLMap]]")
            return 0
    ...
    def checkSSLListener():
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [IO Error with main config file [checkSSLListener]]")
            return str(msg)
        return 0
    ...
    def PatchVhostConf(virtualHostName):
        ...
        try:
            ...
            if not os.path.exists(completePathToConfigFile):
                logging.CodexCPLogFileWriter.writeToFile(f'Configuration file not found: {completePathToConfigFile}')
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f'Error reading configuration file: {str(e)}')
            ...
            if DataVhost.find('/.well-known/acme-challenge') != -1:
                logging.CodexCPLogFileWriter.writeToFile(f'ACME challenge already configured for {virtualHostName}')
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f'Error creating challenge directory: {str(e)}')
...
'''
                    ...
                    logging.CodexCPLogFileWriter.writeToFile(f'Error writing OLS configuration: {str(e)}')
                ...
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile(f'Error writing Apache configuration: {str(e)}')
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f'Error restarting LiteSpeed: {str(e)}')
                return 0, f'Error restarting LiteSpeed: {str(e)}'

        except Exception as e:
            logging.CodexCPLogFileWriter.writeToFile(f'Unexpected error in PatchVhostConf: {str(e)}')
            return 0, f'Unexpected error: {str(e)}'

    @staticmethod
    def installSSLForDomain(virtualHostName, adminEmail='domain@codexpanel.net'):

        try:
            website = Websites.objects.get(domain=virtualHostName)
            adminEmail = website.adminEmail
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile('%s [installSSLForDomain:72]' % (str(msg)))
        ...
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [installSSLForDomain]]")
                return 0
        else:
    ...
    """
                ...
                logging.CodexCPLogFileWriter.writeToFile('hello world aaa')
    ...
    def obtainSSLForADomain(virtualHostName, adminEmail, sslpath, aliasDomain=None, isHostname=False, forceIssue=False):
        ...
        if adminEmail and ('example.org' in adminEmail or 'example.com' in adminEmail):
            ...
            adminEmail = f'{clean_domain}@codexpanel.net'
            logging.CodexCPLogFileWriter.writeToFile(f'Replacing invalid email with {adminEmail}')
                    ...
                    if result.returncode == 0:
                        ...
                        if result.returncode == 0:
                            ...
                            if install_result.returncode == 0:
                                ...
                                logging.CodexCPLogFileWriter.SendEmail(sender_email, adminEmail, result.stdout,
                                                                       'SSL Notification for %s.' % (virtualHostName))
                                return 1
                    return 0
                except Exception as e:
                    logging.CodexCPLogFileWriter.writeToFile(str(e))
                    return 0
            else:
                existingCertPath = '/etc/letsencrypt/live/' + virtualHostName
                if not os.path.exists(existingCertPath):
    ...
    try:
        ...
        if os.path.exists(existingCertPath):
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Certificate for {domain} expires in {diff.days} days")
            except Exception as e:
                logging.CodexCPLogFileWriter.writeToFile(f"Could not check certificate expiry: {str(e)}")

            logging.CodexCPLogFileWriter.writeToFile(f"Certificate exists for {domain}, attempting renewal...")
            ...
            if os.path.exists(acmePath):
                ...
                if result.returncode == 0:
                    logging.CodexCPLogFileWriter.writeToFile(f"Successfully renewed SSL for {domain}")
                    ...
                    logging.CodexCPLogFileWriter.writeToFile(f"Renewal failed for {domain}. Error: {error_details}")
                    logging.CodexCPLogFileWriter.writeToFile(f"Full error output: {error_output}")
        ...
        if sslUtilities.obtainSSLForADomain(domain, adminEmail, sslpath, aliasDomain, isHostname, forceIssue) == 1:
            ...
            #### if in any case ssl failed to obtain and CodexPanel try to issue self-signed ssl, first check if ssl already present.
            ...
            if sslUtilities.installSSLForDomain(domain) == 1:
                logging.CodexCPLogFileWriter.writeToFile("Self signed SSL issued for " + domain + ".")
