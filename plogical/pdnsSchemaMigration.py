...
PowerDNS 4.7+ (including 4.8/4.9/5.0.x) require but older CodexPanel installs
never received.
...
transitive dependency of unrelated package upgrades) but CodexPanel has
...
allows VARCHAR(64000). codexpanel's Django ORM, however, may have created
...
Invocation
----------
    From CodexPanel upgrade  : Upgrade.pdnsSchemaMigrations() in upgrade.py
    From fresh install       : right before startPowerDNS()
    Directly (customer SOS)  :
        /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/pdnsSchemaMigration.py
"""
...
PDNS_DB_NAME_DEFAULT = 'codexpanel'
MYSQL_PASSWORD_FILE = '/etc/codexpanel/mysqlPassword'
HEALTH_FILE = '/etc/codexpanel/health.json'
ERROR_LOG_FILE = '/home/codexpanel/error-logs.txt'
...
def _connect(db_name):
    ...
    try:
        ...
        from CodexCP import settings as django_settings
