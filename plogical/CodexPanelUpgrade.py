...
class Upgradecodexpanel:
    ...
    def PostStatus(self, message):
        finalData = json.dumps({'ipAddress': self.ipAddress, "UpgradecodexpanelStatus": message})

        try:
            resp = requests.post(Upgradecodexpanel.LogURL, data=finalData, timeout=10)
        except:
            pass
    ...
    def RestoreOldCP(self):
        ...
        command = 'rm -rf /usr/local/CodexCP'
        Upgrade.executioner(command, command)

        command = 'mv /usr/local/CodexCPBak /usr/local/CodexCP'
        Upgrade.executioner(command, command)
    ...
    def UpgardeNow(self):
        ...
        ## Backup current CodexCP, incase of failure restore

        self.PostStatus('Backing up current installation..,5')

        command = 'cp -R /usr/local/CodexCP /usr/local/CodexCPBak'
        Upgrade.executioner(command, command)
        ...
        self.PostStatus('codexpanel is now on %s..,40' % (self.branch))
        ...
        self.PostStatus('codexpanel Upgraded/Downgraded to %s. [200]' % (self.branch))
...
def main():
    ...
    uc = Upgradecodexpanel(args.branch,int(args.mail),int(args.dns),int(args.ftp))
    uc.UpgardeNow()
