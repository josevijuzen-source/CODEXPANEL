...
class sslUtilities:
    ...
    def FindIfDomainInCloudflare(virtualHostName):
        try:
            ...
            if RetStatus:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f'top level domain in cf: {topLevelDomain}')
                import CloudFlare
                ...
                for zone in sorted(zones, key=lambda v: v['name']):
                    logging.CodexCPLogFileWriter.writeToFile(f'zone: {zone["name"]}')
                    if zone['name'] == topLevelDomain:
                        if zone['status'] == 'active':
                            return 1, None
                        else:
                            logging.CodexCPLogFileWriter.writeToFile(f'zone is not active in cf: {zone["name"]}')
    ...
    def FindIfDomainInPowerDNS(virtualHostName):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(f'PDNS Result: {str(result)}.')
    ...
    def obtainSSLForADomain(virtualHostName, adminEmail, sslpath, aliasDomain=None, forceIssue=False):
        ...
        if CF_Check:
            ...
            codexpanel_Check, message = sslUtilities.FindIfDomainInPowerDNS(virtualHostName)

            if codexpanel_Check:
                DNS_TO_USE = 'dns_codexpanel'
            else:
        ...
        try:
            ...
            #     acmePath = '/home/codexpanel/.acme.sh/acme.sh'
            ...
            if aliasDomain is None:
                ...
                try:
                    ...
                    # ResultText = open(logging.CodexCPLogFileWriter.fileName, 'r').read()
                    # CurrentMessage = "Trying to obtain SSL for: " + virtualHostName + " and: www." + virtualHostName
                    # logging.CodexCPLogFileWriter.writeToFile(CurrentMessage, 0)

                    logging.CodexCPLogFileWriter.writeToFile(command, 0)

                    output = subprocess.check_output(shlex.split(command)).decode("utf-8")
                    logging.CodexCPLogFileWriter.writeToFile(
                        "Successfully obtained SSL for: " + virtualHostName + " and: www." + virtualHostName, 0)

                    logging.CodexCPLogFileWriter.SendEmail(sender_email, adminEmail, output,
                    ...
                    try:
                        ...
                        finalText = '%s\nTrying to obtain SSL for: %s' % (finalText, virtualHostName)
                        logging.CodexCPLogFileWriter.writeToFile("Trying to obtain SSL for: " + virtualHostName, 0)
                        logging.CodexCPLogFileWriter.writeToFile(command)
                        output = subprocess.check_output(shlex.split(command)).decode("utf-8")
                        logging.CodexCPLogFileWriter.writeToFile(
                            "Successfully obtained SSL for: " + virtualHostName, 0)
                        finalText = '%s\nSuccessfully obtained SSL for: %s.' % (finalText, virtualHostName)
                        logging.CodexCPLogFileWriter.SendEmail(sender_email, adminEmail, finalText,
                                                               'SSL Notification for %s.' % (virtualHostName))

                    except subprocess.CalledProcessError:
                        logging.CodexCPLogFileWriter.writeToFile(
                            'Failed to obtain SSL, issuing self-signed SSL for: ' + virtualHostName, 0)
                        logging.CodexCPLogFileWriter.SendEmail(sender_email, adminEmail,
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [Failed to obtain SSL. [obtainSSLForADomain]]")
            return 0, str(msg)
