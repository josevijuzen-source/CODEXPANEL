...
class mysqlUtilities:
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile('Local IP for MySQL: %s' % (mysqlUtilities.LOCALHOST))
    ...
    def createDatabase(dbname,dbuser,dbpassword, dbcreate = 1, host = None):
        try:
            ...
            if dbcreate:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile(query)
            ...
            if mysqlUtilities.RDS == 0:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile("GRANT ALL PRIVILEGES ON " + dbname + ".* TO '" + dbuser + "'@'%s'" % (HostToUse))
            else:
                cursor.execute(
                    "GRANT INDEX, DROP, UPDATE, ALTER, CREATE, SELECT, INSERT, DELETE ON " + dbname + ".* TO '" + dbuser + "'@'%s'" % (HostToUse))
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile("GRANT INDEX, DROP, UPDATE, ALTER, CREATE, SELECT, INSERT, DELETE ON " + dbname + ".* TO '" + dbuser + "'@'%s'" % (HostToUse))
            ...
            if dbcreate:
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile('Deleting database because failed to create %s' % (dbname))
                #mysqlUtilities.deleteDatabase(dbname, dbuser)
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[createDatabase]")
            return 0
    ...
    def createDBUser(dbuser, dbpassword):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[createDBUser]")
            return 0
    ...
    def deleteDatabase(dbname, dbuser):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[deleteDatabase]")
            return str(msg)
    ...
    def createDatabaseBackup(databaseName, tempStoragePath, rustic=0, RusticRepoName = None,
        ...
        try:
            ...
            cnfPath = '/home/codexpanel/.my.cnf'
...
""" % (password, password)
                ...
                if use_compression:
                    ...
                    with open(backup_file, 'w') as f:
                        ...
                        if result.returncode != 0:
                            ...
                            logging.CodexCPLogFileWriter.writeToFile(result.stderr.decode('utf-8'))
                            return 0
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[createDatabaseBackup]")
            return 0
...
""" % (password, password)
                ...
                os.chmod(cnfPath, 0o600)
                command = 'chown codexpanel:codexpanel %s' % (cnfPath)
            ...
            if rustic == 0:
                ...
                mysql_cmd = f'mysql --defaults-file=/home/codexpanel/.my.cnf -u {mysqluser} --host={mysqlhost} --port {mysqlport} {databaseName}'
                ...
                command = f'sudo -u {externalApp} rustic -r {RusticRepoName} dump {snapshotid}:{databaseName}.sql --password "" 2>/dev/null | mysql --defaults--file=/home/codexpanel/.my.cnf -u %s --host=%s --port %s %s' % (
                mysqluser, mysqlhost, mysqlport, databaseName)
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile(f'{command} {tempStoragePath}/{databaseName} ')
                ProcessUtilities.outputExecutioner(command, None, True)

                return 1

        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[restoreDatabaseBackup]")
            return 0
    ...
    def submitDBDeletion(dbName):
        try:
            ...
            if result == 1:
                ...
                databaseToBeDeleted.delete()
                logging.CodexCPLogFileWriter.writeToFile('Deleted database with some errors. Error: %s' % (result))
                return 1,'None'
    ...
    def showStatus():
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[showStatus]")
            return 0
    ...
    def fetchVariables():
        try:
            ...
            for items in result:
                logging.CodexCPLogFileWriter.writeToFile(str(items))
    ...
    def fetchDatabases():
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[fetchDatabases]")
            return 0
    ...
    def fetchTableData(name):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[fetchTableData]")
            return 0
    ...
    def changePassword(userName, dbPassword, encrypt = None, host = None):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.changePassword]")
            return 0
    ...
    def fetchuser(databaseName):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.fetchuser]")
            return 0
    ...
    def allowRemoteAccess(dbName, userName, remoteIP):
        try:
            ...
            execPath = "/usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/mysqlUtilities.py enableRemoteMYSQL"
            ProcessUtilities.executioner(execPath)

            return 1

        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[mysqlUtilities.allowRemoteAccess]")
            return 0
...
''' % (ipAddressLocal)
        ...
        try:
            ...
            if createUser:
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile('%s [addUserToDB:937]' % (str(msg)))
                    try:
                        cursor.execute("DROP USER '%s'@'%s'" % (user, mysqlUtilities.LOCALHOST))
                        cursor.execute(
                            "CREATE USER '" + user + "'@'%s' IDENTIFIED BY '" % (mysqlUtilities.LOCALHOST) + password + "'")
                    except BaseException as msg:
                        logging.CodexCPLogFileWriter.writeToFile('%s [addUserToDB:943]'  % (str(msg)))
            ...
            if mysqlUtilities.RDS == 0:
                ...
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile('%s [addUserToDB:953]' % (str(msg)))
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[addUserToDB]")
            return 0
    ...
    def UpgradeMariaDB(versionToInstall, tempStatusPath):
        ...
        if result.find(versionToInstall) > -1:
            ...
            logging.CodexCPLogFileWriter.statusWriter(tempStatusPath, f'MySQL is already {result}. [200]')
            return 0

        logging.CodexCPLogFileWriter.statusWriter(tempStatusPath, 'Creating backup of MySQL..,10')

        MySQLBackupDir = '/var/lib/mysql-backupcp'

        from os import getuid
        if getuid() != 0:
            logging.CodexCPLogFileWriter.statusWriter(tempStatusPath, 'This function should run as root. [404]')
        ...
        if not os.path.exists(MySQLBackupDir):
            ...
            logging.CodexCPLogFileWriter.statusWriter(tempStatusPath, 'MySQL backup created..,20')
...
"""
    ...
    def buildMysqldumpCommand(user, host, port, database, use_new_features, use_compression):
        ...
        base_cmd = f"mysqldump --defaults-file=/home/codexpanel/.my.cnf -u {user} --host={host} --port {port}"
    ...
    def checkNewBackupFeatures():
        ...
        try:
            config_file = '/usr/local/CodexCP/plogical/backup_config.json'
            if not os.path.exists(config_file):
                # Try alternate location
                config_file = '/etc/codexpanel/backup_config.json'
