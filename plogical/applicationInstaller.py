...
class ApplicationInstaller(multi.Thread):
    ...
    def UpgradeCP(self):
        command = f'/usr/local/codexpanel/bin/python /usr/local/CodexCP/plogical/upgrade.py "SoftUpgrade,{self.data["branchSelect"]}"'
    ...
    def installJoomla(self):
        ...
        try:
            ...
            try:
                os.remove('/usr/local/CodexCP/joomla.zip')
                os.remove('/usr/local/CodexCP/lscache_plugin.zip')
                os.remove('/usr/local/CodexCP/pkg_lscache.xml')
                os.remove('/usr/local/CodexCP/pkg_script.php')
            except:
                pass
            ...
            command = 'wget https://raw.githubusercontent.com/litespeedtech/lscache-joomla/master/package/lscache-1.3.1.zip -O /usr/local/CodexCP/joomla.zip'
            ProcessUtilities.executioner(command)

            command = 'unzip -o /usr/local/CodexCP/joomla.zip -d /usr/local/CodexCP/'
            ProcessUtilities.executioner(command)

            command = '%s extension:installfile %s --www %s /usr/local/CodexCP/lscache_plugin.zip' % (
            joomlaPath, dbUser, finalPath)
            ProcessUtilities.executioner(command)

            command = '%s extension:installfile %s --www %s /usr/local/CodexCP/com_lscache.zip' % (
            joomlaPath, dbUser, finalPath)
            ProcessUtilities.executioner(command)
    ...
    def DeployWordPress(self):
        try:
            ...
            self.extraArgs['tempStatusPath'] = "/home/codexpanel/" + str(randint(1000, 9999))
            ...
            if cronData.find('WPAutoUpdates.py') == -1:
                ...
                writeToFile.write(
                    '0 12 * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/WPAutoUpdates.py\n')
                writeToFile.close()
    ...
    def _initializeStagingCreation(self, wpobj):
        ...
        tempPath = '%s/%s' % ('/usr/local/CodexCP/tmp', str(randint(1000, 9999)))
    ...
    def DeploytoProduction(self):
        ...
        try:
            ...
            ### Create secure folder
            ACLManager.CreateSecureDir()
            self.tempPath = '%s/%s' % ('/usr/local/CodexCP/tmp', str(randint(1000, 9999)))
    ...
    def WPCreateBackup(self):
        try:
            ...
            if Backuptype == "1":
                ...
                self.tempPath = '%s/%s' % ('/usr/local/CodexCP/tmp', RandomPath)
                ...
                # command = "sudo -u %s touch /home/codexpanel/config.json" % (VHuser)
                # ProcessUtilities.executioner(command)
                ###### write into config

                json_object = json.dumps(config, indent=4)
                configPath = "/home/codexpanel/" + str(randint(1000, 9999))
    ...
    def RestoreWPbackupNow(self):
        try:
            ...
            if BackupDestination == 'SFTP':
                ...
                # 1. Generate SSH keys on the remote server with the name 'codexpanelbackup'
                ssh_keygen_command = "ssh-keygen -t rsa -b 2048 -f ~/.ssh/codexpanelbackup -q -N ''"
                ...
                remote_private_key = "~/.ssh/codexpanelbackup"
                remote_public_key = "~/.ssh/codexpanelbackup.pub"
                ...
                loaclpath = "/home/codexpanel/%s.tar.gz" % BackUpFileName
                ...
                #     ProcessUtilities.executioner(command)
                    ...
                    #         unzippathdb = "%s/ab/usr/local/CodexCP/tmp/%s/%s" % (
                    ...
                    #                     raise BaseException(stdout)
                    #
                    #                 unzippath = "%s/ab/usr/local/CodexCP/tmp/%s/public_html/" % (
                    ...
                    #         if oldhome == "0":
                    #             unzippath = "%s/ab/usr/local/CodexCP/tmp/%s/public_html/" % (self.tempPath, oldtemppath)
                    #         else:
                    #             unzippath = "%s/ab/usr/local/CodexCP/tmp/%s/public_html/public_html/" % (
                    #                 self.tempPath, oldtemppath)
                    #
                    #         command = "sudo -u %s cp -R %s* %s" % (VHuser, unzippath, newWPpath)
                    #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
                    ...
                    #
                    #                 # dump Mysql file in unzippath path
                    #                 unzippath2 = "%s/ab/usr/local/CodexCP/tmp/%s/%s" % (
                    #                     self.tempPath, oldtemppath, DumpFileName)
            ...
            ###S#Backups
                ...
                FinalZipPath = "/home/codexpanel/%s.tar.gz" % (uploadfilename)
                try:
                ...
                #
                #         # dump Mysql file in unzippath path
                #         unzippathdb = "%s/ab/usr/local/CodexCP/tmp/%s/%s" % (self.tempPath, oldtemppath, DumpFileName)
                #         # command = "mysql -u root %s < %s" % (Finaldbname, unzippathdb)
                #         command = 'sudo -u %s %s -d error_reporting=0 /usr/bin/wp --allow-root --skip-plugins --skip-themes --path=%s --quiet db import %s' % (
                #             VHuser, FinalPHPPath, newWPpath, unzippathdb)
                #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
                ...
                #
                #         # dump Mysql file in unzippath path
                #         unzippath2 = "%s/ab/usr/local/CodexCP/tmp/%s/%s" % (self.tempPath, oldtemppath, DumpFileName)
                #         # command = "mysql -u root %s < %s" % (Finaldbname, unzippath2)
                #         command = 'sudo -u %s %s -d error_reporting=0 /usr/bin/wp --allow-root --skip-plugins --skip-themes --path=%s --quiet db import %s' % (
                #             VHuser, FinalPHPPath, newWPpath, unzippath2)
                #         result, stdout = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
            ...
            ####Check if backup type is Both web and DB
                ...
                ############## New Site
                    ...
                    execPath = "/usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/virtualHostUtilities.py"
    ...
    def StartOCRestore(self):
        try:
            ...
            # Ensure /home/codexpanel directory exists with proper permissions
            if not os.path.exists('/home/codexpanel'):
                command = 'mkdir -p /home/codexpanel'
                ProcessUtilities.executioner(command)

            # Set proper permissions to allow application to write to the directory
            command = 'chmod 755 /home/codexpanel'
            ProcessUtilities.executioner(command)

            sftp = ssh.open_sftp()

            logging.statusWriter(self.tempStatusPath, 'Downloading Backups...,15')
            loaclpath = f'/home/codexpanel/{backupfile}'
            ...
            if errorRet:
                ...
                try:
                    sftp.get(f'cpbackups/{folder}/{backupfile}', f'/home/codexpanel/{backupfile}',
                             callback=self.UpdateDownloadStatus)
            ...
            # sftp = ssh.open_sftp()
            #
            # sftp.get(f'cpbackups/{folder}/{backupfile}', f'/home/codexpanel/{backupfile}', callback=self.UpdateDownloadStatus)
            ...
            command = f'mv /home/codexpanel/{backupfile} /home/backup/{backupfile}'
            ProcessUtilities.executioner(command)
...
def main():
    parser = argparse.ArgumentParser(description='CodexPanel Application Installer')
