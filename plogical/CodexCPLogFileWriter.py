...
class CodexCPLogFileWriter:
    fileName = "/home/codexpanel/error-logs.txt"
    ...
    def AddFromHeader(sender, message):
        try:
            ...
            CodexCPLogFileWriter.writeToFile(str(msg) + ' [AddFromHeader]')

    @staticmethod
    def SendEmail(sender, receivers, message, subject=None, type=None):
        try:
            smtpPath = '/home/codexpanel/smtpDetails'
            ...
            if os.path.exists(smtpPath):
                ...
                message = CodexCPLogFileWriter.AddFromHeader(sender, message)
    ...
    def writeToFile(message, email=None):
        try:
            ...
            file = open(CodexCPLogFileWriter.fileName,'a')
            ...
            emailPath = '/usr/local/CodexCP/emailDebug'

            try:
                if os.path.exists(emailPath):
                    SUBJECT = "codexpanel log reporting"
...
""" % (
                    ...
                    if email == None or email == 1:
                        CodexCPLogFileWriter.SendEmail(sender, TO, message)
            except BaseException as msg:
                file = open(CodexCPLogFileWriter.fileName, 'a')
                file.writelines("[" + time.strftime(
    ...
    def statusWriter(tempStatusPath, mesg, append = None):
        try:
            ...
            CodexCPLogFileWriter.writeToFile(str(msg) + ' [statusWriter]')
