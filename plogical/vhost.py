...
class vhost:
    ...
    def addUser(virtualHostUser, path):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"{str(msg)} [addingUsers]")
    ...
    def createDirectories(path, virtualHostUser, pathHTML, pathLogs, confPath, completePathToConfigFile):
        try:
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [createDirectories]]")
                #return [0, "[45 Not able to directories for virtual host [createDirectories]]"]
    ...
    def finalizeVhostCreation(virtualHostName, virtualHostUser):
        try:
            ...
            shutil.copy("/usr/local/CodexCP/index.html", f"/home/{virtualHostName}/public_html/index.html")
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [finalizeVhostCreation]")
    ...
    def createConfigInMainVirtualHostFile(virtualHostName):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + "223 [IO Error with main config file [createConfigInMainVirtualHostFile]]")
    ...
    def deleteVirtualHostConfigurations(virtualHostName):
        if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
            try:
                    ...
                    try:
                        ...
                        for wpSite in wpSites:
                            ...
                            for staging in stagingSites:
                                staging.delete()
                                logging.CodexCPLogFileWriter.writeToFile(f"Deleted staging site record: {staging.id}")
                            # Delete the WP site itself
                            wpSite.delete()
                            logging.CodexCPLogFileWriter.writeToFile(f"Deleted WP site: {wpSite.id}")
                ...
                gitPath = '/home/codexpanel/git/%s' % (virtualHostName)
                ...
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile(f"Warning: Failed to remove resource limits for user {externalApp}: {str(e)}")
                ...
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [Not able to remove virtual host configuration from main configuration file.]")
                return 0
            return 1
        else:
            try:
    ...
    def changePHP(vhFile, phpVersion):
        ...
        logging.CodexCPLogFileWriter.writeToFile(f"PHP version before making sure its available or not: {phpVersion} and vhFile: {vhFile}")
            ...
            try:
                if ApacheVhost.changePHP(phpVersion, vhFile) == 0:
                    ...
                    if not os.path.exists("/usr/local/lsws/lsphp" + str(php) + "/bin/lsphp"):
                        print(0, 'This PHP version is not available on your codexpanel.')
                        return [0, "[This PHP version is not available on your codexpanel. [changePHP]"]

                    writeDataToFile = open(vhFile, "w")

                    path = "  path                    /usr/local/lsws/lsphp" + str(php) + "/bin/lsphp\n"

                    logging.CodexCPLogFileWriter.writeToFile(f"PHP String to be written {path}")
                    ...
                    logging.CodexCPLogFileWriter.writeToFile('apache vhost 1')
    ...
    def addRewriteRules(virtualHostName, fileName=None):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [IO Error with per host config file [changePHP]]")
            return 0
    ...
    def findDomainBW(domainName, totalAllowed):
        try:
            ...
            bwmeta = "/home/codexpanel/%s.bwmeta" % (domainName)
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [findDomainBW]")
            print("0,0")
            return 0, 0
    ...
    def checkIfSSLAliasExists(data, aliasDomain):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "  [checkIfSSLAliasExists]")
            return 1
    ...
    def createAliasSSLMap(confPath, masterDomain, aliasDomain):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "  [createAliasSSLMap]")
    ...
    def finalizeDomainCreation(virtualHostUser, path):
        try:
            ...
            FullPath = '%s/%s' % ('/usr/local/CodexCP/tmp', RanddomFileName)

            FNULL = open(os.devnull, 'w')

            #shutil.copy("/usr/local/CodexCP/index.html", path + "/index.html")

            shutil.copy("/usr/local/CodexCP/index.html", FullPath)
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [finalizeDomainCreation]")
    ...
    def createDirectoryForDomain(masterDomain, domain, phpVersion, path, administratorEmail, virtualHostUser,
        ...
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [createDirectoryForDomain]]")
