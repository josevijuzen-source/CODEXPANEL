...
class IncScheduler(multi.Thread):
    logPath = '/home/codexpanel/incbackuplogs'
    gitFolder = '/home/codexpanel/git'
            ...
            for website in os.listdir(IncScheduler.gitFolder):
                ...
                for file in os.listdir(finalPathInside):
                    ...
                    try:
                        ...
                        data['commitMessage'] = 'Auto commit by CodexPanel %s cron on %s' % (
    ...
    def checkDiskUsage():
        ...
        try:
            ...
            message = '%s - Disk Usage Warning - codexpanel' % (ACLManager.fetchIP())
                ...
                if jobConfig[IncScheduler.frequency] == type:
                    ...
                    for site in websites:
                        ...
                        if retValues[0] == 0:
                            ...
                            adminEmailPath = '/home/codexpanel/adminEmail'
...
""" % (sender, ", ".join(TO), SUBJECT, domain, currentTime)
                ...
                if jobConfig[IncScheduler.frequency] == type:
                    ...
                    #     'port'] + " -i /root/.ssh/codexpanel " + destinationConfig['username'] + "@" + \
                    #           destinationConfig[
...
""" % (sender, ", ".join(TO), SUBJECT, domain, currentTime)
                            ...
                            command = "scp -o StrictHostKeyChecking=no -P " + destinationConfig[
                                'port'] + " -i /root/.ssh/codexpanel " + backupPath + " " + destinationConfig[
                            ...
                            try:
                                ...
                                if ssh_commands_supported:
                                    ...
                                    try:
                                        ...
                                        logging.CodexCPLogFileWriter.writeToFile(f'SSH find command failed: {str(find_err)}, falling back to SFTP [IncScheduler.startNormalBackups]')
                                ...
                                if not backup_found:
                                    try:
                                        ...
                                        try:
                                            ...
                                            logging.CodexCPLogFileWriter.writeToFile(f'Backup directory not found: {finalPath} [IncScheduler.startNormalBackups]')
                                            files = []
                                        ...
                                        for f in files:
                                            ...
                                            if check_domain in f and f.endswith('.tar.gz'):
                                                ...
                                                try:
                                                    ...
                                                    logging.CodexCPLogFileWriter.writeToFile(f'Failed to stat file {file_path}: {str(stat_err)} [IncScheduler.startNormalBackups]')

                                        sftp.close()
                                    except Exception as sftp_err:
                                        logging.CodexCPLogFileWriter.writeToFile(f'SFTP verification failed: {str(sftp_err)} [IncScheduler.startNormalBackups]')

                                # Only send notification if backup was NOT found (backup failed)
                                if not backup_found:
                                    logging.CodexCPLogFileWriter.writeToFile(f'Backup NOT found for {check_domain}, sending failure notification [IncScheduler.startNormalBackups]')
                                    ...
                                    try:
                                        ...
                                        if response.status_code == 200:
                                            ...
                                            if response_data.get('status') == 1:
                                                logging.CodexCPLogFileWriter.writeToFile(f'Failure notification sent successfully for {check_domain} [IncScheduler.startNormalBackups]')
                                            else:
                                                logging.CodexCPLogFileWriter.writeToFile(f'Failure notification API returned error for {check_domain}: {response_data.get("error_message")} [IncScheduler.startNormalBackups]')
                                        else:
                                            logging.CodexCPLogFileWriter.writeToFile(f'Failure notification API returned HTTP {response.status_code} for {check_domain} [IncScheduler.startNormalBackups]')
                                    except requests.exceptions.RequestException as e:
                                        logging.CodexCPLogFileWriter.writeToFile(f'Failed to send backup failure notification for {check_domain}: {str(e)} [IncScheduler.startNormalBackups]')
                                else:
                                    logging.CodexCPLogFileWriter.writeToFile(f'Backup verified successful for {check_domain}, no notification needed [IncScheduler.startNormalBackups]')

                            except Exception as msg:
                                logging.CodexCPLogFileWriter.writeToFile(f'Error checking backup status for site: {str(msg)} [IncScheduler.startNormalBackups]')
    ...
    def forceRunAWSBackup(planName):
        try:
            ...
            for items in plan.websitesinplan_set.all():
                ...
                extraArgs['path'] = '/home/codexpanel/backups/%s/backup-' % (
    ...
    def v2Backups(function):
        try:
            ...
            import json
            import time
            if os.path.exists('/home/codexpanel/v2backups'):
                for website in Websites.objects.all():
                    finalConfigPath = f'/home/codexpanel/v2backups/{website.domain}'
...
""" % (sender, ", ".join(TO), SUBJECT, website.domain, time.strftime("%m.%d.%Y_%H-%M-%S"))
    ...
    def CheckHostName():
        try:
            ...
            virtualHostUtilities.OnBoardingHostName(CurrentHostName, '/home/codexpanel/onboarding_temp_path', skipRDNSCheck)
        except BaseException as msg:
