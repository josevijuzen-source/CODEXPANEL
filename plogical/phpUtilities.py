...
class phpUtilities:
    ...
    installLogPath = "/home/codexpanel/phpExtensionRequestLog"
    ...
    def installPHPExtension(extension,extensions):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[installPHPExtension]")
    ...
    def unInstallPHPExtension(extension,extensions):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[unInstallPHPExtension]")
    ...
    def initiateInstall(extension):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [initiateInstall]")
    ...
    def savePHPConfigBasic(phpVers,allow_url_fopen,display_errors,file_uploads,allow_url_include,memory_limit,max_execution_time,upload_max_filesize,max_input_time,post_max_size, apache):
        try:
            ...
            if missing_directives:
                writeToFile.writelines("\n; Added by CodexPanel PHP Config Manager\n")
    ...
    def GetPHPVersionFromFile(vhFile, domainName=None):
        ...
        if os.path.exists('/usr/local/CodexCP/debug'):
            logging.CodexCPLogFileWriter.writeToFile(f'Apache conf file in GetPHPVersionFromFile {finalConfPath}')
        ...
        if os.path.exists('/usr/local/CodexCP/debug'):
            logging.CodexCPLogFileWriter.writeToFile(f'VHFile in GetPHPVersion {vhFile}')
