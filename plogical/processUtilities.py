...
class ProcessUtilities(multi.Thread):
    debugPath = '/usr/local/CodexCP/debug'
    ...
    def sendCommand(command, user=None, dir=None, retries=3):
        ...
        try:
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                if len(data) == 0:
                    logging.writeToFile(f"    â””â”€ Empty response from lscpd")
                else:
                    try:
                        exit_char = data[-1]
                        # Log raw data for debugging
                        logging.writeToFile(f"    â””â”€ Response length: {len(data)}, last char: {repr(exit_char)}")
                        ...
                        logging.writeToFile(f"    â””â”€ {clean_command} [Exit Code: {exit_code}]")
                    except Exception as e:
                        logging.writeToFile(f"    â””â”€ Failed to log exit code: {str(e)}")
        ...
        try:
            if getpass.getuser() == 'root':
                ...
                if retRequired:
                    ...
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.writeToFile(f"    â””â”€ [Exit Code: {exit_code}]")
                    if exit_code == 0:
                        return 1, output
                    else:
                        return 0, output
    ...
    def BuildCommand(path, functionName, parameters):
        ...
        execPath = "/usr/local/CodexCP/bin/python %s %s " % (path, functionName)
