...
class mailUtilities:
    ...
    installLogPath = "/home/codexpanel/openDKIMInstallLog"
    spamassassinInstallLogPath = "/home/codexpanel/spamassassinInstallLogPath"
    RspamdInstallLogPath = "/home/codexpanel/RspamdInstallLogPath"
    RspamdUnInstallLogPath = "/home/codexpanel/RspamdUnInstallLogPath"
    codexpanelHome = "/home/codexpanel"
    mailScannerInstallLogPath = "/home/codexpanel/mailScannerInstallLogPath"
    ...
    def SendEmail(sender, receivers, message):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg))
    @staticmethod
    def AfterEffects(domain):
        path = "/usr/local/CodexCP/install/rainloop/codexpanel.net.ini"

        if not os.path.exists("/usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/domains/"):
            os.makedirs("/usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/domains/")

        finalPath = "/usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/domains/" + domain + ".ini"
        finalPathJson = "/usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/domains/" + domain + ".json"
...
""")
    ...
    def createEmailAccount(domain, userName, password, restore = None):
        try:
            ...
            execPath = "/usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/mailUtilities.py"
            ...
            #if not os.path.exists('/usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'):
    ...
    def setupDKIM(virtualHostName):
        try:
            ...
            if not os.path.exists(keyTable):
                ...
                writeToFile.write("##### CodexPanel Generated File - Do not edit if you don't know what you are doing.\n")
                writeToFile.close()
...
"""
                ...
                if not os.path.exists(postfixFilePath):
                    logging.CodexCPLogFileWriter.writeToFile(f"configureOpenDKIM: {postfixFilePath} not found, skipping postfix DKIM configuration")
...
"""
                ...
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [configureOpenDKIM]")
                print("0," + str(msg))
                return
            except BaseException as msg:
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [configureOpenDKIM]")
    ...
    def checkHome():
        try:
            try:
                ...
                if getpass.getuser() == 'root':
                    if not os.path.exists(mailUtilities.codexpanelHome):
                        command = "mkdir " + mailUtilities.codexpanelHome
                        subprocess.call(shlex.split(command), stdout=FNULL)

                    command = "sudo chown -R codexpanel:codexpanel " + mailUtilities.codexpanelHome
                    subprocess.call(shlex.split(command), stdout=FNULL)
                else:
                    if not os.path.exists(mailUtilities.codexpanelHome):
                        command = "mkdir " + mailUtilities.codexpanelHome
                    ...
                    command = "chown -R codexpanel:codexpanel " + mailUtilities.codexpanelHome
                    ProcessUtilities.executioner(command)
            except:
                FNULL = open(os.devnull, 'w')
                command = "chown -R codexpanel:codexpanel " + mailUtilities.codexpanelHome
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [checkHome]")
    ...
    def installOpenDKIM(install, openDKIMINstall):
        try:
            ...
            if res == 1:
                ...
                writeToFile.close()
                logging.CodexCPLogFileWriter.writeToFile("[Could not Install OpenDKIM.]")
                return 0
            else:
                writeToFile = open(mailUtilities.installLogPath, 'a')
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[installOpenDKIM]")
    ...
    def restartServices():
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [restartServices]")
    ...
    def installSpamAssassin(install, SpamAssassin):
        try:
            ...
            if res == 1:
                ...
                writeToFile.writelines("Can not be installed.[404]\n")
                writeToFile.close()
                logging.CodexCPLogFileWriter.writeToFile("[Could not Install SpamAssassin.]")
                return 0
            else:
                writeToFile = open(mailUtilities.spamassassinInstallLogPath, 'a')
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[installSpamAssassin]")
...
"""
            ...
            if res == 1:
                ...
                writeToFile.writelines("Can not be installed.[404]\n")
                writeToFile.close()
                logging.CodexCPLogFileWriter.writeToFile("[Could not Install Rspamd.]")
                return 0
            else:
...
"""
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[installRspamd]")

    @staticmethod
    def uninstallRspamd(install, rspamd):
        from manageServices.serviceManager import ServiceManager
        try:
            logging.CodexCPLogFileWriter.writeToFile( "start................[uninstallRspamd]")
            ...
            writeToFile.writelines("Can not be installed.[404]\n")
            writeToFile.close()
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[uninstallRspamd]")
    ...
    def changeRspamdConfig(install, changeRspamdConfig):
        try:
            ...
            tempfilepath = "/home/codexpanel/tempfilerspamdconfigs"
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[changeRspamdConfig]")
    ...
    def changePostfixConfig(install , changePostfixConfig):
        try:
            tempfilepath = "/home/codexpanel/tempfilepostfixconfigs"
            file = open(tempfilepath, "r")
            jsondata1 = file.read()
            jsondata = json.loads(jsondata1)
            file.close()
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[changePostfixConfig]")
    ...
    def changeRedisxConfig(install, changeRedisxConfig):
        try:
            tempfilepath = "/home/codexpanel/saveRedisConfigurations"
            file = open(tempfilepath, "r")
            jsondata1 = file.read()
            jsondata = json.loads(jsondata1)
            file.close()
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[changeRedisxConfig]")
    ...
    def changeclamavConfig(install, changeclamavConfig):
        try:
            tempfilepath = "/home/codexpanel/saveclamavConfigurations"
            file = open(tempfilepath, "r")
            jsondata1 = file.read()
            jsondata = json.loads(jsondata1)
            file.close()
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[changeclamavConfig]")
    ...
    def installMailScanner(install, SpamAssassin):
        try:
            ...
            if mailUtilities.checkIfSpamAssassinInstalled():
                ...
                command = 'chmod +x /usr/local/CodexCP/CPScripts/mailscannerinstaller.sh'
                ProcessUtilities.executioner(command)


                command = '/usr/local/CodexCP/CPScripts/mailscannerinstaller.sh'
                ...
                if res == 1:
                    ...
                    writeToFile.writelines("Can not be installed.[404]\n")
                    writeToFile.close()
                    logging.CodexCPLogFileWriter.writeToFile("[Could not Install MailScanner.]")
                    return 0
                else:
                    writeToFile = open(mailUtilities.mailScannerInstallLogPath, 'a')
                ...
                writeToFile = open(mailUtilities.mailScannerInstallLogPath, 'a')
                writeToFile.writelines("Please install SpamAssassin from CodexPanel before installing MailScanner.[404]\n")
                writeToFile.close()
    ...
    def configureSpamAssassin():
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [configureSpamAssassin]")
            print("0," + str(msg))
            return
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [configureSpamAssassin]")
        ...
        try:
            ...
            if install == '1':
                if not os.path.exists('/etc/systemd/system/cpecs.service'):
                    shutil.copy("/usr/local/CodexCP/postfixSenderPolicy/cpecs.service", "/etc/systemd/system/cpecs.service")
    ...
    def checkIfMailScannerInstalled():
        try:
            ...
            path = "/usr/local/CodexCP/public/mailwatch"
    ...
    def reverse_dns_lookup(ip_address):
        try:
            ...
            fetchURLs = requests.get('https://codexpanel.net/dnsServers.txt')
            ...
            if fetchURLs.status_code == 200:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile(f'DNS urls {urls}.')
                ...
                for url in urls:
                    try:
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            logging.CodexCPLogFileWriter.writeToFile(f'url to call {ip_address} is {url}')

                        if response.status_code == 200:
                            data = response.json()

                            if os.path.exists(ProcessUtilities.debugPath):
                                logging.CodexCPLogFileWriter.writeToFile(f'response from dns system {str(data)}')
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile(f'rDNS result of {ip_address} is {str(results)}')

                ###

                return results
        except BaseException as e:
            logging.CodexCPLogFileWriter.writeToFile(f'Error in fetch rDNS {str(msg)}')
    ...
    def configureRelayHost(smtpHost, smtpPort, smtpUser, smtpPassword):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [configureRelayHost]')
            print('0,%s' % str(msg))
    ...
    def removeRelayHost():
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [removeRelayHost]')
            print('0,%s' % str(msg))
...
class MailServerManagerUtils(multi.Thread):
            ...
            try:
                ...
                for items in postFixLines:
                    if items.find('myhostname') > -1 and items[0] != '#':
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            logging.CodexCPLogFileWriter.writeToFile(f'Mail server SSL is issued with value: {self.mailHostName}')

                        self.MailSSL = 1
            except BaseException as msg:
                self.MailSSL = 0
                logging.CodexCPLogFileWriter.writeToFile('%s. [checkIfMailServerSSLIssued:864]' % (str(msg)))
    ...
    def RunServerLevelEmailChecks(self):
        try:
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Completed [200].')
    ...
    def install_postfix_dovecot(self):
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Re-installing postfix..,10')
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Re-installing Dovecot..,15')
    ...
    def setup_email_Passwords(self, mysqlPassword):
        try:
            ...
            mysql_virtual_domains = "/usr/local/CodexCP/install/email-configs-one/mysql-virtual_domains.cf"
            mysql_virtual_forwardings = "/usr/local/CodexCP/install/email-configs-one/mysql-virtual_forwardings.cf"
            mysql_virtual_mailboxes = "/usr/local/CodexCP/install/email-configs-one/mysql-virtual_mailboxes.cf"
            mysql_virtual_email2email = "/usr/local/CodexCP/install/email-configs-one/mysql-virtual_email2email.cf"
            dovecotmysql = "/usr/local/CodexCP/install/email-configs-one/dovecot-sql.conf.ext"
    ...
    def setup_postfix_dovecot_config(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
                self.centos_lib_dir_to_ubuntu("/usr/local/CodexCP/install/email-configs-one/master.cf", "/usr/libexec/",
                                              "/usr/lib/")
                self.centos_lib_dir_to_ubuntu("/usr/local/CodexCP/install/email-configs-one/main.cf",
                                              "/usr/libexec/postfix",
            ...
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/mysql-virtual_domains.cf",
                        "/etc/postfix/mysql-virtual_domains.cf")
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/mysql-virtual_forwardings.cf",
                        "/etc/postfix/mysql-virtual_forwardings.cf")
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/mysql-virtual_mailboxes.cf",
                        "/etc/postfix/mysql-virtual_mailboxes.cf")
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/mysql-virtual_email2email.cf",
                        "/etc/postfix/mysql-virtual_email2email.cf")
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/main.cf", main)
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/master.cf", master)
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/dovecot.conf", dovecot)
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/dovecot-sql.conf.ext", dovecotmysql)
    ...
    def ResetEmailConfigurations(self):
        try:
            ...
            try:
                ...
                if os.path.exists(ProcessUtilities.debugPath):
                    logging.CodexCPLogFileWriter.writeToFile('%s. [setupConnection:75]' % (str(msg)))
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Resetting configurations..,40')
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Configurations reset..,70')
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Fixing permissions..,90')

            self.fixcodexpanelPermissions()

            command = '/usr/local/CodexCP/bin/python /usr/local/CodexCP/dns/dnsManager.py ResetDNSConfigurations --tempStatusPath /home/codexpanel/dnscheck'
            ProcessUtilities.executioner(command)

            command = 'touch /home/codexpanel/postfix'
            ProcessUtilities.executioner(command)
