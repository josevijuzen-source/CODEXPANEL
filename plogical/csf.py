...
class CSF(multi.Thread):
    installLogPath = "/home/codexpanel/csfInstallLog"
    ...
    def run(self):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [CSF.run]')
    ...
    def installCSF():
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(CSF.installLogPath, 'Downloading CSF..\n', 1)
            ...
            logging.CodexCPLogFileWriter.statusWriter(CSF.installLogPath, 'Extracting CSF..\n', 1)
            ...
            logging.CodexCPLogFileWriter.statusWriter(CSF.installLogPath, 'Installing CSF..\n', 1)

            os.chdir('csf')

            ### manually update csf views.py because it does not load CodexPanel properly in default configurations
...
def configservercsfiframe(request):
    ...
    try:
        ...
        tmp.write(qs)
        tmp.close()
        command = "/usr/local/csf/bin/codexpanel.pl '" + tmp.name + "'"
...
'''
            ...
            WriteToFile = open('codexpanel/configservercsf/views.py', 'w')
            WriteToFile.write(content)
            WriteToFile.close()

            ### now update content of signals.py
            WriteToFile = open('codexpanel/configservercsf/signals.py', 'w')
            WriteToFile.close()
...
'''
            ...
            WriteToFile = open('codexpanel/configservercsf/apps.py', 'w')
            WriteToFile.write(content)
            WriteToFile.close()
...
'''
            ...
            WriteToFile = open('codexpanel/configservercsf/urls.py', 'w')
            WriteToFile.write(content)
            WriteToFile.close()
...
if [ ! -e "/etc/csf/csf.conf" ]; then
	cp -avf csf.codexpanel.conf /etc/csf/csf.conf
fi
...
if [ ! -e "/etc/csf/csf.allow" ]; then
	cp -avf csf.codexpanel.allow /etc/csf/csf.allow
fi
...
if [ ! -e "/etc/csf/csf.ignore" ]; then
	cp -avf csf.codexpanel.ignore /etc/csf/csf.ignore
fi
if [ ! -e "/etc/csf/csf.pignore" ]; then
	cp -avf csf.codexpanel.pignore /etc/csf/csf.pignore
fi
...
cp -avf uninstall.codexpanel.sh /usr/local/csf/bin/uninstall.sh
...
chmod -v 700 auto.codexpanel.pl
./auto.codexpanel.pl $OLDVERSION
...
mkdir -vp /usr/local/CodexCP/public/static/configservercsf/
cp -avf csf/* /usr/local/CodexCP/public/static/configservercsf/
cp -avf csf/* codexpanel/configservercsf/static/configservercsf/
chmod 755 /usr/local/CodexCP/public/static/configservercsf/

cp codexpanel/codexpanel.pl /usr/local/csf/bin/
chmod 700 /usr/local/csf/bin/codexpanel.pl
cp -avf codexpanel/configservercsf /usr/local/CodexCP/

mkdir /home/codexpanel/plugins
touch /home/codexpanel/plugins/configservercsf

if ! cat /usr/local/CodexCP/CodexCP/settings.py | grep -q configservercsf; then
    sed -i "/pluginHolder/ i \ \ \ \ 'configservercsf'," /usr/local/CodexCP/CodexCP/settings.py
fi
if ! cat /usr/local/CodexCP/CodexCP/urls.py | grep -q configservercsf; then
    sed -i "/pluginHolder/ i \ \ \ \ path('configservercsf/',include('configservercsf.urls'))," /usr/local/CodexCP/CodexCP/urls.py
fi
#if ! cat /usr/local/CodexCP/baseTemplate/templates/baseTemplate/index.html | grep -q configservercsf; then
#    sed -i "/url 'csf'/ i <li><a href='/configservercsf/' title='ConfigServer Security and Firewall'><span>ConfigServer Security \&amp; Firewall</span></a></li>" /usr/local/CodexCP/baseTemplate/templates/baseTemplate/index.html
#fi
#if ! cat /usr/local/CodexCP/baseTemplate/templates/baseTemplate/index.html | grep -q configserver; then
#    sed -i "/trans 'Plugins'/ i \{\% include \"/usr/local/CodexCP/configservercsf/templates/configservercsf/menu.html\" \%\}" /usr/local/CodexCP/baseTemplate/templates/baseTemplate/index.html
#fi
...
'''
            ...
            WriteToFile = open('install.codexpanel.sh', 'w')
            WriteToFile.write(content)
            WriteToFile.close()
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                ...
                logging.CodexCPLogFileWriter.statusWriter(CSF.installLogPath,
            ...
            for items in data:
                ...
                #    writeToConf.writelines('UI_USER = "CodexPanel"\n')
            ...
            logging.CodexCPLogFileWriter.statusWriter(CSF.installLogPath, 'CSF successfully Installed.[200]\n', 1)
            ...
            writeToFile.close()
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[installCSF]")
    ...
    def removeCSF(self):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[removeCSF]")
    ...
    def fetchCSFSettings():
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [fetchCSFSettings]")
    ...
    def changeStatus(controller, status):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[changeStatus]")
            print('0', str(msg))
    ...
    def modifyPorts(protocol, portsPath):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[modifyPorts]")
            print('0', str(msg))
    ...
    def allowIP(ipAddress):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[allowIP]")
    ...
    def blockIP(ipAddress):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[blockIP]")
    ...
    def checkIP(ipAddress):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[checkIP]")
