...
def update_all_config_files_with_password(new_password):
    """
    Update all configuration files that use the CodexPanel database password.
    This includes FTP, PowerDNS, Postfix, Dovecot configurations.
    """
    config_updates = [
        # Django settings
        {
            'path': '/usr/local/CodexCP/CodexCP/settings.py',
            'updates': [
                (r"('codexpanel'[^}]+?'PASSWORD':\s*')[^']+'", r"\1%s'" % new_password)
            ]
        },
        # FTP configurations
        {
...
def update_dovecot_connect_string(connect_line, new_password):
    ...
    Format: connect = host=localhost dbname=codexpanel user=codexpanel password=oldpass
    """
...
def restart_affected_services():
    """
    Restart services that use the CodexPanel database password.
    """
    services_to_restart = [
...
# Try to import settings, but handle case where CodexCP directory is damaged
try:
    from CodexCP import settings
except ImportError:
    print("WARNING: Cannot import CodexCP settings. Attempting recovery...")
    
    def recover_database_credentials():
        """Attempt to recover or reset database credentials"""
        
        # First, ensure we have root MySQL password
        if not os.path.exists('/etc/codexpanel/mysqlPassword'):
            print("FATAL: Cannot find MySQL root password file at /etc/codexpanel/mysqlPassword")
            print("Manual intervention required.")
            sys.exit(1)
        
        root_password = open('/etc/codexpanel/mysqlPassword', 'r').read().strip()
        codexpanel_password = None
        
        # Try to read existing settings.py to get CodexPanel password
        settings_path = '/usr/local/CodexCP/CodexCP/settings.py'
        if os.path.exists(settings_path):
            try:
                ...
                import re
                # Extract CodexPanel database password
                db_pattern = r"'default':[^}]*'USER':\s*'codexpanel'[^}]*'PASSWORD':\s*'([^']+)'"
                match = re.search(db_pattern, settings_content, re.DOTALL)
                
                if match:
                    codexpanel_password = match.group(1)
                    print("Found existing CodexPanel password in settings.py")
                    
                    # Test if this password actually works
                    try:
                        test_conn = mysql.connect(host='localhost', user='codexpanel', 
                                                passwd=codexpanel_password, db='codexpanel')
                        test_conn.close()
                        print("Verified CodexPanel database credentials are valid")
                    except:
        ...
        if codexpanel_password is None:
            print("Resetting CodexPanel database user password...")
            
            # Check if we're on Ubuntu or CentOS
            # On Ubuntu, CodexPanel uses root password; on CentOS, it uses a separate password
            if os.path.exists('/etc/lsb-release'):
                # Ubuntu - use root password
                codexpanel_password = root_password
                reset_to_root = True
            else:
                # CentOS/others - generate new password
                chars = string.ascii_letters + string.digits
                codexpanel_password = ''.join(secrets.choice(chars) for _ in range(14))
                reset_to_root = False
            
            try:
                # Connect as root and reset CodexPanel user
                conn = mysql.connect(host='localhost', user='root', passwd=root_password)
                cursor = conn.cursor()
                
                # Check if CodexPanel database exists
                cursor.execute("SHOW DATABASES LIKE 'codexpanel'")
                if not cursor.fetchone():
                    print("Creating CodexPanel database...")
                    cursor.execute("CREATE DATABASE IF NOT EXISTS codexpanel")
                
                # Reset CodexPanel user - drop and recreate to ensure clean state
                cursor.execute("DROP USER IF EXISTS 'codexpanel'@'localhost'")
                cursor.execute("CREATE USER 'codexpanel'@'localhost' IDENTIFIED BY '%s'" % codexpanel_password)
                cursor.execute("GRANT ALL PRIVILEGES ON codexpanel.* TO 'codexpanel'@'localhost'")
                cursor.execute("FLUSH PRIVILEGES")
                
                conn.close()
                
                if reset_to_root:
                    print("Reset CodexPanel user password to match root password (Ubuntu style)")
                else:
                    print("Reset CodexPanel user with new generated password (CentOS style)")
                
                # Update all configuration files with the new password
                print("Updating all service configuration files with new password...")
                update_all_config_files_with_password(codexpanel_password)
                ...
                temp_pass_file = '/tmp/codexpanel_recovered_password'
                with open(temp_pass_file, 'w') as f:
                    f.write(codexpanel_password)
                os.chmod(temp_pass_file, 0o600)
                print("Saved recovered password to temporary file")
                
            except Exception as e:
                print("Failed to reset CodexPanel database user: %s" % str(e))
                print("Manual intervention required. Please run:")
                print("  mysql -u root -p")
                print("  CREATE DATABASE IF NOT EXISTS codexpanel;")
                print("  GRANT ALL PRIVILEGES ON codexpanel.* TO 'codexpanel'@'localhost' IDENTIFIED BY 'your_password';")
                print("  FLUSH PRIVILEGES;")
                sys.exit(1)
        
        return codexpanel_password, root_password
    
    # Perform recovery
    codexpanel_password, root_password = recover_database_credentials()
...
class Upgrade:
    ...
    LogPathNew = '/home/codexpanel/upgrade_logs'
    SoftUpgrade = 0
    ...
    @staticmethod
    def updateRepoURL():
        command = "sed -i 's|sgp.codexpanel.sh|cdn.codexpanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|lax.codexpanel.sh|cdn.codexpanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|fra.codexpanel.sh|cdn.codexpanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|mirror.codexpanel.net|cdn.codexpanel.sh|g' /etc/yum.repos.d/MariaDB.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|sgp.codexpanel.sh|cdn.codexpanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|lax.codexpanel.sh|cdn.codexpanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|fra.codexpanel.sh|cdn.codexpanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)

        command = "sed -i 's|mirror.codexpanel.net|cdn.codexpanel.sh|g' /etc/yum.repos.d/litespeed.repo"
        Upgrade.executioner(command, command, 0)
    ...
    def dockerUsers():
        ...
        command = 'usermod -aG docker codexpanel'
        Upgrade.executioner(command, 'adduser docker', 0)
    ...
    def fixSudoers():
        try:
            ...
            command = "chsh -s /bin/false codexpanel"
        ...
        try:
            ...
            if os.path.exists('/etc/os-release'):
                with open('/etc/os-release', 'r') as f:
                    ...
                    # Check for version 10.x (AlmaLinux 10, etc.) â€” the el9 binary runs on el10
    ...
    def installCustomOLSBinaries():
        ...
        try:
            ...
            # Platform-specific URLs and checksums (OpenLiteSpeed v2.5.1 â€” all features config-driven, static linking)
            ...
            # Core v2.5.1: HttpReq::getDocRoot NULL-vhost hardening â€” no module can crash the worker on unmatched-Host 4xx responses
            ...
            # rhel9 artifact covers EL9 + EL10 (AlmaLinux 10); ubuntu artifact covers 22.04/24.04 (not <22.04 â€” see detectPlatform)
            BINARY_CONFIGS = {
                'rhel8': {
                    'url': 'https://codexpanel.net/openlitespeed-2.5.1-x86_64-rhel8',
                    'module_url': 'https://codexpanel.net/codexpanel_ols-2.7.5-x86_64-rhel8.so',
                    'modsec_url': 'https://codexpanel.net/mod_security-2.5.1-x86_64-rhel8.so',
                    'sha256': {
                ...
                'rhel9': {
                    'url': 'https://codexpanel.net/openlitespeed-2.5.1-x86_64-rhel9',
                    'module_url': 'https://codexpanel.net/codexpanel_ols-2.7.5-x86_64-rhel9.so',
                    'modsec_url': 'https://codexpanel.net/mod_security-2.5.1-x86_64-rhel9.so',
                    'sha256': {
                ...
                'ubuntu': {
                    'url': 'https://codexpanel.net/openlitespeed-2.5.1-x86_64-ubuntu',
                    'module_url': 'https://codexpanel.net/codexpanel_ols-2.7.5-x86_64-ubuntu.so',
                    'modsec_url': 'https://codexpanel.net/mod_security-2.5.1-x86_64-ubuntu.so',
                    'sha256': {
            ...
            MODULE_PATH = "/usr/local/lsws/modules/codexpanel_ols.so"
            ...
            try:
                ...
                if os.path.exists(MODULE_PATH):
                    shutil.copy2(MODULE_PATH, f"{backup_dir}/codexpanel_ols.so.backup")
            ...
            tmp_module = "/tmp/codexpanel_ols.so"
            ...
            if MODULE_URL:
                ...
                Upgrade.stdOut("Note: No CodexPanel module for this platform", 0)
            ...
            if module_downloaded:
                try:
                    ...
                    Upgrade.stdOut("Installed CodexPanel module", 0)
                except Exception as e:
        ...
        try:
            ...
            if os.path.exists(backup_binary):
                ...
                for backup_name, target in (
                        ("codexpanel_ols.so.backup", module_path or "/usr/local/lsws/modules/codexpanel_ols.so"),
    ...
    def configureCustomModule():
        """Configure CodexPanel module in OpenLiteSpeed config"""
        try:
            Upgrade.stdOut("Configuring CodexPanel module...", 0)
...
"""
    ...
    def download_install_phpmyadmin():
        try:
            ...
            try:
                shutil.rmtree("/usr/local/CodexCP/public/phpmyadmin")
            except:
                pass

            Upgrade.stdOut("Installing phpMyAdmin...", 0)
            
            command = 'wget -q -O /usr/local/CodexCP/public/phpmyadmin.zip https://github.com/josevijuzen-source/CODEXPANEL/raw/main/phpmyadmin.zip'
            Upgrade.executioner_silent(command, 'Download phpMyAdmin')

            command = 'unzip -q /usr/local/CodexCP/public/phpmyadmin.zip -d /usr/local/CodexCP/public/'
            Upgrade.executioner_silent(command, 'Extract phpMyAdmin')

            command = 'mv /usr/local/CodexCP/public/phpMyAdmin-*-all-languages /usr/local/CodexCP/public/phpmyadmin'
            subprocess.call(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

            command = 'rm -f /usr/local/CodexCP/public/phpmyadmin.zip'
            ...
            data = open('/usr/local/CodexCP/public/phpmyadmin/config.sample.inc.php', 'r').readlines()

            writeToFile = open('/usr/local/CodexCP/public/phpmyadmin/config.inc.php', 'w')
...
"""
            ...
            writeToFile.writelines("$cfg['TempDir'] = '/usr/local/CodexCP/public/phpmyadmin/tmp';\n")

            writeToFile.close()

            os.mkdir('/usr/local/CodexCP/public/phpmyadmin/tmp')

            command = 'cp /usr/local/CodexCP/plogical/phpmyadminsignin.php /usr/local/CodexCP/public/phpmyadmin/phpmyadminsignin.php'
            Upgrade.executioner(command, 0)
            ...
            try:
                ...
                command = "sed -i 's|localhost|%s|g' /usr/local/CodexCP/public/phpmyadmin/phpmyadminsignin.php" % (
                    mysqlhost)
                Upgrade.executioner(command, 0)
    ...
    def setupComposer():
        ...
        command = "wget https://codexpanel.sh/composer.sh"
        Upgrade.executioner(command, 0)
    ...
    def downoad_and_install_raindloop():
        try:
            ...
            # if os.path.exists("/usr/local/CodexCP/public/rainloop"):
            #
            #     if os.path.exists("/usr/local/lscp/codexpanel/rainloop/data"):
            #         pass
            #     else:
            #         command = "mv /usr/local/CodexCP/public/rainloop/data /usr/local/lscp/codexpanel/rainloop/data"
            #         Upgrade.executioner(command, 0)
            #
            #         command = "chown -R lscpd:lscpd /usr/local/lscp/codexpanel/rainloop/data"
            #         Upgrade.executioner(command, 0)
            #
            #     iPath = os.listdir('/usr/local/CodexCP/public/rainloop/rainloop/v/')
            #
            #     path = "/usr/local/CodexCP/public/snappymail/snappymail/v/%s/include.php" % (iPath[0])
            #
            #     data = open(path, 'r').readlines()
            #     writeToFile = open(path, 'w')
            #
            ...
            #             writeToFile.writelines(
            #                 "			$sCustomDataPath = '/usr/local/lscp/codexpanel/rainloop/data';\n")
            #         else:
            #             writeToFile.writelines(items)
            #
            #     writeToFile.close()
            ...
            os.chdir("/usr/local/CodexCP/public")
            ...
            if os.path.exists('/usr/local/CodexCP/public/snappymail'):
                shutil.rmtree('/usr/local/CodexCP/public/snappymail')

            while (1):
                command = 'unzip -q snappymail-%s.zip -d /usr/local/CodexCP/public/snappymail' % (Upgrade.SnappyVersion)
            ...
            os.chdir("/usr/local/CodexCP/public/snappymail")
            ...
            iPath = os.listdir('/usr/local/CodexCP/public/snappymail/snappymail/v/')
            ...
            command = "mkdir -p /usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/configs/"
            Upgrade.executioner_silent(command, 'mkdir snappymail configs', 0)

            command = f'wget -q -O /usr/local/CodexCP/snappymail_codexpanel.php  https://raw.githubusercontent.com/the-djmaze/snappymail/master/integrations/codexpanel/install.php'
            Upgrade.executioner_silent(command, 'verify certificate', 0)

            command = f'/usr/local/lsws/lsphp80/bin/php /usr/local/CodexCP/snappymail_codexpanel.php'
            Upgrade.executioner_silent(command, 'verify certificate', 0)

            # labsPath = '/usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/configs/application.ini'
            ...
            includeFileOldPath = '/usr/local/CodexCP/public/snappymail/_include.php'
            includeFileNewPath = '/usr/local/CodexCP/public/snappymail/include.php'

            # if os.path.exists(includeFileOldPath):
            #     writeToFile = open(includeFileOldPath, 'a')
            #     writeToFile.write("\ndefine('APP_DATA_FOLDER_PATH', '/usr/local/lscp/codexpanel/rainloop/data/');\n")
            #     writeToFile.close()
            ...
            #             command = f'mkdir /usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
            #             command = f'chmod 700 /usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
            #             command = f'chown lscpd:lscpd /usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/plugins/mailbox-detect'
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
            #             command = f'wget -O /usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/plugins/mailbox-detect/index.php https://raw.githubusercontent.com/the-djmaze/snappymail/master/plugins/mailbox-detect/index.php'
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
            #             command = f'chmod 644 /usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/plugins/mailbox-detect/index.php'
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
            #             command = f'chown lscpd:lscpd /usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/plugins/mailbox-detect/index.php'
            #             Upgrade.executioner(command, 'verify certificate', 0)
            #
            ...
            #             PluginsFilePath = '/usr/local/lscp/codexpanel/rainloop/data/_data_/_default_/configs/plugin-mailbox-detect.json'
            #
    ...
    def downloadLink():
        try:
            ...
            try:
                ...
                path = "/usr/local/CodexCP/version.txt"
                writeToFile = open(path, 'w')
    ...
    def setupCLI():
        try:
            ...
            command = "ln -s /usr/local/CodexCP/cli/codexPanel.py /usr/bin/codexpanel"
            Upgrade.executioner(command, 'CLI Symlink', 0)

            command = "chmod +x /usr/local/CodexCP/cli/codexPanel.py"
    ...
    def staticContent():
        ...
        command = "rm -rf /usr/local/CodexCP/public/static"
        ...
        command = '/usr/local/codexpanel/bin/python manage.py collectstatic --noinput --clear'
        Upgrade.executioner(command, 'Remove old static content', 0)

        os.chdir(cwd)

        shutil.move("/usr/local/CodexCP/static", "/usr/local/CodexCP/public/")
    ...
    def applyLoginSystemMigrations():
        try:
            ...
            connection, cursor = Upgrade.setupConnection('codexpanel')

            try:
                cursor.execute(
                    'CREATE TABLE `baseTemplate_codexpanelcosmetic` (`id` integer AUTO_INCREMENT NOT NULL PRIMARY KEY, `MainDashboardCSS` longtext NOT NULL)')
            except:
                pass
...
""")
    ...
    def setupWebmail():
        ...
        try:
            ...
            already_configured = os.path.exists('/etc/codexpanel/webmail.conf') and \
            ...
            if not already_configured:
                ...
                with open('/etc/dovecot/master-users', 'w') as f:
                    f.write('codexpanel_master:' + password_hash + '\n')
                os.chmod('/etc/dovecot/master-users', 0o600)
                subprocess.call(['chown', 'dovecot:dovecot', '/etc/dovecot/master-users'])

                # Write /etc/codexpanel/webmail.conf
                webmail_conf = {
                    'master_user': 'codexpanel_master',
                    'master_password': master_password
                }
                with open('/etc/codexpanel/webmail.conf', 'w') as f:
                    json.dump(webmail_conf, f)
                os.chmod('/etc/codexpanel/webmail.conf', 0o600)
                subprocess.call(['chown', 'codexpanel:codexpanel', '/etc/codexpanel/webmail.conf'])
...
"""
            ...
            Upgrade.executioner(
                'python /usr/local/CodexCP/manage.py makemigrations webmail',
                'Webmail makemigrations', shell=True
            )
            Upgrade.executioner(
                'python /usr/local/CodexCP/manage.py migrate',
                'Webmail migrate', shell=True
            )

            # Fix webmail.conf ownership for lscpd (may be wrong on existing installs)
            if os.path.exists('/etc/codexpanel/webmail.conf'):
                subprocess.call(['chown', 'codexpanel:codexpanel', '/etc/codexpanel/webmail.conf'])
                os.chmod('/etc/codexpanel/webmail.conf', 0o600)
  ...
  PRIMARY KEY (`id`)
    ...
    def GeneralMigrations():
        try:
            ...
            command = '/usr/local/codexpanel/bin/python manage.py makemigrations'
            Upgrade.executioner(command, 'python manage.py makemigrations', 0)

            command = '/usr/local/codexpanel/bin/python manage.py makemigrations'
            Upgrade.executioner(command, '/usr/local/codexpanel/bin/python manage.py migrate', 0)
  ...
  CONSTRAINT `websiteFunctions_gdr_owner_id_4cf7983e_fk_websiteFu` FOREIGN KEY (`owner_id`) REFERENCES `websiteFunctions_gdrive` (`id`)
    ...
    def enableServices():
        try:
            servicePath = '/home/codexpanel/powerdns'
            writeToFile = open(servicePath, 'w+')
            writeToFile.close()

            servicePath = '/home/codexpanel/postfix'
            writeToFile = open(servicePath, 'w+')
            writeToFile.close()

            servicePath = '/home/codexpanel/pureftpd'
            writeToFile = open(servicePath, 'w+')
            writeToFile.close()
        except:
            pass
    ...
    def backupCriticalFiles():
        ...
        import tempfile
        backup_dir = tempfile.mkdtemp(prefix='codexpanel_backup_')
        
        critical_files = [
            '/usr/local/CodexCP/CodexCP/settings.py',
            '/usr/local/CodexCP/.git/config',  # Git configuration
        ]
        
        # Also backup any custom configurations
        custom_configs = [
            '/usr/local/CodexCP/baseTemplate/static/baseTemplate/custom/',
            '/usr/local/CodexCP/public/phpmyadmin/config.inc.php',
            '/usr/local/CodexCP/rainloop/data/_data_/',
        ]
        
        # Backup Imunify360 directories and configuration
        imunify_paths = [
            '/usr/local/CodexCP/public/imunify',
            '/usr/local/CodexCP/public/imunifyav',
    ...
    'rootdb': {
        ...
        'PORT': '%s',
            ...
            settingsFile = '/usr/local/CodexCP/CodexCP/settings.py'

            Upgrade.stdOut("Critical files backed up to: " + backup_dir)

            ## Always do a fresh clone for clean upgrade
            
            Upgrade.stdOut("Performing clean upgrade by removing and re-cloning codexpanel...")
            
            # Set git config first
            command = 'git config --global user.email "support@codexpanel.net"'
            if not Upgrade.executioner(command, command, 1):
                return 0, 'Failed to execute %s' % (command)

            command = 'git config --global user.name "CodexPanel"'
            if not Upgrade.executioner(command, command, 1):
                return 0, 'Failed to execute %s' % (command)
            ...
            # Remove old CodexCP directory
            if os.path.exists('CodexCP'):
                Upgrade.stdOut("Removing old CodexCP directory...")
                try:
                    shutil.rmtree('CodexCP')
                    Upgrade.stdOut("Old CodexCP directory removed successfully.")
                except Exception as e:
                    Upgrade.stdOut(f"Error removing CodexCP directory: {str(e)}")
                    # Try to restore backup if removal fails
                    Upgrade.restoreCriticalFiles(backup_dir, backed_up_files)
                    return 0, 'Failed to remove old CodexCP directory'

            # Clone the new repository directly to CodexCP
            Upgrade.stdOut("Cloning fresh CodexPanel repository...")
            command = 'git clone https://github.com/josevijuzen-source/CODEXPANEL CodexCP'
            if not Upgrade.executioner(command, command, 1):
                # Try to restore backup if clone fails
                Upgrade.stdOut("Clone failed, attempting to restore backup...")
                Upgrade.restoreCriticalFiles(backup_dir, backed_up_files)
                return 0, 'Failed to clone CodexPanel repository'
    ...
    def installLSCPD(branch):
        try:
            ...
            if Upgrade.SoftUpgrade == 0:
                ...
                command = f'cp -f /usr/local/CodexCP/{lscpdSelection} /usr/local/lscp/bin/{lscpdSelection}'
                Upgrade.executioner(command, command, 0)
    ...
    ### disable dkim signing in rspamd in ref to https://github.com/usmannasir/CodexPanel/issues/1176
    @staticmethod
    def FixRSPAMDConfig():
...
include '/usr/local/CodexCP/public/snappymail/index.php';
...
?>""" % (generate_pass())
                ...
                writeToFile = open('/usr/local/CodexCP/public/snappymail.php', 'w')
                writeToFile.write(content)
                writeToFile.close()

                command = "chown -R lscpd:lscpd /usr/local/lscp/codexpanel/snappymail/data"
            ...
            ###### fix Core CodexPanel permissions

            command = "find /usr/local/CodexCP -type d -exec chmod 0755 {} \;"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "find /usr/local/CodexCP -type f -exec chmod 0644 {} \;"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod -R 755 /usr/local/CodexCP/bin"
            Upgrade.executioner(command, 'chown core code', 0)

            ## change owner

            command = "chown -R root:root /usr/local/CodexCP"
            Upgrade.executioner(command, 'chown core code', 0)
            ...
            command = "chown -R lscpd:lscpd /usr/local/CodexCP/public/phpmyadmin/tmp"
            Upgrade.executioner(command, 'chown core code', 0)
            ...
            command = "chown -R lscpd:lscpd /usr/local/lscp/codexpanel/rainloop"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod 700 /usr/local/CodexCP/cli/codexPanel.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod 700 /usr/local/CodexCP/plogical/upgradeCritical.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod 755 /usr/local/CodexCP/postfixSenderPolicy/client.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chmod 640 /usr/local/CodexCP/CodexCP/settings.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = "chown root:codexpanel /usr/local/CodexCP/CodexCP/settings.py"
            Upgrade.executioner(command, 'chown core code', 0)

            command = 'chmod +x /usr/local/CodexCP/CLManager/CLPackages.py'
            ...
            fileM = ['/usr/local/lsws/FileManager/', '/usr/local/CodexCP/install/FileManager',
                     '/usr/local/CodexCP/serverStatus/litespeed/FileManager',
            ...
            command = 'chmod +x /usr/local/CodexCP/plogical/renew.py'
            Upgrade.executioner(command, command, 0)

            command = 'chmod +x /usr/local/CodexCP/CLManager/CLPackages.py'
            Upgrade.executioner(command, command, 0)

            clScripts = ['/usr/local/CodexCP/CLScript/panel_info.py',
                         '/usr/local/CodexCP/CLScript/CloudLinuxPackages.py',
                         '/usr/local/CodexCP/CLScript/CloudLinuxUsers.py',
                         '/usr/local/CodexCP/CLScript/CloudLinuxDomains.py'
                , '/usr/local/CodexCP/CLScript/CloudLinuxResellers.py',
                         '/usr/local/CodexCP/CLScript/CloudLinuxAdmins.py',
                         '/usr/local/CodexCP/CLScript/CloudLinuxDB.py', '/usr/local/CodexCP/CLScript/UserInfo.py']

            for items in clScripts:
                command = 'chmod +x %s' % (items)
                Upgrade.executioner(command, 0)

            command = 'chmod 600 /usr/local/CodexCP/plogical/adminPass.py'
            Upgrade.executioner(command, 0)

            command = 'chmod 600 /etc/cagefs/exclude/codexpanelexclude'
            Upgrade.executioner(command, 0)

            command = "find /usr/local/CodexCP/ -name '*.pyc' -delete"
            Upgrade.executioner(command, 0)
            ...
            command = 'chmod 640 /usr/local/lscp/codexpanel/logs/access.log'
            Upgrade.executioner(command, 0)

            command = '/usr/local/lsws/lsphp72/bin/php /usr/local/CodexCP/public/snappymail.php'
            Upgrade.executioner_silent(command, 'Configure SnappyMail')

            command = 'chmod 600 /usr/local/CodexCP/public/snappymail.php'
            ...
            command = 'chmod 700 %s' % ('/home/codexpanel')
            Upgrade.executioner(command, 0)
    ...
    def someDirectories():
        ...
        command = "mkdir -p /usr/local/lscp/codexpanel/logs"
        Upgrade.executioner(command, 0)
    ...
    def upgradeDovecot():
        try:
            ...
            configbackups = '/home/codexpanel/configbackups'
...
panel_info = /usr/local/CodexCP/CLScript/panel_info.py
packages = /usr/local/CodexCP/CLScript/CloudLinuxPackages.py
users = /usr/local/CodexCP/CLScript/CloudLinuxUsers.py
domains = /usr/local/CodexCP/CLScript/CloudLinuxDomains.py
resellers = /usr/local/CodexCP/CLScript/CloudLinuxResellers.py
admins = /usr/local/CodexCP/CLScript/CloudLinuxAdmins.py
db_info = /usr/local/CodexCP/CLScript/CloudLinuxDB.py

[lvemanager_config]
ui_user_info = /usr/local/CodexCP/CLScript/UserInfo.py
...
"""
                ...
                writeToFile = open('/etc/cagefs/exclude/codexpanelexclude', 'w')
                writeToFile.write(content)
                writeToFile.close()
    ...
    def runSomeImportantBash():
        ...
        # Remove invalid crons from /etc/crontab Reference: https://github.com/usmannasir/CodexPanel/issues/216
        command = """sed -i '/CodexCP/d' /etc/crontab"""
        ...
        # Install any CodexPanel missing crons to root crontab so its visible to users via crontab -l as root user
        ...
        if os.path.exists(cronPath):
            ...
            if data.find('findBWUsage') == -1:
                content = """
0 * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/findBWUsage.py >/dev/null 2>&1
0 * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/postfixSenderPolicy/client.py hourlyCleanup >/dev/null 2>&1
0 0 1 * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/postfixSenderPolicy/client.py monthlyCleanup >/dev/null 2>&1
0 2 * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/upgradeCritical.py >/dev/null 2>&1
0 0 * * 4 /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/renew.py >/dev/null 2>&1
7 0 * * * "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" > /dev/null
*/3 * * * * if ! find /home/*/public_html/ -maxdepth 2 -type f -newer /usr/local/lsws/cgid -name '.htaccess' -exec false {} +; then /usr/local/lsws/bin/lswsctrl restart; fi
* * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/manage.py run_scheduled_scans >/usr/local/lscp/logs/scheduled_scans.log 2>&1
"""
            ...
            if data.find('IncScheduler.py') == -1:
                content = """
0 12 * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py Daily
0 0 * * 0 /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py Weekly
"""
            ...
            if data.find("IncScheduler.py '30 Minutes'") == -1:
                content = """
*/30 * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py '30 Minutes'
0 * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py '1 Hour'
0 */6 * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py '6 Hours'
0 */12 * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py '12 Hours'
0 1 * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py '1 Day'
0 0 */3 * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py '3 Days'
0 0 * * 0 /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py '1 Week'
"""
                writeToFile = open(cronPath, 'a')
                writeToFile.write(content)
                writeToFile.close()
...
"""
            ...
            if data.find('pdnsHealthCheck.py') == -1:
                content = """
*/5 * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/pdnsHealthCheck.py >/dev/null 2>&1
"""
                writeToFile = open(cronPath, 'a')
                writeToFile.write(content)
                writeToFile.close()
...
0 0 * * 4 /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/renew.py >/dev/null 2>&1
7 0 * * * "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" > /dev/null
0 0 * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py Daily
0 0 * * 0 /usr/local/CodexCP/bin/python /usr/local/CodexCP/IncBackups/IncScheduler.py Weekly
* * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/manage.py run_scheduled_scans >/usr/local/lscp/logs/scheduled_scans.log 2>&1
*/5 * * * * /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/pdnsHealthCheck.py >/dev/null 2>&1
"""
...
'''
    ...
    def upgrade(branch):
        ...
        if not os.path.exists('/usr/local/lsws/bin/openlitespeed'):
            ...
            command = 'wget https://raw.githubusercontent.com/josevijuzen-source/CODEXPANEL/main/install/litespeed/httpd_config.xml'
            Upgrade.executioner(command, command, 0)
            ...
            if Upgrade.installCustomOLSBinaries():
                ...
                try:
                    ...
                    if 'autoSSL' not in content:
                        content = re.sub(
                            r'(adminEmails\s+\S+)',
                            r'\1\nautoSSL                   1\nacmeEmail                 admin@codexpanel.net',
                            content,
                            count=1
                        )
        ...
        if os.path.exists('/usr/local/codexpanel.' + versionNumbring):
            os.remove('/usr/local/codexpanel.' + versionNumbring)

        ##

        # execPath = "sudo /usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/csf.py"
        ...
        if os.path.exists('/usr/local/CodexCP/configservercsf'):
            command = 'rm -rf /usr/local/CodexCP/configservercsf'
        ...
        if os.path.exists(integrationConfig):
            try:
                ...
                if 'ui_path =/usr/local/CodexCP/public/imunifyav' in configContent:
                    # This is ImunifyAV configuration
                    Upgrade.stdOut("Detected ImunifyAV configuration, reconfiguring...")
                    imunifyAVPath = '/usr/local/CodexCP/public/imunifyav'

                    if os.path.exists(imunifyAVPath):
                        execPath = "/usr/local/CodexCP/bin/python /usr/local/CodexCP/CLManager/CageFS.py"
                        command = execPath + " --function submitinstallImunifyAV"
                        Upgrade.executioner(command, command, 1)

                        # Set permissions on ImunifyAV execute file
                        imunifyAVExecute = '/usr/local/CodexCP/public/imunifyav/bin/execute.py'
                ...
                elif 'ui_path =/usr/local/CodexCP/public/imunify' in configContent:
                    # This is Imunify360 configuration
                    Upgrade.stdOut("Detected Imunify360 configuration, checking system installation...")
                    imunify360Path = '/usr/local/CodexCP/public/imunify'
                    ...
                    if os.path.exists(imunify360Path):
                        ...
                        imunify360Execute = '/usr/local/CodexCP/public/imunify/bin/execute.py'
    ...
    def restoreImunify360():
        ...
        try:
            ...
            imunifyPath = '/usr/local/CodexCP/public/imunify'
            imunifyAVPath = '/usr/local/CodexCP/public/imunifyav'
            ...
            if os.path.exists(imunifyAVPath):
                ...
                if os.path.exists(configPath):
                    ...
                    executePath = '/usr/local/CodexCP/public/imunifyav/bin/execute.py'
            ...
            if os.path.exists(imunifyPath):
                ...
                binDir = '/usr/local/CodexCP/public/imunify/bin'
                executeFile = '/usr/local/CodexCP/public/imunify/bin/execute.py'
    ...
    def finalImunifyPermissions():
        ...
        try:
            ...
            if os.path.exists(executeFile):
                ...
                if result:
                    Upgrade.stdOut("âœ… SUCCESS: Imunify360 execute permissions set successfully!")
                else:
                    Upgrade.stdOut("âŒ FAILED: Could not set Imunify360 execute permissions")
                ...
                try:
                    ...
                    if file_stat.st_mode & stat.S_IXUSR:
                        Upgrade.stdOut("âœ… VERIFIED: Execute permission confirmed on Imunify360 file")
                    else:
                        Upgrade.stdOut("âŒ VERIFICATION FAILED: Execute permission not set")
                except Exception as verify_error:
                    Upgrade.stdOut(f"âš ï¸  Could not verify permissions: {str(verify_error)}")
            else:
                Upgrade.stdOut(f"âš ï¸  Imunify360 execute file not found: {executeFile}")

            Upgrade.stdOut("=== FINAL STEP COMPLETE ===")

        except Exception as e:
            Upgrade.stdOut(f"âŒ ERROR in final permission setting: {str(e)}")

        Upgrade.installDNS_codexpanelACMEFile()
...
extprocessor proxyApacheBackendSSL {
  ...
  respBuffer              0
    ...
    @staticmethod
    def installDNS_codexpanelACMEFile():
        filePath = '/root/.acme.sh/dns_codexpanel.sh'
        if os.path.exists(filePath):
            os.remove(filePath)
        shutil.copy('/usr/local/CodexCP/install/dns_codexpanel.sh', filePath)
