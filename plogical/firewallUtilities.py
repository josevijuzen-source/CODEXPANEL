...
class FirewallUtilities:
    ...
    def doCommand(command):
        try:
            ...
            if res == 0:
                logging.CodexCPLogFileWriter.writeToFile("Failed to apply rule: " + command + " Error #" + str(res))
                return 0

        except OSError as msg:
            logging.CodexCPLogFileWriter.writeToFile("Failed to apply rule: " + command + " Error: " + str(msg))
            return 0
        except ValueError as msg:
            logging.CodexCPLogFileWriter.writeToFile("Failed to apply rule: " + command + " Error: " + str(msg), 1)
            return 0
        return 1
    ...
    def addRule(proto,port,ipAddress):
        if not _firewallInputsValid(proto, port, ipAddress):
            logging.CodexCPLogFileWriter.writeToFile("Rejected firewall addRule with invalid input: %s %s %s" % (str(proto), str(port), str(ipAddress)))
            return 0
    ...
    def addSieveFirewallRule():
        ...
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile("Sieve port 4190 added to firewall successfully")
            return 1
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile("Failed to add Sieve port 4190 to firewall: " + str(msg))
            return 0

    @staticmethod
    def deleteRule(proto, port, ipAddress):
        if not _firewallInputsValid(proto, port, ipAddress):
            logging.CodexCPLogFileWriter.writeToFile("Rejected firewall deleteRule with invalid input: %s %s %s" % (str(proto), str(port), str(ipAddress)))
            return 0
        ruleFamily = 'rule family="ipv4"'
        sourceAddress = 'source address="' + ipAddress + '"'
        ruleProtocol = 'port protocol="' + proto + '"'
    ...
    def addSSHKey(tempPath, path=None):
        try:
            ...
            if os.path.exists(pathToSSH):
                ...
                sshFile.writelines("#Created by codexpanel\n")
                sshFile.close()
