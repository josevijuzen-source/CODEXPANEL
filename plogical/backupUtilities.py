...
class backupUtilities:
    ...
    completeKeyPath = "/home/codexpanel/.ssh"
    destinationsPath = "/home/codexpanel/destinations"
    licenseKey = '/usr/local/lsws/conf/license.key'
    NiceDefault = '10'
    CPUDefault = '1000'
    CloudBackupConfigPath = '/home/codexpanel/CloudBackup.json'
    time = 10
    ...
    def prepareBackupMeta(backupDomain, backupName, tempStoragePath, backupPath, FromInner=1):
        try:
            ...
            if FromInner:
                ...
                #logging.CodexCPLogFileWriter.statusWriter(status, 'Setting up meta data..')
                command = f"echo 'Setting up meta data..' > {status}"
                ProcessUtilities.executioner(command, website.externalApp)
            else:
                status = '/home/codexpanel/dummy'

            if os.path.exists(ProcessUtilities.debugPath):
                logging.CodexCPLogFileWriter.writeToFile(f'Creating meta for {backupDomain}.')
            ...
            ### try to take care of - https://github.com/usmannasir/CodexPanel/issues/1196
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile('%s. [167:prepMeta]' % (str(msg)))
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile('%s. [158:prepMeta]' % (str(msg)))
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile('%s. [179:prepMeta]' % (str(msg)))
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.CodexCPLogFileWriter.writeToFile(f'Path to meta file {metaPath}')
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"{str(msg)} [207][5009]")
            if FromInner:
                #logging.CodexCPLogFileWriter.statusWriter(status, "%s [207][5009]" % (str(msg)), status)
    ...
    def startBackup(tempStoragePath, backupName, backupPath, metaPath=None):
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(backupFileNamePath, backupName)

            #####

            status = os.path.join(backupPath, 'status')

            logging.CodexCPLogFileWriter.statusWriter(status, "Making archive of home directory.\n")
            ...
            ### doing backup of whole dir and keeping it in public_html folder will restore from here - ref https://github.com/usmannasir/CodexPanel/issues/1196
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Backing up databases..")
            print('1,None')
            ...
            status = os.path.join(backupPath, 'status')
            logging.CodexCPLogFileWriter.statusWriter(status, "Aborted, " + str(msg) + ".[365] [5009]")
    ...
    def BackupRoot(tempStoragePath, backupName, backupPath, metaPath=None, externalApp = None, CPHomeStorage=None):
        ...
        ### CPHomeStorage /home/codexpanel/<random_number>
        ...
        if os.path.exists(sslStoragePath):
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f'{str(msg)}. [283:startBackup]')
        ...
        try:
            for childDomain in childDomains:
                ...
                TempConfPath = f'/home/codexpanel/{actualChildDomain}.vhost.conf'
        ...
        if os.path.islink(status) or os.path.islink(tempStoragePath or os.path.islink(backupPath)) or os.path.islink(
                metaPath):
            logging.CodexCPLogFileWriter.writeToFile('symlinked.')
            #logging.CodexCPLogFileWriter.statusWriter(status, 'Symlink attack. [365][5009]')
            return 0

        ## backup email accounts

        if externalApp == None:
            logging.CodexCPLogFileWriter.statusWriter(status, "Backing up email accounts..\n")
        else:
        ...
        try:
            ...
            if externalApp == None:
                logging.CodexCPLogFileWriter.statusWriter(status, "Preparing final compressed package..\n")
            else:
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile('%s. [backupRoot:499]' % str(msg))
                for items in backupObs:
                    items.status = 1
                    items.size = totalSize
                    items.save()
            ...
            if externalApp == None:
                logging.CodexCPLogFileWriter.statusWriter(status, "Completed\n")
            else:
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, '%s. [511:BackupRoot][[5009]]\n' % str(msg))
            if externalApp == None:
                logging.CodexCPLogFileWriter.statusWriter(status, '%s. [511:BackupRoot][[5009]]\n')
            else:
    ...
    def initiateBackup(tempStoragePath, backupName, backupPath):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [initiateBackup]")
        ...
        try:
            ...
            for database in databases:
                ...
                if ((VERSION == '2.1' or VERSION == '2.3') and int(BUILD) >= 1) or (VERSION == '2.4' and int(BUILD) >= 0):
                    ...
                    logging.CodexCPLogFileWriter.writeToFile('Backup version 2.1.1+ detected..')
                    databaseUsers = database.findall('databaseUsers')
                    for databaseUser in databaseUsers:

                        dbUser = databaseUser.find('dbUser').text
                        res = mysqlUtilities.mysqlUtilities.createDatabase(dbName, dbUser, 'codexpanel')
                        if res == 0:
                    ...
                    if mysqlUtilities.mysqlUtilities.createDatabase(dbName, dbUser, "CodexPanel") == 0:
                        raise BaseException
    ...
    def startRestore(backupName, dir):
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(pid, str(os.getpid()))

            status = os.path.join(completPath, 'status')
            logging.CodexCPLogFileWriter.statusWriter(status, "Extracting Main Archive!")
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Creating Accounts,Databases and DNS records!")
            ...
            if result[0] == 1:
                ...
                if os.path.exists(sslStoragePath):
                    ...
                    try:
                        ...
                        logging.CodexCPLogFileWriter.writeToFile('%s. [555:startRestore]' % (str(msg)))

            else:
                logging.CodexCPLogFileWriter.statusWriter(status, "Error Message: " + result[
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Creating Child Domains!")
            ...
            try:
                for childDomain in childDomains:
                    ...
                    if retValues[0] == 1:
                        ...
                        try:
                            ...
                            if os.path.exists(completPath + '/' + domain + '.vhost.conf'):
                                ...
                                if CurrentServer == ProcessUtilities.decideServer():
                                    ...
                                    url = "https://platform.cyberpersons.com/CodexPanelAdOns/Adonpermission"
                                    data = {
                                        "name": "all",
                                    ...
                                    if (Status == 1):
                                        ...
                                        tempStatusPath = '/home/codexpanel/fakePath'

                                        if os.path.exists(ProcessUtilities.debugPath):
                                            logging.CodexCPLogFileWriter.writeToFile(f'Conf path of apache for child domain {domain} in backup is {childConfPathinBKUPApache}')
                                        ...
                                        if childData.find('proxyApacheBackendSSL') > -1 and os.path.exists(childConfPathinBKUPApache):
                                            ...
                                            if os.path.exists(finalConfPathApache):
                                                ...
                                                if os.path.exists(ProcessUtilities.debugPath):
                                                    logging.CodexCPLogFileWriter.writeToFile(
                                                        f'codexpanel was able to successfully convert {domain} to apache conf as {finalConfPathApache} exists..')
                        ...
                        logging.CodexCPLogFileWriter.writeToFile('Error domain %s' % (domain))
                        logging.CodexCPLogFileWriter.statusWriter(status, "Error Message: " + retValues[
                ...
                status.close()
                logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [startRestore]")
                return 0

            ## Restore Aliases

            logging.CodexCPLogFileWriter.statusWriter(status, "Restoring Domain Aliases!")
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Restoring email accounts!")
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.statusWriter(status, "Error Message: " + str(
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Restoring Databases!")
                ...
                if ((VERSION == '2.1' or VERSION == '2.3') and int(BUILD) >= 1) or (VERSION == '2.4' and int(BUILD) >= 0):
                    ...
                    for databaseUser in databaseUsers:
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            ...
                            logging.CodexCPLogFileWriter.writeToFile('Database user: %s' % (dbUser))
                            logging.CodexCPLogFileWriter.writeToFile('Database host: %s' % (dbHost))
                            logging.CodexCPLogFileWriter.writeToFile('Database password: %s' % (password))
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Extracting web home data!")
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Extracting email accounts!")
            ...
            if os.path.exists(completPath + '/vhost.conf'):
                if CurrentServer == ProcessUtilities.decideServer():
                    ...
                    if (Status == 1):
                        ...
                        if os.path.exists(ProcessUtilities.debugPath):
                            logging.CodexCPLogFileWriter.writeToFile(f'Conf path of apache for main site {masterDomain} in backup is {confPathApache}')
                        ...
                        if childData.find('proxyApacheBackendSSL') > -1 and os.path.exists(confPathApache):
                            ...
                            if os.path.exists(ProcessUtilities.debugPath):
                                logging.CodexCPLogFileWriter.writeToFile(
                                    f'Apache conf path of main domain exists which means CodexPanel successfully converted site to Apache for {masterDomain}')
            ...
            logging.CodexCPLogFileWriter.statusWriter(status, "Done")
            ...
            status = os.path.join(completPath, 'status')
            logging.CodexCPLogFileWriter.statusWriter(status, str(msg) + " [736][5009]")
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [startRestore]")
    ...
    def initiateRestore(backupName, dir):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [initiateRestore]")
    ...
    #         ## Temp changes
    #
    #         command = 'chmod 600 %s' % ('/root/.ssh/codexpanel.pub')
    #         ProcessUtilities.executioner(command)
    #
    #         command = "scp -o StrictHostKeyChecking=no -P " + port + " /root/.ssh/codexpanel.pub " + user + "@" + IPAddress + ":~/.ssh/authorized_keys"
    #         setupKeys = pexpect.spawn(command, timeout=3)
    #
    #         if os.path.exists(ProcessUtilities.debugPath):
    #             logging.CodexCPLogFileWriter.writeToFile(command)
    #
    ...
    #         ## Temp changes
    #
    #         command = 'chmod 644 %s' % ('/root/.ssh/codexpanel.pub')
    #         ProcessUtilities.executioner(command)
    #
    #         return [1, "None"]
    #
    ...
    #         ProcessUtilities.executioner(command)
    #
    #         logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [sendKey]")
    ...
    #             pass
    #         else:
    #             logging.CodexCPLogFileWriter.writeToFile("Host is Down.")
    ...
    def sendKey(IPAddress, password, port='22', user='root'):
        try:
            ...
            if os.path.exists('/root/.ssh/codexpanel.pub'):
                pass
            else:
            ...
            try:
                ...
                sftp.put('/root/.ssh/codexpanel.pub', '.ssh/authorized_keys')
                sftp.close()
    ...
    def setupSSHKeys(IPAddress, password, port='22', user='root'):
        try:
            ...
            if password != 'NOT-NEEDED':
                ...
                private_key_path = '/root/.ssh/codexpanel'
    ...
    def checkIfHostIsUp(IPAddress):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "[checkIfHostIsUp]")
    ...
    def checkConnection(IPAddress, port='22', user='root'):
        try:
            ...
            command = "sudo ssh -i /root/.ssh/codexpanel -o StrictHostKeyChecking=no -p " + port + ' ' + user + "@" + IPAddress
            ...
            logging.CodexCPLogFileWriter.writeToFile("Timeout " + IPAddress + " [checkConnection]")
            return [0, "371 Timeout while making connection to this server [checkConnection]"]
        except pexpect.EOF as msg:
            logging.CodexCPLogFileWriter.writeToFile("EOF " + IPAddress + "[checkConnection]")
            return [0, "374 Remote Server is not able to authenticate for transfer to initiate. [checkConnection]"]
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " " + IPAddress + " [checkConnection]")
    ...
    def verifyHostKey(IPAddress, port='22', user='root'):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile("Timeout [verifyHostKey]")
            return [0, "Timeout [verifyHostKey]"]
        except pexpect.EOF as msg:
            logging.CodexCPLogFileWriter.writeToFile("EOF [verifyHostKey]")
            return [0, "EOF [verifyHostKey]"]
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [verifyHostKey]")
    ...
    def createBackupDir(IPAddress, port='22', user='root'):
        ...
        try:
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/codexpanel " + user + "@" + IPAddress + " mkdir ~/backup"
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/codexpanel " + user + "@" + IPAddress + ' "cat ~/.ssh/authorized_keys ~/.ssh/temp > ~/.ssh/authorized_temp"'
            ...
            command = "sudo ssh -o StrictHostKeyChecking=no -p " + port + " -i /root/.ssh/codexpanel " + user + "@" + IPAddress + ' "cat ~/.ssh/authorized_temp > ~/.ssh/authorized_keys"'
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [createBackupDir]")
    ...
    def host_key_verification(IPAddress):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [host_key_verification]")
            return 0
    ...
    def getAliases(masterDomain):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + "  [getAliases]")
            print(0)
    ...
    def CheckIfSleepNeeded(self):
        import psutil
        while (1):
            logging.CodexCPLogFileWriter.writeToFile('Current CPU percent %s.' % (int(psutil.cpu_percent(interval=None))))
    ...
    def CloudBackups(self):
        ...
        command = 'nice -n %s tar czf %s.tar.gz -C %s %s' % (self.nice, self.BackupPath, '/home/codexpanel/backups/%s' % (self.extraArgs['domain']), self.BackupPath.split('/')[-1])
        ProcessUtilities.executioner(command)
        ...
        command = 'chown codexpanel:codexpanel %s' % (finalPath)
        ProcessUtilities.executioner(command)
        ...
        if self.extraArgs['port'] != 0:
            ...
            command = "scp -o StrictHostKeyChecking=no -P %s -i /root/.ssh/codexpanel %s root@%s:/home/codexpanel/backups/%s/" % (self.extraArgs['port'], finalPath, self.extraArgs['ip'], self.extraArgs['destinationDomain'])
    ...
    def SubmitCloudBackupRestore(self):
        try:
            ...
            self.BackupPath = '/home/codexpanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'])
            self.website = Websites.objects.get(domain=self.extraArgs['domain'])
            ...
            command = 'tar -xf %s -C %s' % (self.BackupPath, '/home/codexpanel/backups/%s/' % (self.extraArgs['domain']))
            ProcessUtilities.executioner(command)

            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'],
                                                      'Main Archive extracted,20')

            self.extractedPath = '/home/codexpanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].rstrip('.tar.gz'))
                    ...
                    for child in childDomains:
                        try:
                            ...
                            virtualHostUtilities.createDomain(self.website.domain, child['domain'], child['php'], child['path'], 1, 0, 0,
                                                              self.website.admin.userName, 0, "/home/codexpanel/" + str(randint(1000, 9999)))

                except BaseException as msg:
                    logging.CodexCPLogFileWriter.writeToFile('%s [SubmitCloudBackupRestore:1533]' % str(msg))
            ...
            for db in databases:
                ...
                if mysqlUtilities.mysqlUtilities.createDatabase(db['databaseName'], db['databaseUser'], "CodexPanel") == 0:
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '%s [404].' % str(msg))

    ### Cloud Backup functions ends

    def fetchAWSKeys(self):
        path = '/home/codexpanel/.aws'
    ...
    def SubmitS3BackupRestore(self):
        ...
        try:
            ...
            self.BackupPath = '/home/codexpanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].split('/')[-1])
            ...
            self.extractedPath = '/home/codexpanel/backups/%s/%s' % (self.extraArgs['domain'], self.extraArgs['backupFile'].split('/')[-1].rstrip('.tar.gz'))
...
def submitBackupCreation(tempStoragePath, backupName, backupPath, backupDomain):
    try:
        ...
        ## /home/example.com/backup - backupPath
        ## /home/codexpanel/1047.xml - metaPath
        ## /home/backup/<random_number> - CPHomeStorage
        ...
        schedulerPath = f'/home/codexpanel/{backupDomain}-backup.txt'
        ...
        logging.CodexCPLogFileWriter.writeToFile(backupPath)
        logging.CodexCPLogFileWriter.writeToFile(tempStoragePath)

        execPath = "sudo nice -n 10 /usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/backupUtilities.py"
        ...
        if output.find('[5009]') > -1:
            logging.CodexCPLogFileWriter.writeToFile(output)
            writeToFile = open(schedulerPath, 'w')
        ...
        command = f'chown codexpanel:codexpanel {result[2]}'
        ProcessUtilities.executioner(command)
        ...
        for database in databases:
            ...
            dbName = database.find('dbName').text
            res = mysqlUtilities.mysqlUtilities.createDatabaseBackup(dbName, '/home/codexpanel')
            if res == 0:
                ## This login can be further improved later.
                logging.CodexCPLogFileWriter.writeToFile('Failed to create database backup for %s. This could be false positive, moving on.' % (dbName))

            # Move database backup (check for both .sql.gz and .sql)
            if os.path.exists(f'/home/codexpanel/{dbName}.sql.gz'):
                command = f'mv /home/codexpanel/{dbName}.sql.gz {CPHomeStorage}/{dbName}.sql.gz'
                ProcessUtilities.executioner(command)
                # Also move metadata file if it exists
                if os.path.exists(f'/home/codexpanel/{dbName}.backup.json'):
                    command = f'mv /home/codexpanel/{dbName}.backup.json {CPHomeStorage}/{dbName}.backup.json'
                    ProcessUtilities.executioner(command)
            elif os.path.exists(f'/home/codexpanel/{dbName}.sql'):
                command = f'mv /home/codexpanel/{dbName}.sql {CPHomeStorage}/{dbName}.sql'
                ProcessUtilities.executioner(command)
        ...
        ProcessUtilities.executioner(command, 'codexpanel')
...
def cancelBackupCreation(backupCancellationDomain, fileName):
    try:
        ...
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"{str(msg)} [cancelBackupCreation]")
...
def main():
    parser = argparse.ArgumentParser(description='CodexPanel Backup Generator')
