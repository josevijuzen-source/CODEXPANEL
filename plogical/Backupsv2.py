...
class CPBackupsV2(multi.Thread):
    ...
    def __init__(self, data):
        ...
        try:
            ...
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Refresh Token: {refreshToken}")
                ...
                logging.CodexCPLogFileWriter.writeToFile(f"Token Not upadate inside. Error: {str(msg)}")
                pass
            ...
            # else:
            #     logging.CodexCPLogFileWriter.writeToFile("Token Not upadate..........")
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile("Error update token............%s"%msg)
        ...
        self.StatusFile = f'/home/codexpanel/{self.website.domain}_rustic_backup_log'
        self.StatusFile_Restore = f'/home/codexpanel/{self.website.domain}_rustic_backup_log_Restore'
    ...
    def run(self):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [CPBackupsV2.run]')

    def FetchSnapShots(self):
        try:
            command = f'rustic -r {self.repo} snapshots --password "" --json 2>/dev/null'
            # SLSkjoSCczb6wxTMCBPmBMGq/UDSpp28-u cyber5986 rustic -r rclone:None:codexpanel.net snapshots --password "" --json 2>/dev/null
            result = json.loads(
        ...
        try:
            ...
            if type == CPBackupsV2.SFTP:
                ...
                try:
                    ...
                    ssh.close()
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.CodexCPLogFileWriter.writeToFile(f'Successfully connected to {config["host"]} through user {config["user"]}')
                except BaseException as msg:
                    return 0, str(msg)
...
'''
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [Configure.run]')
            return 0, str(msg)
    ...
    def BackupConfig(self):
        ...
        # command = f'chown codexpanel:codexpanel {self.FinalPathRuctic}'
        # ProcessUtilities.executioner(command)
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.CodexCPLogFileWriter.writeToFile(f'Status {str(status)}')
        ...
        if status:
            ...
            command = f'chown codexpanel:codexpanel {self.FinalPathRuctic}/config.json'
            ProcessUtilities.executioner(command)
    ...
    def InitiateBackup(self):
        logging.CodexCPLogFileWriter.writeToFile("[Create Backup start]")
        ...
        while (1):
            ...
            if self.website.BackupLock == 0:
                ...
                # command = f'chown codexpanel:codexpanel {self.FinalPath}'
                # ProcessUtilities.executioner(command)
                ...
                try:
                    ...
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.CodexCPLogFileWriter.writeToFile(f'Snapshots to be merged {str(self.snapshots)}')
    ...
    def BackupRustic(self):
        ...
        if os.path.exists(ProcessUtilities.debugPath):
            logging.CodexCPLogFileWriter.writeToFile(f'Status code {status}')
    ...
    def RestoreConfig(self):
        try:
            ...
            # try:
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.CodexCPLogFileWriter.writeToFile(f'ACL in config: {ConfigContent["acl"]}')
            #     acl = ACL.objects.get(name=ConfigContent['acl']['name'])
            # except:
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.CodexCPLogFileWriter.writeToFile('ACL Already existed.')
            ...
            #
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.CodexCPLogFileWriter.writeToFile(f'Passed content to Create ACL Func: {json.dumps(dataToPass)}')
            #
            #     resp = createACLFunc(requestNew)
            #     if os.path.exists(ProcessUtilities.debugPath):
            #         logging.CodexCPLogFileWriter.writeToFile(f'CreateACLFunc stats: {str(resp.content)}')
            ...
            try:
                ...
                for database in ConfigContent['databases']:
                    ...
                    if os.path.exists(ProcessUtilities.debugPath):
                        logging.CodexCPLogFileWriter.writeToFile(f'Databasename: {dbName}')
                    ...
                    for databaseUser in databaseUsers:
                        ...
                        if first:
                            ...
                            try:
                                ...
                                logging.CodexCPLogFileWriter.writeToFile('Database exists, changing Database password.. %s' % (dbName))

                                if mysqlUtilities.mysqlUtilities.changePassword(dbUser, password, 1, dbHost) == 0:
                                    logging.CodexCPLogFileWriter.writeToFile('Failed changing password for database: %s' % (dbName))
                                else:
                                    logging.CodexCPLogFileWriter.writeToFile('Password successfully changed for database: %s.' % (dbName))

                            except:

                                logging.CodexCPLogFileWriter.writeToFile('Database did not exist, creating new.. %s' % (dbName))

                                if mysqlUtilities.mysqlUtilities.createDatabase(dbName, dbUser, "CodexPanel") == 0:
                                    logging.CodexCPLogFileWriter.writeToFile('Failed the creation of database: %s' % (dbName))
                                else:
                                    logging.CodexCPLogFileWriter.writeToFile('Database: %s successfully created.' % (dbName))
    ...
    def FetchCurrentSchedules(website):
        try:
            finalConfigPath = f'/home/codexpanel/v2backups/{website}'
    ...
    def refresh_V2Gdive_token(refresh_token):
        try:
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.CodexCPLogFileWriter.writeToFile('Current Token: ' + refresh_token )
            ...
            if os.path.exists(ProcessUtilities.debugPath):
                logging.CodexCPLogFileWriter.writeToFile('newtoken: ' + newtoken )
                logging.CodexCPLogFileWriter.writeToFile(r.text)

            return newtoken
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile(f'Error in tkupdate: {str(msg)}')
    ...
    def CreateScheduleV2(website, repo, frequency, websiteData, websiteDatabases, websiteEmails, retention):
        try:
            ...
            if not os.path.exists('/home/codexpanel/v2backups/'):

                command = 'mkdir -p /home/codexpanel/v2backups/'
...
if __name__ == "__main__":
    try:
        ...
        cpbuv2 = CPBackupsV2(
            {'function': 'InitiateRestore', 'domain': 'codexpanel.net', 'BasePath': '/home/backup', 'SnapShotID': 1,
             'BackendName': 'usman'})
