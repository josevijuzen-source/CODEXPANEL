    ...
    def DeployWPContainer(self):
        ...
        try:
            ...
            if result.find("not found") > -1:
                ...
                execPath = "/usr/local/CodexCP/bin/python /usr/local/CodexCP/dockerManager/dockerInstall.py"
                ProcessUtilities.executioner(execPath)
...
services:
  '{self.data['ServiceName']}':
    user: root
    image: codexpanel/openlitespeed:latest
    ports:
      - "{self.data['port']}:8088"
#      - "443:443"
    environment:
...
'''
            ...
            TempCompose = f'/home/codexpanel/{self.data["finalURL"]}-docker-compose.yml'
            ...
            execPath = "/usr/local/CodexCP/bin/python /usr/local/CodexCP/plogical/DockerSites.py"
    ...
    def setup_docker_environment(self):
        try:
            ...
            command = "docker network ls | grep codexpanel"
            network_exists = ProcessUtilities.outputExecutioner(command, None, None, None, 1)
            if not network_exists:
                command = "docker network create codexpanel"
                ProcessUtilities.outputExecutioner(command, None, None, None, 1)
    ...
    def log_deployment_metrics(self, metrics):
        ...
        if metrics:
            try:
                log_file = f"/var/log/codexpanel/docker/{self.data['ServiceName']}_metrics.json"
...
def Main():
    try:
        ...
        parser = argparse.ArgumentParser(description='CodexPanel Docker Sites')
        ...
        if args.function == "SetupProxy":
            ...
            # finalURL, blogTitle, adminUser, adminPassword, adminEmail, htaccessPath, externalApp
            data = {
                "JobID": '/home/codexpanel/hey.txt',
                "ComposePath": "/home/docker.codexpanel.net/docker-compose.yml",
                "MySQLPath": '/home/docker.codexpanel.net/public_html/sqldocker',
                ...
                "MemoryMySQL": '512',
                "port": '8000',
                "SitePath": '/home/docker.codexpanel.net/public_html/wpdocker',
                "CPUsSite": '2',
                "MemorySite": '512',
                "SiteName": 'wp docker test',
                "finalURL": 'docker.codexpanel.net',
                "blogTitle": 'docker site',
                "adminUser": 'testdbwp',
                "adminPassword": 'testdbwp',
                "adminEmail": 'usman@cyberpersons.com',
                "htaccessPath": '/home/docker.codexpanel.net/public_html/.htaccess',
                "externalApp": 'docke8463',
                "docRoot": "/home/docker.codexpanel.net"
            }
