...
class remoteTransferUtilities:
    ...
    def writeAuthKey(pathToKey):
        try:
            ...
            if presenseCheck == 0:
                ...
                writeToFile.writelines("#Added by codexpanel\n")
            ...
            logging.CodexCPLogFileWriter.writeToFile("For remote transfer, I am not able to write key to auth file, Error Message: "+str(msg))
            ...
            if backupUtil.backupUtilities.checkIfHostIsUp(ipAddress) == 1:
                ...
                if checkConn[0] == 0:
                    writeToFile.writelines("[" + time.strftime(
                        "%m.%d.%Y_%H-%M-%S") + "]" + " Connection to:" + ipAddress + " Failed, please resetup this destination from codexpanel, aborting. [5010]" + "\n")
                    writeToFile.close()
                    return
                else:
                    pass
            ...
            try:
                ...
                for virtualHost in accountsToTransfer:
                    try:
                        ...
                        logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [remoteTransferUtilities.backupProcess:173]")
                        pass

                portpath = "/home/codexpanel/remote_port"
                try:
                    os.remove(portpath)
                except OSError:
                    pass
    ...
    def sendBackup(completedPathToSend, IPAddress, folderNumber,writeToFile):
        try:
            ...
            command = "sudo scp -o StrictHostKeyChecking=no -i /root/.ssh/codexpanel -P "+ sshPort + " " + completedPathToSend + " root@" + IPAddress + ":/home/backup/transfer-" + folderNumber + "/"
            ...
            if return_Code == 0:
                logging.CodexCPLogFileWriter.writeToFile("Remote backup file sent: %s" % completedPathToSend)
                writeToFile.writelines("[" + time.strftime(
    ...
    def startRestore(backupDir, backupLogPath, dir):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + " [remoteTransferUtilities.startRestore]")
