...
from .CodexCPLogFileWriter import CodexCPLogFileWriter as logging
...
class ACLManager:
    ...
    def FindIfChild():
        try:
            ...
            config = json.loads(open('/home/codexpanel/cluster', 'r').read())
    ...
    def CheckStatusFilleLoc(statusFile, domain=None):
        ...
        try:
            ...
            if domain != None:
                value = statusFile.split('codexpanel/')[1]
                #logging.writeToFile(f'value of log file {value}')
                if value == f'{domain}_rustic_backup_log':
                    return 1
            return 0

        if (statusFile[:18] != "/home/codexpanel/." or statusFile[:16] == "/home/codexpanel" or statusFile[:4] == '/tmp' or statusFile[
                                                                                                                 :18] == '/usr/local/CodexCP') \
                and statusFile != '/usr/local/CodexCP/CodexCP/settings.py' and statusFile.find(
            '..') == -1 and statusFile != '/home/codexpanel/.my.cnf' and statusFile != '/home/codexpanel/.bashrc' and statusFile != '/home/codexpanel/.bash_logout' and statusFile != '/home/codexpanel/.profile':
            return 1
        else:
            return 0
    ...
    def CreateSecureDir():
        ...
        UploadPath = '/usr/local/CodexCP/tmp/'

        if not os.path.exists(UploadPath):
            command = 'mkdir %s' % (UploadPath)
            ProcessUtilities.executioner(command)

        command = 'chown codexpanel:codexpanel %s' % (UploadPath)
        ProcessUtilities.executioner(command)
    ...
    def GetServiceStatus(dic):
        if os.path.exists('/home/codexpanel/postfix'):
            dic['emailAsWhole'] = 1
        else:
            dic['emailAsWhole'] = 0
