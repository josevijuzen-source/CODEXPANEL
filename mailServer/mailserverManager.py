...
try:
    ...
    pass
import plogical.CodexCPLogFileWriter as logging
import json
import shlex
import subprocess
try:
...
class MailServerManager(multi.Thread):
    ...
    def run(self):
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [MailServerManager.run]')
    ...
    def createEmailAccount(self):
        ...
        if not os.path.exists('/home/codexpanel/postfix'):
        ...
        try:
            ...
            if result[0] == 1:
                ...
                if EmailLimits != -1:
                    ...
                    RandomFile = "/home/codexpanel/" + str(randint(100000, 999999))
                    writeToFile = open(RandomFile, 'w')
                    writeToFile.write(limitString)
                    writeToFile.close()

                    execPath = "/usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/mailUtilities.py"
    ...
    def fixMailSSL(self, data = None):
        try:
            ...
            execPath = "/usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/virtualHostUtilities.py"
    ...
    def generateDKIMKeys(self):
        try:
            ...
            if output.find("1,None") > -1:
                ...
                command = 'chown codexpanel:codexpanel -R /usr/local/CodexCP/lib/python3.6/site-packages/tldextract/.suffix_cache'
                ProcessUtilities.executioner(command)

                command = 'chown codexpanel:codexpanel -R /usr/local/CodexCP/lib/python3.8/site-packages/tldextract/.suffix_cache'
                ProcessUtilities.executioner(command)

                command = 'chown codexpanel:codexpanel -R /usr/local/CodexCP/lib/python*/site-packages/tldextract/.suffix_cache'
    ...
    def RunServerLevelEmailChecks(self):
        try:
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Checking if MailServer SSL issued..,10')
    ...
    def install_postfix_dovecot(self):
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Postfix/dovecot reinstalled.,40')

        except BaseException as msg:
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '%s [install_postfix_dovecot][404]' % (str(msg)), 10)
            return 0
    ...
    def setup_postfix_dovecot_config(self):
        try:
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu:
                self.centos_lib_dir_to_ubuntu("/usr/local/CodexCP/install/email-configs-one/master.cf", "/usr/libexec/", "/usr/lib/")
                self.centos_lib_dir_to_ubuntu("/usr/local/CodexCP/install/email-configs-one/main.cf", "/usr/libexec/postfix",
            ...
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/mysql-virtual_domains.cf", "/etc/postfix/mysql-virtual_domains.cf")
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/mysql-virtual_forwardings.cf",
                        "/etc/postfix/mysql-virtual_forwardings.cf")
            shutil.copy("/usr/local/CodexCP/install/email-configs-one/mysql-virtual_mailboxes.cf", "/etc/postfix/mysql-virtual_mailboxes.cf")
...
"""
            ...
            logging.CodexCPLogFileWriter.writeToFile(f'Error in configureOpenDKIM {str(msg)}')
            return 0
    ...
    def installSieveAfterReset(self):
        ...
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile("Sieve reinstalled and configured after email reset")
            return 1
            
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile("Failed to reinstall Sieve after email reset: " + str(msg))
            return 0
...
"""
            ...
            logging.CodexCPLogFileWriter.writeToFile("Sieve configured in Dovecot successfully")
            return 1
            
        except BaseException as msg:
            logging.CodexCPLogFileWriter.writeToFile("Failed to configure Sieve in Dovecot: " + str(msg))
            return 0
    ...
    def ResetEmailConfigurations(self):
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing postfix/dovecot..,5')

            if self.install_postfix_dovecot() == 0:
                return 0

            # Ensure Sieve remains functional after email debugger reset
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Reinstalling Sieve after email reset..,45')
            ...
            if self.setup_postfix_dovecot_config() == 0:
                logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'setup_postfix_dovecot_config failed. [404].')
                return 0

            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Restoreing OpenDKIM configurations..,70')
            ...
            if self.configureOpenDKIM() == 0:
                logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'configureOpenDKIM failed. [404].')
                return 0


            if self.MailSSL:
                logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Setting up Mail Server SSL if any..,75')
                from plogical.virtualHostUtilities import virtualHostUtilities
        ...
        try:
            ...
            if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
                ...
                if (Status == 1):
                    ...
                    return redirect("https://codexpanel.net/codexpanel-addons")
            else:
                template = 'mailServer/EmailLimits.html'
        except BaseException as msg:
            template = 'mailServer/EmailLimits.html'
    ...
    def _regenerateVirtualRegexp(self):
        ...
        try:
            ...
            content = "# Auto-generated by CodexPanel - DO NOT EDIT MANUALLY\n"
            for rule in rules:
            ...
            logging.CodexCPLogFileWriter.writeToFile(str(msg) + ' [_regenerateVirtualRegexp]')
            return False


def main():

    parser = argparse.ArgumentParser(description='CodexPanel')
