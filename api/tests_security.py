...
class SecurityUtilsTests(SimpleTestCase):
    ...
    def test_terminal_secret_prefers_secret_file(self):
        ...
        old_path = os.environ.get("codexpanel_TERMINAL_JWT_SECRET_FILE")
        old_secret = os.environ.get("codexpanel_TERMINAL_JWT_SECRET")
        try:
            os.environ["codexpanel_TERMINAL_JWT_SECRET_FILE"] = secret_path
            os.environ.pop("codexpanel_TERMINAL_JWT_SECRET", None)
            self.assertEqual(get_terminal_jwt_secret(), "file-secret")
        finally:
            if old_path is None:
                os.environ.pop("codexpanel_TERMINAL_JWT_SECRET_FILE", None)
            else:
                os.environ["codexpanel_TERMINAL_JWT_SECRET_FILE"] = old_path

            if old_secret is not None:
                os.environ["codexpanel_TERMINAL_JWT_SECRET"] = old_secret
    ...
    def test_terminal_secret_has_legacy_fallback_for_compatibility(self):
        old_path = os.environ.get("codexpanel_TERMINAL_JWT_SECRET_FILE")
        old_secret = os.environ.get("codexpanel_TERMINAL_JWT_SECRET")
        os.environ["codexpanel_TERMINAL_JWT_SECRET_FILE"] = "/path/that/does/not/exist"
        os.environ.pop("codexpanel_TERMINAL_JWT_SECRET", None)
