    ...
    try:
        ...
        if DeleteFileID != None:
            try:
                ...
                logging.CodexCPLogFileWriter.writeToFile("DeleteFileID ....... %s....msg.....%s" % (DeleteFileID, msg))
                Deleted = 0
        wm = WebsiteManager()
...
def get_website_resources(request):
    try:
        ...
        logging.CodexCPLogFileWriter.writeToFile(f'Error in get_website_resources: {str(msg)}')
...
def get_terminal_jwt(request):
    import logging
    logger = logging.getLogger("codexpanel.ssh.jwt")
    try:
        data = json.loads(request.body)
