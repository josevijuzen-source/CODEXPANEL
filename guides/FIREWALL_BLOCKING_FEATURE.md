# Firewall Blocking Feature for CodexPanel

## Overview

This feature adds a convenient "Block IP" button directly in the CodexPanel dashboard's SSH Security Analysis section, allowing administrators to quickly block malicious IP addresses without needing to access SSH or manually run firewall commands.
...
- **Admin-Only Access**: Restricted to administrators with CodexPanel addons
...
- **File**: `CodexPanel/baseTemplate/views.py`
- **Method**: POST
- **Authentication**: Admin-only with CodexPanel addons
- **Functionality**:
...
#### 2. URL Configuration
- **File**: `CodexPanel/baseTemplate/urls.py`
- **Route**: `re_path(r'^blockIPAddress$', views.blockIPAddress, name='blockIPAddress')`

### Frontend Changes

#### 1. Template Updates
- **File**: `CodexPanel/baseTemplate/templates/baseTemplate/homePage.html`
- **Changes**:
...
- **File**: `CodexPanel/baseTemplate/static/baseTemplate/custom-js/system-status.js`
- **Features**:
...
### Prerequisites
1. CodexPanel with admin privileges
2. CodexPanel addons enabled
3. Active firewalld service

### How to Use
1. Navigate to **Dashboard** in CodexPanel
...
2. **Premium Feature**: Requires CodexPanel addons
...
1. **"Premium feature required" error**
   - Ensure CodexPanel addons are enabled
...
Check CodexPanel logs for detailed error information:
- `/usr/local/CodexCP/logs/CodexPanel.log`
- Firewalld logs: `journalctl -u firewalld`

## Support

For issues or questions regarding this feature:
1. Check CodexPanel documentation
2. Review firewall configuration
3. Check system logs for detailed error messages
4. Contact CodexPanel support if needed

---

**Note**: This feature enhances CodexPanel's security capabilities by providing a streamlined way to block malicious IP addresses directly from the web interface, improving the overall user experience for server administrators.
