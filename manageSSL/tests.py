...
class TestManageSSL(TestCase):
    ...
    def test_issueSSL(self):
        ...
        command = 'rm -rf /home/%s/public_html/' % ('codexpanel.xyz')
        ProcessUtilities.normalExecutioner(command)

        command = 'mkdir /home/%s/public_html/' % ('codexpanel.xyz')
        ProcessUtilities.normalExecutioner(command)

        command = 'chown cyberpa:cyberpa /home/%s/public_html/' % ('codexpanel.xyz')
        ProcessUtilities.normalExecutioner(command)

        path = '/home/%s/public_html/index.html' % ('codexpanel.xyz')

        writeToFile = open(path, 'w')
        writeToFile.write('codexpanel')
        writeToFile.close()

        exists = 0
        if self.MakeRequestRaw('https://codexpanel.xyz').find('codexpanel') > -1:
            exists = 1
