...
class FirewallManager:
    ...
    def getCurrentRules(self, userID = None):
        try:
            ...
            # Ensure CodexPanel port 7080 rule exists in database for visibility
            CodexPanel_rule_exists = False
            for rule in rules:
                if rule.port == '7080':
                    CodexPanel_rule_exists = True
                    break
            
            if not CodexPanel_rule_exists:
                # Create database entry for port 7080 (already enabled in system firewall)
                try:
                    CodexPanel_rule = FirewallRules(
                        name="codexpanel Admin",
                        proto="tcp",
                        port="7080",
                        ipAddress="0.0.0.0/0"
                    )
                    CodexPanel_rule.save()
                    logging.CodexCPLogFileWriter.writeToFile("Added CodexPanel port 7080 to firewall database for UI visibility")
                except Exception as e:
                    logging.CodexCPLogFileWriter.writeToFile(f"Failed to add CodexPanel port 7080 to database: {str(e)}")
    ...
    def editRule(self, userID = None, data = None):
        ...
        try:
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"Firewall rule edited successfully. ID: {ruleID}, Name: {newRuleName}")
        ...
        try:
            ...
            if res == 1:
                ...
                final_dic = {'reload_status': 0,
                             'error_message': "Can not reload firewall, see CodexCP main log file."}
                final_json = json.dumps(final_dic)
                return HttpResponse(final_json)
        ...
        try:
            ...
            if res == 1:
                ...
                final_dic = {'start_status': 0,
                             'error_message': "Can not start firewall, see CodexCP main log file."}
                final_json = json.dumps(final_dic)
                return HttpResponse(final_json)
        ...
        try:
            ...
            if res == 1:
                ...
                final_dic = {'stop_status': 0,
                             'error_message': "Can not stop firewall, see CodexCP main log file."}
                final_json = json.dumps(final_dic)
                return HttpResponse(final_json)
    ...
    def installModSec(self, userID = None, data = None):
        try:
            ...
            execPath = "/usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/modSec.py"
    ...
    def saveModSecConfigurations(self, userID = None, data = None):
        try:
            ...
            if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
                ...
                tempConfigPath = "/home/codexpanel/" + str(randint(1000, 9999))
    ...
    def installCSF(self):
        try:
            ...
            execPath = "sudo /usr/local/CodexCP/bin/python " + virtualHostUtilities.codexpanel + "/plogical/csf.py"
    ...
    def modifyPorts(self, data = None):
        try:
            ...
            portsPath = '/home/codexpanel/' + str(randint(1000, 9999))
    ...
    def submitinstallImunify(self):
        try:
            ...
            if currentACL['admin'] == 1:
                ...
                logging.CodexCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath,
            ...
            logging.CodexCPLogFileWriter.statusWriter(ServerStatusUtil.lswsInstallStatusPath, str(msg) + ' [404].', 1)
    ...
    def saveLitespeed_conf(self, userID = None, data = None):
        try:
            ...
            tempRulesPath = '/home/codexpanel/pre_main_global.conf'
    ...
    def exportFirewallRules(self, userID = None):
        """
        Export all custom firewall rules to a JSON file, excluding default CodexPanel rules
        """
        try:
            currentACL = ACLManager.loadedACL(userID)
            ...
            # Default CodexPanel rules to exclude
            default_rules = ['codexpanel Admin', 'SSHCustom']
            ...
            logging.CodexCPLogFileWriter.writeToFile(f"Firewall rules exported successfully. Total rules: {len(custom_rules)}")
    ...
    def importFirewallRules(self, userID = None, data = None):
        ...
        try:
            ...
            # Default CodexPanel rules to exclude from import
            default_rules = ['codexpanel Admin', 'SSHCustom']
            ...
            for rule_data in import_data['rules']:
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile(f"Error importing rule {rule_data.get('name', 'Unknown')}: {str(e)}")
            
            logging.CodexCPLogFileWriter.writeToFile(f"Firewall rules import completed. Imported: {imported_count}, Skipped: {skipped_count}, Errors: {error_count}")
