...
class secMiddleware:
    ...
    def __call__(self, request):
                    ...
                    if type(value) == str or type(value) == bytes:
                        ...
                        for items in value:
                            if isinstance(items, str) and (items.find('- -') > -1 or items.find('\n') > -1 or items.find(';') > -1 or items.find(
                                ...
                                logging.writeToFile(request.body)
                                final_dic = {
                                    'error_message': "Data supplied is not accepted, following characters are not allowed in the input ` $ & ( ) [ ] { } ; : â€˜ < >.",
                                    "errorMessage": "Data supplied is not accepted, following characters are not allowed in the input ` $ & ( ) [ ] { } ; : â€˜ < >."}
                                final_json = json.dumps(final_dic)
                                return HttpResponse(final_json)
                    else:
                        continue
        ...
        # response['Content-Security-Policy'] = "default-src 'self' codexpanel.cloud *.codexpanel.cloud"
