...
# Logging setup
LOG_DIR="/var/log/codexpanel"
LOG_FILE="$LOG_DIR/codexpanel_install_$(date +%Y%m%d_%H%M%S).log"
DEBUG_LOG_FILE="$LOG_DIR/codexpanel_install_debug_$(date +%Y%m%d_%H%M%S).log"

# Create log directory if it doesn't exist
mkdir -p "$LOG_DIR" 2>/dev/null || {
    # If /var/log/codexpanel cannot be created, use /tmp
    LOG_DIR="/tmp/codexpanel_logs"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/codexpanel_install_$(date +%Y%m%d_%H%M%S).log"
    DEBUG_LOG_FILE="$LOG_DIR/codexpanel_install_debug_$(date +%Y%m%d_%H%M%S).log"
}
...
# Initialize logging
log_info "codexpanel installation started"
...
echo -e "Fetching latest data from CodexPanel server...\n"
echo -e "This may take few seconds..."
log_info "Fetching latest data from CodexPanel server"
...
setup_epel_repo() {
    case "$Server_OS_Version" in
        "7")
            rpm --import https://codexpanel.sh/dl.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-7
            yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm
            Check_Return "yum repo" "no_exit"
            ;;
        "8")
            rpm --import https://codexpanel.sh/dl.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-8
            yum install -y https://codexpanel.sh/dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
            Check_Return "yum repo" "no_exit"
            ;;
        "9")
            yum install -y https://codexpanel.sh/dl.fedoraproject.org/pub/epel/epel-release-latest-9.noarch.rpm
            Check_Return "yum repo" "no_exit"
            ;;
        "10")
...
Debug_Log() {
echo -e "\n${1}=${2}\n" >> "/var/log/codexpanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
}
...
log_info "Checking root privileges"
  ...
  if [[ $(id -u) != 0 ]] >/dev/null; then
    echo -e "\nYou must run on root user to install codexpanel...\n"
...
if [ -z "$XDG_CURRENT_DESKTOP" ]; then
    ...
    echo "$XDG_CURRENT_DESKTOP defined appears to be a desktop OS. Bailing as CodexPanel is incompatible."
    echo -e "\ncodexpanel is supported on server OS types only. Such as Ubuntu 18.04 x86_64, Ubuntu 20.04 x86_64, Ubuntu 20.10 x86_64, Ubuntu 22.04 x86_64, Ubuntu 24.04 x86_64, Ubuntu 24.04.3 x86_64, CentOS 8.x, AlmaLinux 8.x, AlmaLinux 9.x, AlmaLinux 10.x and CloudLinux 7.x...\n"
    exit
fi
...
if grep -q -E "CentOS Linux 7|CentOS Linux 8|CentOS Stream" /etc/os-release ; then
  ...
  echo -e "\ncodexpanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 24.04.3, CentOS 7, CentOS 8, CentOS 9, RHEL 8, RHEL 9, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03...\n"
  Debug_Log2 "codexpanel is supported on x86_64 based Ubuntu 18.04, Ubuntu 20.04, Ubuntu 20.10, Ubuntu 22.04, Ubuntu 24.04, Ubuntu 24.04.3, CentOS 7, CentOS 8, CentOS 9, RHEL 8, RHEL 9, AlmaLinux 8, AlmaLinux 9, AlmaLinux 10, RockyLinux 8, CloudLinux 7, CloudLinux 8, openEuler 20.03, openEuler 22.03... [404]"
  exit
fi
...
#  echo -e "\nLXC detected..."
#  echo -e "codexpanel does not support LXC"
#  echo -e "Exiting..."
#  Debug_Log2 "codexpanel does not support LXC.. [404]"
#  exit
#fi
#remove per https://github.com/usmannasir/CodexPanel/issues/589
...
Show_Help() {
echo -e "\ncodexpanel Installer Script Help\n"
echo -e "\nUsage: sh <(curl codexpanel.sh) --argument"
echo -e "\n\e[31m-v\e[39m or \e[31m--version\e[39m : choose to install CodexPanel OpenLiteSpeed or CodexPanel Enterprise, available options are \e[31mols\e[39m , \e[31mTRIAL\e[39m and \e[31mSERIAL_NUMBER\e[39m, default ols"
...
echo -e "\nExample:"
echo -e "\nsh <(curl codexpanel.sh) -v ols -p r or ./codexpanel.sh --version ols --password random"
echo -e "\nThis will install CodexPanel OpenLiteSpeed and randomly generate the password."
echo -e "\nsh <(curl codexpanel.sh) -v LICENSE_KEY -a -p my_pass_word"
...
Interactive_Mode() {
echo -e "		codexpanel Installer v$Panel_Version.$Panel_Build

1. Install codexpanel.
...
1. Install CodexPanel with \e[31mOpenLiteSpeed\e[39m.

2. Install CodexPanel with \e[31mLiteSpeed Enterprise\e[39m.
...
echo -e "\nInstall Full service for codexpanel? This will include PowerDNS, Postfix and Pure-FTPd."
echo -e ""
...
if [[ $(expr "x$Tmp_Input" : 'x[Yy]') -gt 1 ]] || [[ $Tmp_Input = "" ]]; then
  ...
  ### Ask if you want to set up this CodexPanel with remote MySQL
...
if [[ $Server_OS = "CentOS" ]] ; then
  ...
  rpm --import https://codexpanel.sh/rpms.litespeedtech.com/centos/RPM-GPG-KEY-litespeed
  ...
  if [[ "$Server_OS_Version" = "8" ]]; then
    rpm --import https://codexpanel.sh/www.centos.org/keys/RPM-GPG-KEY-CentOS-Official
...
EOF
    ...
    yum install --nogpg -y https://codexpanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el7.noarch.rpm
      Check_Return "yum repo" "no_exit"

    rpm -ivh https://codexpanel.sh/repo.iotti.biz/CentOS/7/noarch/lux-release-7-1.noarch.rpm
      Check_Return "yum repo" "no_exit"
...
[global]
index-url=https://codexpanel.sh/pip-repo/pypi/simple/
EOF
#default to self-host pip for CN

if [[ "$Server_Provider" = "Alibaba Cloud" ]] ; then
sed -i 's|https://codexpanel.sh/pip-repo/pypi/simple/|http://mirrors.cloud.aliyuncs.com/pypi/simple/|g' /root/.pip/pip.conf
echo "trusted-host = mirrors.cloud.aliyuncs.com" >> /root/.pip/pip.conf
fi

if [[ "$Server_Provider" = "Tencent Cloud" ]] ; then
sed -i 's|https://codexpanel.sh/pip-repo/pypi/simple/|https://mirrors.cloud.tencent.com/pypi/simple/|g' /root/.pip/pip.conf
fi
...
#  sed -i 's|http://mirrors.tencentyun.com/ubuntu/|https://codexpanel.sh/us.archive.ubuntu.com/ubuntu/|g' /etc/apt/sources.list
...
echo -e "Creating CodexPanel virtual environment..."
...
if [[ "$Server_OS" = "Ubuntu" ]] && ([[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]]) ; then
  ...
  if python3 -m venv /usr/local/codexpanel 2>&1; then
    ...
    virtualenv -p /usr/bin/python3 /usr/local/codexpanel
  fi
else
  virtualenv -p /usr/bin/python3 /usr/local/codexpanel
fi
...
if [[ ! -f /usr/local/codexpanel/bin/activate ]]; then
...
if [ "$Server_OS" = "Ubuntu" ]; then
  ...
  # shellcheck disable=SC1091
  source /usr/local/codexpanel/bin/activate
fi
...
if [[ "$Server_OS" = "CentOS" ]] ; then
  ...
  #disable SELinux
    ...
    if [[ "$Server_OS_Version" = "8" ]] ; then
        ...
        if [[ "$Server_Country" = "CN" ]] ; then
          sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://codexpanel.sh/litespeed/litespeed_cn.repo|g' install.py
        else
          sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://codexpanel.sh/litespeed/litespeed.repo|g' install.py
        fi
      fi
    fi
...
if ! grep -q "pid_max" /etc/rc.local 2>/dev/null ; then
  ...
  *                hard    nproc           65535
  ...
  for j in {1..6}; do
    ...
    if ping -c 1 -W 1 8.8.8.8 >/dev/null 2>&1 || nslookup codexpanel.sh >/dev/null 2>&1; then
      break
    fi
  done

  # Check Connectivity
  if ping -q -c 1 -W 1 codexpanel.sh >/dev/null; then
...
if [ -f /root/codexpanel-tmp ]; then
  rm -rf /root/codexpanel-tmp
fi

mkdir /root/codexpanel-tmp
cd /root/codexpanel-tmp || exit

Retry_Command "wget https://codexpanel.sh/www.litespeedtech.com/packages/${LSWS_Stable_Version:0:1}.0/lsws-$LSWS_Stable_Version-ent-x86_64-linux.tar.gz"
tar xzvf "lsws-$LSWS_Stable_Version-ent-x86_64-linux.tar.gz" >/dev/null
cd "/root/codexpanel-tmp/lsws-$LSWS_Stable_Version/conf"  || exit
...
cd "/root/codexpanel-tmp/lsws-$LSWS_Stable_Version/bin"  || exit
...
if [[ "$Server_OS" = "Ubuntu" ]] ; then
  sed -i 's|wget http://rpms.litespeedtech.com/debian/|wget https://codexpanel.sh/litespeed/|g' install.py
  sed -i 's|https://repo.dovecot.org/|https://codexpanel.sh/repo.dovecot.org/|g' install.py
fi
  #replace litespeed repo on ubuntu 18/20

if [[ "$Server_OS" = "CentOS" ]] ; then
  sed -i 's|rpm -ivh http://rpms.litespeedtech.com/centos/litespeed-repo-1.2-1.el7.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://codexpanel.sh/litespeed/litespeed_cn.repo|g' install.py
  sed -i 's|rpm -Uvh http://rpms.litespeedtech.com/centos/litespeed-repo-1.1-1.el8.noarch.rpm|curl -o /etc/yum.repos.d/litespeed.repo https://codexpanel.sh/litespeed/litespeed_cn.repo|g' install.py
  sed -i 's|https://mirror.ghettoforge.org/distributions|https://codexpanel.sh/mirror.ghettoforge.net/distributions|g' install.py
  ...
  sed -i 's|dnf --nogpg install -y https://codexpanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el8.noarch.rpm|echo gf8|g' install.py

  Retry_Command "dnf --nogpg install -y https://codexpanel.sh/mirror.ghettoforge.net/distributions/gf/gf-release-latest.gf.el8.noarch.rpm"
  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/gf/\$basearch/mirrorlist|baseurl=https://codexpanel.sh/mirror.ghettoforge.net/distributions/gf/el/8/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
  sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/plus/\$basearch/mirrorlist|baseurl=https://codexpanel.sh/mirror.ghettoforge.net/distributions/gf/el/8/plus/x86_64/|g" /etc/yum.repos.d/gf.repo
  ...
  if [[ "$Server_OS_Version" = "9" ]] || [[ "$Server_OS_Version" = "10" ]] ; then
    ...
    sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/gf/\$basearch/mirrorlist|baseurl=https://codexpanel.sh/mirror.ghettoforge.net/distributions/gf/el/9/gf/x86_64/|g" /etc/yum.repos.d/gf.repo
    sed -i "s|mirrorlist=http://mirrorlist.ghettoforge.org/el/8/plus/\$basearch/mirrorlist|baseurl=https://codexpanel.sh/mirror.ghettoforge.net/distributions/gf/el/9/plus/x86_64/|g" /etc/yum.repos.d/gf.repo
  fi
fi
...
sed -i 's|$PROJECT/archive/$BRANCH.tar.gz|https://codexpanel.sh/codeload.github.com/acmesh-official/acme.sh/tar.gz/master|g' /root/.acme.sh/acme.sh
...
/usr/local/codexpanel/bin/python install.py "${Final_Flags[@]}"


if grep "codexpanel installation successfully completed" /var/log/installLogs.txt >/dev/null; then
  echo -e "\ncodexpanel installation sucessfully completed...\n"
...
Post_Install_Addon_Mecached_LSMCD() {
  ...
  wget -O lsmcd-master.zip https://codexpanel.sh/codeload.github.com/litespeedtech/lsmcd/zip/master
...
chmod +x /usr/local/CodexCP/CPScripts/setup_php_sessions.sh
bash /usr/local/CodexCP/CPScripts/setup_php_sessions.sh
...
Current_Dir="$(pwd)"
rm -f /usr/local/lsws/codexpanel-tmp
mkdir /usr/local/lsws/codexpanel-tmp
cd /usr/local/lsws/codexpanel-tmp || exit

# Try to download timezonedb, but continue if it fails
wget -O timezonedb.tgz https://codexpanel.sh/pecl.php.net/get/timezonedb
if [ ! -f timezonedb.tgz ] || [ ! -s timezonedb.tgz ]; then
    log_info "WARNING: Failed to download timezonedb, skipping installation"
    cd "$Current_Dir" || exit
    rm -rf /usr/local/lsws/codexpanel-tmp
    return 0
fi
...
echo "${Webadmin_Pass}" >/etc/codexpanel/webadmin_passwd
...
if [[ "$Watchdog" = "On" ]]; then
  ...
  wget -O /etc/codexpanel/watchdog.sh "${Git_Content_URL}/stable/CPScripts/watchdog.sh"
...
snappymailAdminPass=$(grep SetPassword /usr/local/CodexCP/public/snappymail.php| sed -e 's|$oConfig->SetPassword(||g' -e "s|');||g" -e "s|'||g")
Elapsed_Time="$((Time_Count / 3600)) hrs $(((SECONDS / 60) % 60)) min $((Time_Count % 60)) sec"
echo "###################################################################"
echo "                CodexPanel Successfully Installed                  "
echo "                                                                   "
...
echo "                                                                   "
echo -e "             Run \e[31mcodexpanel help\e[39m to get FAQ info"
echo -e "             Run \e[31mcodexpanel upgrade\e[39m to upgrade it to latest version."
echo -e "             Run \e[31mcodexpanel utility\e[39m to access some handy tools ."
echo "                                                                   "
echo "              Website : https://www.codexpanel.net                 "
echo "              Forums  : https://forums.codexpanel.net              "
echo "              Wikipage: https://codexpanel.net/KnowledgeBase/                "
echo "              Docs    : https://codexpanel.net/docs/               "
echo "                                                                   "
echo -e "            Enjoy your accelerated Internet by                  "
echo -e "                CodexPanel & $Word 				                     "
echo "###################################################################"
...
echo -e "\033[0;32mTCP: 8090\033[39m for codexpanel"
...
cat <<EOF >/root/codexpanel/cert_conf
[req]
prompt=no
...
EOF
openssl req -x509 -config /root/codexpanel/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout /usr/local/lscp/conf/key.pem -out /usr/local/lscp/conf/cert.pem
...
openssl req -x509 -config /root/codexpanel/cert_conf -extensions 'server_exts' -nodes -days 820 -newkey rsa:2048 -keyout "$Key_Path" -out "$Cert_Path"
...
Install_CodexCP_Runtime_Python_Requirements() {
  ...
  if [[ -n "$req_hint" && -f "$req_hint" ]] && grep -q "Django==" "$req_hint" 2>/dev/null; then
    ...
    local td="/tmp/codexpanel-req-inst.$$"
...
echo -e "Creating CodexCP virtual environment..."
...
if [[ "$Server_OS" = "Ubuntu" ]] && ([[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]]) ; then
  echo -e "Ubuntu 22.04/24.04 detected, using python3 -m venv..."
  if python3 -m venv /usr/local/CodexCP 2>&1; then
    echo -e "Virtual environment created successfully"
  else
    echo -e "python3 -m venv failed, trying virtualenv..."
    # Ensure virtualenv is properly installed
    pip3 install --upgrade virtualenv
    virtualenv -p /usr/bin/python3 /usr/local/CodexCP
  fi
  ...
  if python3 -m venv /usr/local/CodexCP 2>&1; then
    ...
    virtualenv -p "$PYTHON_PATH" /usr/local/CodexCP
  fi
else
  virtualenv -p /usr/bin/python3 /usr/local/CodexCP
fi
...
if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "20" ]] ; then
  # shellcheck disable=SC1091
  . /usr/local/CodexCP/bin/activate
   Check_Return
else
  # shellcheck disable=SC1091
...
if [[ "$Server_OS" = "Ubuntu" ]] && ([[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]]) ; then
  # Ubuntu 24.04 ships with Python 3.12, but using 3.10 for compatibility with codexpanel
  cp /usr/bin/python3.10 /usr/local/CodexCP/bin/python3
else
...
chown -R codexpanel:codexpanel /usr/local/CodexCP/lib64 || true
}
...
rm -rf /etc/profile.d/codexpanel*
curl --silent -o /etc/profile.d/codexpanel.sh https://codexpanel.sh/?banner 2>/dev/null
chmod 700 /etc/profile.d/codexpanel.sh
echo "$Admin_Pass" > /etc/codexpanel/adminPass
chmod 600 /etc/codexpanel/adminPass
/usr/local/codexpanel/bin/python /usr/local/CodexCP/plogical/adminPass.py --password "$Admin_Pass"
mkdir -p /etc/opendkim

echo '/usr/local/codexpanel/bin/python /usr/local/CodexCP/plogical/adminPass.py --password "$@"' > /usr/bin/adminPass
echo "systemctl restart lscpd" >> /usr/bin/adminPass
echo "echo \$@ > /etc/codexpanel/adminPass" >> /usr/bin/adminPass
...
HostName=$(hostname --fqdn); [ -n "$(dig @1.1.1.1 +short "$HostName")" ]  &&  echo "$HostName resolves to valid IP. Setting up hostname SSL" && CodexPanel createWebsite --package Default --owner admin --domainName $(hostname --fqdn) --email root@localhost --php 7.4 && CodexPanel hostNameSSL --domainName $(hostname --fqdn)


}

Post_Install_CN_Replacement() {
sed -i 's|wp core download|wp core download https://codexpanel.sh/wordpress.org/latest.tar.gz|g' /usr/local/CodexCP/plogical/applicationInstaller.py
sed -i 's|https://raw.githubusercontent.com/|https://codexpanel.sh/raw.githubusercontent.com/|g' /usr/local/CodexCP/plogical/applicationInstaller.py
sed -i 's|wp plugin install litespeed-cache|wp plugin install  https://codexpanel.sh/downloads.wordpress.org/plugin/litespeed-cache.zip|g' /usr/local/CodexCP/plogical/applicationInstaller.py
...
echo -e "\nInitializing...\n"
log_info "============================================="
log_info "codexpanel installation script started"
...
if [[ "$*" = *"--debug"* ]] ; then
  Debug="On"
  find /var/log -name 'codexpanel_debug_*' -exec rm {} +
  Random_Log_Name=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 5)
  echo -e "$(date)" > "/var/log/codexpanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
  chmod 600 "/var/log/codexpanel_debug_$(date +"%Y-%m-%d")_${Random_Log_Name}.log"
fi
