...
class IMAPClient:
    ...
    codexpanel's Dovecot uses namespace: separator='.', prefix='INBOX.'
    ...
    # (codexpanel creates these in mailUtilities.py)
    SPECIAL_FOLDERS = {
        'sent': 'INBOX.Sent',
        'drafts': 'INBOX.Drafts',
    ...
    def delete_messages(self, folder, uids):
        self._select(folder)
        uid_str = ','.join(str(u) for u in uids)
        # codexpanel/Dovecot uses "INBOX.Deleted Items" as trash
