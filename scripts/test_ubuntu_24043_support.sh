...
# Test script for Ubuntu 24.04.3 support in codexpanel
# This script verifies that CodexPanel properly detects and handles Ubuntu 24.04.3
...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$NAME" == "Ubuntu" ]] && [[ "$VERSION" == *"24.04.3"* ]]; then
        echo "âœ… Ubuntu 24.04.3 detected"
    else
        echo "âš ï¸  This test is designed for Ubuntu 24.04.3"
...
if [ "$VERSION_ID" = "24" ]; then
    echo "âœ… Version parsing correct (24)"
else
    echo "âŒ Version parsing incorrect (expected: 24, got: $VERSION_ID)"
fi
...
if command -v python3 &> /dev/null; then
    ...
    if [[ "$PYTHON_VERSION" == "3.12" ]]; then
        echo "âœ… Python 3.12 detected (expected for Ubuntu 24.04.3)"
    else
        echo "âš ï¸  Python version $PYTHON_VERSION (Ubuntu 24.04.3 typically has Python 3.12)"
    fi
else
...
if command -v apt &> /dev/null; then
    ...
    if apt list --installed | grep -q "ubuntu-release"; then
        echo "âœ… Ubuntu release packages found"
    else
        echo "âš ï¸  Ubuntu release packages not found"
    fi
else
    echo "âŒ APT package manager not found"
fi
...
if command -v python3 -m venv --help &> /dev/null; then
    echo "âœ… Python3 venv module available"
    
    # Test creating a virtual environment
    TEST_VENV="/tmp/codexpanel_test_venv"
    if python3 -m venv "$TEST_VENV" 2>/dev/null; then
        echo "âœ… Virtual environment creation successful"
        rm -rf "$TEST_VENV"
    else
        echo "âŒ Virtual environment creation failed"
    fi
else
    echo "âŒ Python3 venv module not available"
fi

# Test 6: CodexPanel version detection
echo ""
echo "Test 6: CodexPanel Version Detection"
echo "------------------------------------"
if [ -f /usr/local/CodexCP/plogical/upgrade.py ]; then
...
try:
    ...
    if os_type == 9:  # Ubuntu24 constant
        print('âœ… Ubuntu 24.04 detection working')
    else:
        print(f'âš ï¸  OS type {os_type} detected (expected: 9 for Ubuntu24)')
except Exception as e:
    print(f'âŒ Error testing OS detection: {e}')
" 2>/dev/null; then
        echo "âœ… CodexPanel OS detection test completed"
    else
        echo "âŒ CodexPanel OS detection test failed"
    fi
else
    echo "âš ï¸  CodexPanel not installed - skipping detection test"
fi
...
if uname -m | grep -qE 'x86_64|aarch64'; then
    ...
    echo "âŒ Unsupported architecture"
fi
