...
def validate_access_token(token, scan_id):
    ...
    try:
        ...
        # OPTION 2: Try codexpanel's own API Key (for post-scan file operations from platform)
        # The platform sends back the same API key that CodexPanel used to submit the scan
        try:
            ...
            # First, check if this is a valid CodexPanel API key (any admin's key)
...
def scan_callback(request):
    ...
    try:
        ...
        # Update scan status in CodexPanel database
        try:
