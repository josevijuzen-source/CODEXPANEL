...
class FTPManager:
    ...
    def createFTPAccount(self):
        ...
        if not os.path.exists('/home/codexpanel/pureftpd'):
    ...
    def installPureFTPD(self):
        ...
        command = 'id -u ftpuser >/dev/null 2>&1 || useradd -u 2001 -s /bin/false -d /bin/null -c "pureftpd user" -g ftpgroup ftpuser'
        ProcessUtilities.executioner(command, 'root', True)
    ...
    def startPureFTPD(self):
        ...
        if output.strip() != 'active':
            logging.CodexCPLogFileWriter.writeToFile('[ERROR] Pure-FTPd failed to start. Service status: ' + output)
    ...
    def installPureFTPDConfigurations(self, mysqlPassword):
        ...
        #         shutil.copytree("/usr/local/CodexCP/install/pure-ftpd-one", ftpdPath)
        #
        #     else:
        #         shutil.copytree("/usr/local/CodexCP/install/pure-ftpd-one", ftpdPath)
        #
        ...
        try:
            ...
            try:
                ...
                if e.errno != errno.EEXIST:
                    logging.CodexCPLogFileWriter.writeToFile("[ERROR] Could not create directory for FTP SSL: " + str(e))
                    raise
            ...
            cwd = os.getcwd()
            os.chdir('/usr/local/CodexCP/install')
            ftpdPath = "/etc/pure-ftpd"
            import shutil
            ...
            if ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu or ProcessUtilities.decideDistro() == ProcessUtilities.ubuntu20:
                try:
                    ...
                    logging.CodexCPLogFileWriter.writeToFile("[ERROR] Error creating extra pure-ftpd directories: " + str(msg), ".  Should be ok", 1)
                    logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], "[ERROR] Error creating extra pure-ftpd directories: " + str(msg), "" + " [installPureFTPDConfigurations][404]")
                    return 0
            ...
            logging.CodexCPLogFileWriter.writeToFile('[ERROR] ' + str(msg) + " [installPureFTPDConfigurations]")
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], '[ERROR] ' + str(msg) + " [installPureFTPDConfigurations][404]")
            return 0
    ...
    def ResetFTPConfigurations(self):
        try:
            ...
            logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'Removing and re-installing FTP..,5')
            ...
            if self.installPureFTPDConfigurations(settings.DATABASES['default']['PASSWORD']) == 0:
                logging.CodexCPLogFileWriter.statusWriter(self.extraArgs['tempStatusPath'], 'installPureFTPDConfigurations failed. [404].')
                return 0
